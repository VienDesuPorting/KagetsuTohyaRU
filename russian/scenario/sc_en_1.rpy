# TODO: Translation updated at 2022-06-01 13:43

# game/scenario/sc_en.rpy:9
translate russian s1_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:10
translate russian s1_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:26
translate russian s1_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:27
translate russian s1_0af6a5fa_2:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:28
translate russian s1_0af6a5fa_3:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:29
translate russian s1_0af6a5fa_4:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:30
translate russian s1_0af6a5fa_5:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:31
translate russian s1_0af6a5fa_6:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:32
translate russian s1_a5a032cb:

    # "--------I open my eyes from my slumber."
    "--------Я просыпаюсь и открываю глаза."

# game/scenario/sc_en.rpy:33
translate russian s1_1bc18805:

    # " \ \ \ \ \ \ \ It seems morning has come."
    " \ \ \ \ \ \ \  Похоже, наступило утро."

# game/scenario/sc_en.rpy:34
translate russian s1_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:41
translate russian s1_69c6f07a:

    # " I wake up to a soft morning sunlight."
    " Я проснулся от мягкого утреннего солнечного света."

# game/scenario/sc_en.rpy:42
translate russian s1_c0e8cebe:

    # " Maybe I fell asleep quickly last night, but my body feels completely refreshed."
    " Возможно я рано уснул вчера, потому как чувствую себя полностью отдохнувшим."

# game/scenario/sc_en.rpy:43
translate russian s1_1a54eb51:

    # " It almost feels as if it's weird to wake up now."
    " Почти странно просыпаться прямо сейчас."

# game/scenario/sc_en.rpy:44
translate russian s1_b4fac766:

    # nvl clear
    # "\"Mmmmm.\""
    nvl clear
    "\"Ммммм.\""

# game/scenario/sc_en.rpy:46
translate russian s1_6c25fcb5:

    # " I stretch after propping my body up."
    " Я приподнимаюсь и потягиваюсь."

# game/scenario/sc_en.rpy:47
translate russian s1_7fec1e3d:

    # " The sun is an autumn one.{w} Not cold, not hot.{w} Just perfect."
    " Солнце как осенью.{w} Не холодно, не жарко.{w} Просто идеально."

# game/scenario/sc_en.rpy:48
translate russian s1_000238c6:

    # " But I can tell from the stillness of the air that Hisui hasn't come yet."
    " По тишине витающей в воздухе могу сказать, что Хисуи ещё не пришла."

# game/scenario/sc_en.rpy:49
translate russian s1_624fb8f8:

    # " Hisui opens the door and goes out before I wake up.{w} And then, after a little while, she comes back to wake me up."
    " Обычно Хисуи открывает дверь и выходит до того как я проснусь.{w} А чуть позже она возвращается, чтобы разбудить меня."

# game/scenario/sc_en.rpy:50
translate russian s1_7e84ec41:

    # nvl clear
    # "\"Then| I guess it's not seven o'clock yet.\""
    nvl clear
    "\"Тогда думаю ещё нет и семи часов.\""

# game/scenario/sc_en.rpy:52
translate russian s1_169e94ec:

    # " The clock tells me it's half past six.{w} It must be time for Hisui to come."
    " Часы говорили мне, что сейчас пол-сельмого.{w} Обычно, в это время приходит Хисуи."

# game/scenario/sc_en.rpy:53
translate russian s1_94a94119:

    # "\"It's weird that I woke up before Hisui came.{w} I didn't do anything differently yesterday.\""
    "\"Странно, что я проснулся до того как пришла Хисуи.{w} Я не делал ничего необычного вчера.\""

# game/scenario/sc_en.rpy:54
translate russian s1_5c517283:

    # " Yesterday|{w} yesterday|"
    " Вчера,{w} вчера"

# game/scenario/sc_en.rpy:55
translate russian s1_fc80872f:

    # " I try to think of when I went to sleep, and all my thoughts stop."
    " Я пытаюсь вспомнить когда заснул, но все мои мысли останавливаются."

# game/scenario/sc_en.rpy:56
translate russian s1_fcb52b04:

    # nvl clear
    # "\"| Eh?\""
    nvl clear
    "\"А?\""

# game/scenario/sc_en.rpy:58
translate russian s1_b566e495:

    # " Yesterday| what did I do yesterday|?"
    " Вчера, что я делал вчера?"

# game/scenario/sc_en.rpy:59
translate russian s1_0a6d07df:

    # " I don't remember what I did yesterday at all.{w} Maybe it's because yesterday was just another normal day.{w} But I can't even remember when I went to bed|{w} Something feels off."
    " Я вообще не помню что было вчера.{w} Может быть потому что вчера был очередной ничем не примечательный день.{w} Но я даже не могу вспомнить когда лёг спать.{w} Что-то не так."

# game/scenario/sc_en.rpy:60
translate russian s1_92033f44:

    # "\"Hold on, then| what did I do the day before yesterday?\""
    " Стоп, тогда что я делал позавчера?"

# game/scenario/sc_en.rpy:61
translate russian s1_baa516a1:

    # " I put my hand on my forehead and think."
    " Я положил руку на лоб и начал думать."

# game/scenario/sc_en.rpy:62
translate russian s1_c3a98c8a:

    # nvl clear
    # "\"------\""
    nvl clear
    "\"------\""

# game/scenario/sc_en.rpy:64
translate russian s1_2328f00f:

    # " The day before yesterday|{w} Today's Wednesday|{w} so it must've been Monday."
    " Позавчера,{w} сегодня среда,{w} значит тогда был понедельник"

# game/scenario/sc_en.rpy:65
translate russian s1_b01c389c:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:66
translate russian s1_0db01b26:

    # " I must have gone to school, since it was a school day."
    " Скорее всего я ходил в школу, так как это учебный день."

# game/scenario/sc_en.rpy:67
translate russian s1_b01c389c_1:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:68
translate russian s1_86f227b7:

    # " Then what did I do at school|"
    " Тогда что я делал в школе?"

# game/scenario/sc_en.rpy:69
translate russian s1_b0de56c9:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:70
translate russian s1_f35da17b:

    # " Well, it must have been a day just like yesterday."
    " Что ж, видимо это был такой же день, как и вчерашний."

# game/scenario/sc_en.rpy:71
translate russian s1_79d82bf5:

    # nvl clear
    # "\"|| Hey|\""
    nvl clear
    "\"Так\""

# game/scenario/sc_en.rpy:73
translate russian s1_c3abc61d:

    # " Maybe I'm not fully awake yet."
    " Может быть я не до конца проснулся."

# game/scenario/sc_en.rpy:74
translate russian s1_92389040:

    # " For some reason, when I try to remember, my mind turns blank."
    " Почему-то, когда я пытаюсь всё это вспоминать, моя голова пустеет."

# game/scenario/sc_en.rpy:75
translate russian s1_9dcbf016:

    # " What is this|{w} I can feel my current existence, but everything else is hard to grasp."
    " Что это такое?{w} \[Я ощущаю моё текущее существование, но всё остальное трудно осознать\]."

# game/scenario/sc_en.rpy:76
translate russian s1_32aa0d9b:

    # "------No| maybe|"
    "------\[Нет, возможно\]"

# game/scenario/sc_en.rpy:77
translate russian s1_8c71c4c5:

    # " There's one thing."
    " Есть ещё кое что."

# game/scenario/sc_en.rpy:78
translate russian s1_ebff3e2f:

    # " It feels like I forgot something big| or I had something unspeakable happen to me|"
    " Мне кажется я забыл что-то важное или, возможно, со мной случилось что-то невообразимое"

# game/scenario/sc_en.rpy:79
translate russian s1_9bb493cc:

    # nvl clear
    # "\"| It hurts|\""
    nvl clear
    "\"Больно\""

# game/scenario/sc_en.rpy:81
translate russian s1_2488d666:

    # " When I think hard, the back of my head hurts."
    " Когда я отчаянно пытаюсь что-то вспомнить, мой затылок начинает болеть."

# game/scenario/sc_en.rpy:82
translate russian s1_150071ed:

    # " I touch it, and there's a little bump."
    " Когда я касаюсь его, я обнаруживаю небольшую шишку."

# game/scenario/sc_en.rpy:83
translate russian s1_2b45ff7c:

    # "\"|?\""
    "\"?\""

# game/scenario/sc_en.rpy:84
translate russian s1_25695884:

    # " I think maybe I hit something while I was sleeping|{nw}"
    " Наверное я обо что-то ударился пока спал.{nw}"

# game/scenario/sc_en.rpy:87
translate russian s1_ee9c2d6c:

    # " Knock, knock.{w} Someone's at the door."
    " Тук, тук.{w} Кто-то у двери."

# game/scenario/sc_en.rpy:88
translate russian s1_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:92
translate russian s1_522a2fba:

    # "\"Excuse me, Shiki-sama.\""
    "\"Простите, Шики-сама.\""

# game/scenario/sc_en.rpy:93
translate russian s1_21f7c014:

    # " Whispering that, Hisui opens the door."
    " Прошептав это, Хисуи открыла дверь."

# game/scenario/sc_en.rpy:94
translate russian s1_99befc6f:

    # "\"Hey.{w} Did you sleep well, Hisui?\""
    "\"Хей.{w} Ты хорошо поспала, Хисуи?\""

# game/scenario/sc_en.rpy:95
translate russian s1_13bffe73:

    # " I wave my hand."
    " Я машу рукой."

# game/scenario/sc_en.rpy:96
translate russian s1_b0de56c9_1:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:97
translate russian s1_63c95f20:

    # " Hisui raises her eyebrows in surprise, and then|{nw}"
    " Хисуи подняла брови в удивлении и затем:{nw}"

# game/scenario/sc_en.rpy:101
translate russian s1_3c0d2797:

    # "\"Good morning, Shiki-sama.\""
    "\"Доброе утро, Шики-сама.\""

# game/scenario/sc_en.rpy:102
translate russian s1_f2bd7f77:

    # " She quietly greets me."
    " Она тихо поприветствовала меня."

# game/scenario/sc_en.rpy:103
translate russian s1_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:107
translate russian s1_f6bfb826:

    # "\"You are awake today.{w} Then I'll bring your change of clothes immediately.{w} Please wait a moment.\""
    "\"Вы проснулись.{w} Тогда я немедленно принесу вашу сменную одежду.{w} Пожалуйста, подождите немного.\""

# game/scenario/sc_en.rpy:115
translate russian s1_d66e4237:

    # " Hisui walks out of the room as usual."
    " Хисуи как обычно вышла из комнаты."

# game/scenario/sc_en.rpy:116
translate russian s1_191ba422:

    # " | Well, since I can't even remember yesterday, I probably shouldn't be saying 'as usual.'"
    " Чтож, если я не могу вспомнить вчерашний день, наверное мне не следует говорить 'как обычно'"

# game/scenario/sc_en.rpy:117
translate russian s1_1ac6a201:

    # nvl clear
    # "\"Ah, just a sec.{w} I have something to ask.\""
    nvl clear
    "\"Ах, подожди секунду.{w} Мне нужно кое-что спросить.\""

# game/scenario/sc_en.rpy:122
translate russian s1_2f9841ff:

    # "\"Yes, what is it, Shiki-sama?\""
    "\"Да, что такое Шики-сама?\""

# game/scenario/sc_en.rpy:123
translate russian s1_2dd8b6cc:

    # "\"Ah, it's nothing serious, but can you tell me what I did yesterday?\""
    "\"Ничего такого, но ты можешь сказать мне что я делал вчера?\""

# game/scenario/sc_en.rpy:127
translate russian s1_949bfdc3:

    # " Ah|{w} for some reason, she looks really uncomfortable."
    " Ох,{w} по какой то причине она выглядит очень сконфуженной."

# game/scenario/sc_en.rpy:128
translate russian s1_286a2a55:

    # nvl clear
    # "\"Eh| you don't have to tell me in detail.{w} Tell me things like when I returned, what we had for dinner, whether Akiha got mad| things like that.\""
    nvl clear
    "\"Э, ты можешь не рассказывать детально.{w} Расскажи мне например когда я вернулся, что мы ели на ужин, сердилась ли Акиха или что-то в этом роде.\""

# game/scenario/sc_en.rpy:133
translate russian s1_b0de56c9_2:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:134
translate russian s1_5c101c03:

    # " Hisui bows her head apologetically.{w} Whether she really doesn't know, or my behaviour yesterday was extremely embarrassing, she looks really uncomfortable talking about it."
    " Хисуи склоняет голову в извинении.{w} Действительно ли она не знает, или моё поведение вчера попросту слишком смущающее, похоже ей и правда очень неудобно говорить об этом."

# game/scenario/sc_en.rpy:135
translate russian s1_13b41d40:

    # " | For some reason, I can't help but feel I'm abusing her|"
    " Почему то я чувствую, что таким образом оскорбляю её."

# game/scenario/sc_en.rpy:136
translate russian s1_c6e3d23c:

    # nvl clear
    # "\"Sorry| I asked something stupid.{w} I did it without thinking, so you don't have to worry about it.\""
    nvl clear
    "\"Извини, я спросил глупость.{w} Я сделал это не подумав, так что не беспокойся об этом.\""

# game/scenario/sc_en.rpy:140
translate russian s1_a9911db3:

    # " Hisui looks up with apologetic eyes."
    " Хисуи смотрит на меня извиняющимися глазами."

# game/scenario/sc_en.rpy:141
translate russian s1_7a6625d7:

    # "\"Now, now, don't worry about it.{w} Could you bring me the change of clothes?{w} I can't go to the dining room in this.\""
    "\"Не стоит беспокоиться об этом.{w} Не могла бы ты принести мне сменную одежду?{w} Я не могу пойти завтракать вот так.\""

# game/scenario/sc_en.rpy:144
translate russian s1_b01c389c_2:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:152
translate russian s1_dde0b5c3:

    # " Hisui leaves the room, even though it looks like she wants to say something."
    " Хисуи вышла из комнаты, хотя она выглядела так, будто хотела что-то сказать."

# game/scenario/sc_en.rpy:153
translate russian s1_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:168
translate russian s2_d67632be:

    # "This scene is \"Morning of Beginning - What do I do today?\"{nw}"
    "Эта сцена - \"Утро начала - Что я буду делать сегодня?\"{nw}"

# game/scenario/sc_en.rpy:169
translate russian s2_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:170
translate russian s2_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:186
translate russian s2_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:187
translate russian s2_0af6a5fa_2:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:188
translate russian s2_0af6a5fa_3:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:189
translate russian s2_0af6a5fa_4:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:190
translate russian s2_2fc73b13:

    # " I dreamt a ridiculous dream."
    " Мне снился нелепый сон."

# game/scenario/sc_en.rpy:191
translate russian s2_557bafa4:

    # " Something really ridiculous."
    " Что-то действительно нелепое."

# game/scenario/sc_en.rpy:192
translate russian s2_eec7499e:

    # " Whether it was ridiculously fun{nw}"
    " Настолько нелепое, что даже смешное{nw}"

# game/scenario/sc_en.rpy:193
translate russian s2_40e597d5:

    # " Or ridiculously scary{nw}"
    " Или пугающе нелепое{nw}"

# game/scenario/sc_en.rpy:194
translate russian s2_948d5f9f:

    # " I can't even tell."
    " Не могу сказать."

# game/scenario/sc_en.rpy:195
translate russian s2_f1f32704:

    # " It was a messed up dream."
    " Это был странный сон."

# game/scenario/sc_en.rpy:196
translate russian s2_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:202
translate russian s2_5f1b45d6:

    # "\"Shiki-sama?{w} Are you awake, Shiki-sama?\""
    "\"Шики-сама?{w} Вы проснулись, Шики-сама?\""

# game/scenario/sc_en.rpy:203
translate russian s2_a4ea8ded:

    # " The careful voice of Hisui."
    " Осторожный голос Хисуи."

# game/scenario/sc_en.rpy:204
translate russian s2_bf2dccb4:

    # "\"It's time to wake up.{w} Won't you wake up for today, Shiki-sama?\""
    "\"Время просыпаться.{w} Вы не проснётесь сегодня, Шики-сама?\""

# game/scenario/sc_en.rpy:205
translate russian s2_a83ad203:

    # " How can I not wake up after hearing such words?"
    " Как я могу не проснуться, услышав такие слова?"

# game/scenario/sc_en.rpy:206
translate russian s2_b01c389c:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:207
translate russian s2_0f996486:

    # " I force my still tired body to wake up."
    " Я напрягаю своё всё ещё уставшее тело и просыпаюсь."

# game/scenario/sc_en.rpy:208
translate russian s2_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:214
translate russian s2_ddbd64a6:

    # "\"Did you have a good night's rest, Shiki-sama?\""
    "\"Вы хорошо отдохнули, Шики-сама?\""

# game/scenario/sc_en.rpy:215
translate russian s2_4abd60ca:

    # "\"Mmm| morning, Hisui.{w} Sorry you have to deal with a person like me.\""
    "\"Ммм, утречка, Хисуи.{w} Извини что тебе приходится иметь дело с таким человеком как я.\""

# game/scenario/sc_en.rpy:218
translate russian s2_4f8366d6:

    # "\"No, it's nothing like that.{w} I enjoy waking you up in the morning.\""
    "\"Нет, ничего такого.{w} Мне нравится будить вас по утрам.\""

# game/scenario/sc_en.rpy:219
translate russian s2_69c6e56b:

    # "\"Oh?{w} Okay| well, if you enjoy it I guess it's okay|\""
    "\"Да?{w} Хорошо, раз тебе это нравится, то всё в порядке\""

# game/scenario/sc_en.rpy:220
translate russian s2_5ff37c4e:

    # " I would hate to be doing what she is doing."
    " Я бы не хотел делать то, что делает она."

# game/scenario/sc_en.rpy:223
translate russian s2_5f57d498:

    # " | Eh|{w} Hisui is carefully looking at my face."
    " Э?{w} Хисуи осторожно смотрит на моё лицо."

# game/scenario/sc_en.rpy:224
translate russian s2_71dfcd11:

    # nvl clear
    # "\"Shiki-sama, you don't look well.{w} Is your health okay?\""
    nvl clear
    "\"Шики-сама вы выглядите не хорошо.{w} Вы в порядке?\""

# game/scenario/sc_en.rpy:226
translate russian s2_1e1f7b3d:

    # "\"Yeah| I feel normal.{w} I'm not even sweating.\""
    "\"Да, я в норме.{w} Я даже не вспотел.\""

# game/scenario/sc_en.rpy:227
translate russian s2_cd7d45f2:

    # " I bring my hand up to my forehead as I speak."
    " Я кладу свою руку на лоб когда говорю это."

# game/scenario/sc_en.rpy:228
translate russian s2_803ec7a4:

    # " | Sweat.{w} Now that I think about it, I dreamt something weird last night|"
    " \[Пот\].{w} Теперь, когда я задумался над этим, я вспомнил, что видел нечто странное этой ночью."

# game/scenario/sc_en.rpy:229
translate russian s2_c496b079:

    # nvl clear
    # "\"Well, it's nothing serious.{w} I'll try waking up by myself, too.{w} All right!{w} Maybe I'll buy an alarm clock today!\""
    nvl clear
    "\"Что ж, ничего серьёзного.{w} Я попытаюсь просыпаться самостоятельно.{w} Всё хорошо!{w} Может быть сегодня я куплю будильник!\""

# game/scenario/sc_en.rpy:231
translate russian s2_8f9d0dbf:

    # " I clap my hands as I suggest it."
    " Я хлопаю в ладоши, когда говорю это."

# game/scenario/sc_en.rpy:234
translate russian s2_8e7179fa:

    # " And Hisui just stands there with a reluctant expression."
    " А Хисуи просто стоит с неохотным выражением лица."

# game/scenario/sc_en.rpy:235
translate russian s2_86c9e3d2:

    # nvl clear
    # "\"Hisui|?\""
    nvl clear
    "\"Хисуи?\""

# game/scenario/sc_en.rpy:239
translate russian s2_671e7078:

    # "\"Shiki-sama| do you dislike me waking you up every morning?\""
    "\"Шики-сама, вам не нравится когда я бужу вас по утрам?\""

# game/scenario/sc_en.rpy:240
translate russian s2_3aa3766e:

    # "\"Eh| no| it's not that.{w} Absolutely not|\""
    "\"А? Нет, всё нормально.{w} Точно нормально."

# game/scenario/sc_en.rpy:243
translate russian s2_604e3b90:

    # "\"Then please refrain from saying such things.{w} If I trouble you, Shiki-sama, then I can't perform any of my duties.\""
    "\"Тогда, пожалуйста, воздержитесь говорить такие вещи.{w} Если я \[беспокою вас\], Шики-сама, то это означает, что я не выполняю своих обязанностей.\""

# game/scenario/sc_en.rpy:244
translate russian s2_ce78d43f:

    # " She looks at me silently."
    " Она тихо посмотрела на меня."

# game/scenario/sc_en.rpy:245
translate russian s2_b74bfc58:

    # " I do want to apologize| but it feels as if I'm being threatened|"
    " Я хочу извиниться, но мне кажется, что в этой фразе была \[угроза\]."

# game/scenario/sc_en.rpy:246
translate russian s2_5e2bd2f8:

    # nvl clear
    # "\"Ah| right|{w} certainly, you're more reliable than an alarm clock.\""
    nvl clear
    "\"Ладно{w} ты действительно надёжнее любого будильника.\""

# game/scenario/sc_en.rpy:250
translate russian s2_49e8a092:

    # "\"Thank you for saying that."
    "\"Спасибо что сказали это."

# game/scenario/sc_en.rpy:251
translate russian s2_0fa3dc99:

    # " Then, Shiki-sama, what will you do today?\""
    " Тогда, Шики-сама, что вы будете делать сегодня?\""

# game/scenario/sc_en.rpy:252
translate russian s2_bf2c82d4:

    # "\"Eh?{w} Today|\""
    "\"А?{w} Сегодня?\""

# game/scenario/sc_en.rpy:253
translate russian s2_acdfed58:

    # " I was about to say \"Of course, I'll|\"{w} Then all my thoughts stopped."
    " Я собирался сказать \"Конечно же я\"{w} Но в этот момент мои мысли остановились."

# game/scenario/sc_en.rpy:254
translate russian s2_7bfe5f46:

    # nvl clear
    # " I don't know what I was going to do today."
    nvl clear
    " Я не знаю что я собирался делать сегодня."

# game/scenario/sc_en.rpy:256
translate russian s2_0941167b:

    # " Did I fall asleep yesterday thinking about what I should do today?"
    " \[Вчера я уснул, размышляя о том, что я хотел делать сегодня\]?"

# game/scenario/sc_en.rpy:257
translate russian s2_d8877362:

    # " I've been thinking about what I did yesterday, and then I finally just accepted the fact that I can't remember."
    " Я начинаю думать о том, что делал вчера, и наконец понимаю, что я не могу вспомнить этого."

# game/scenario/sc_en.rpy:258
translate russian s2_c5f49e55:

    # " It's not like remembering what I did yesterday will change what I'll do today."
    " Похоже что воспоминание о том, что я делал вчера никак не изменит моих планов на сегодня."

# game/scenario/sc_en.rpy:259
translate russian s2_1ae0a50b:

    # nvl clear
    # "\"Shiki-sama, what are your plans for today?\""
    nvl clear
    "\"Шики-сама, какие у вас планы на сегодня?\""

# game/scenario/sc_en.rpy:268
translate russian s2_b698c104:

    # "\"Ah, right.{w} Today|\""
    "\"А, да.{w} Сегодня\""

# game/scenario/sc_en.rpy:269
translate russian s2_691efad9:

    # " Today I'll|{nw}"
    " Сегодня я{nw}"

# game/scenario/sc_en.rpy:270
translate russian s2_0af6a5fa_5:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:285
translate russian s3_a6851a19:

    # "This scene is \"Morning: Unchanging breakfast table.\"{nw}"
    "Эта сцена - \"Утро: Неизменный обеденный стол.\"{nw}"

# game/scenario/sc_en.rpy:286
translate russian s3_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:287
translate russian s3_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:307
translate russian s3_9862f6b7:

    # " When I enter the living room after changing, Akiha is drinking tea as usual."
    " Когда я вошёл в гостиную после того как переоделся, Акиха обыкновенно пила чай."

# game/scenario/sc_en.rpy:311
translate russian s3_0c7fc9cb:

    # " Akiha, who's been looking at the courtyard, now looks up at me."
    " Акиха, смотрящая во двор, теперь посмотрела на меня."

# game/scenario/sc_en.rpy:315
translate russian s3_636b62de:

    # "\"Good morning, Nii-san.{w} I'm glad that you look well this morning.\""
    "\"Доброе утро, Нии-сан.{w} Рада, что ты хорошо выглядишь этим утром.\""

# game/scenario/sc_en.rpy:316
translate russian s3_bbee2d32:

    # " A satisfied smile.{w} What she must really be saying is 'You got up early for once.'"
    " Удовлетворённая улыбка.{w} Она будто бы говорит 'Ты встал рано на этот раз'."

# game/scenario/sc_en.rpy:317
translate russian s3_f792ee6a:

    # nvl clear
    # "\"Yeah.{w} Nihao.\""
    nvl clear
    "\"Да.{w} Нихао.\""

# game/scenario/sc_en.rpy:319
translate russian s3_dc5ae28a:

    # " After a simple greeting I head into the dining room."
    " После незамысловатого приветствия я направился завтракать."

# game/scenario/sc_en.rpy:322
translate russian s3_5a26bbea:

    # "\"I stand corrected.{w} You're just as always today.{w} Why such an insincere greeting?\""
    "\"Поправляюсь.{w} Ты как обычно сегодня.{w} Почему такое неискреннее приветствие?\""

# game/scenario/sc_en.rpy:323
translate russian s3_bbf3326f:

    # " With that she turns her scolding eyes to me."
    " С этими словами она бросила на меня свой упрекающий взгляд."

# game/scenario/sc_en.rpy:324
translate russian s3_b0e040cd:

    # " It's a pity| she doesn't understand the friendly feeling I tried to convey with my simple greeting."
    " Жаль, она не поняла того дружелюбного настроения, которое я хотел передать своим простым приветствием."

# game/scenario/sc_en.rpy:325
translate russian s3_86f98dd6:

    # nvl clear
    # "\"It's not insincere.{w} Saying 'I slept well' all the time gets boring.{w} So I thought maybe I would try something more exotic today.\""
    nvl clear
    "\"Это не неискренне.{w} Постоянно говорить 'Я хорошо спал' надоедает.{w} Поэтому я подумал, может быть я попробую сегодня что-то необычное.\""

# game/scenario/sc_en.rpy:329
translate russian s3_677cd6b2:

    # "\"Hmm?{w} It sounded insincere to me, but if you say so, that's the way it is."
    "\"Хм?{w} Это звучало неискренне для меня, но раз ты так говоришь, так тому и быть."

# game/scenario/sc_en.rpy:330
translate russian s3_5507f21f:

    # " If that's the case, how about I use that greeting tomorrow, too?\""
    " В таком случае, что если я тоже использую это приветствие завтра?\""

# game/scenario/sc_en.rpy:331
translate russian s3_ac3794fe:

    # "\"Ooooh| that would be a welcome---\""
    "\"Оооох, это было бы здорово---\""

# game/scenario/sc_en.rpy:332
translate russian s3_095f3771:

    # " For a moment I think about Akiha greeting me energetically."
    " На минутку я задумался об Акихе, энергично приветствующей меня."

# game/scenario/sc_en.rpy:333
translate russian s3_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:335
translate russian s3_8d2bc8eb:

    # "\"------\"{nw}"
    "\"------\"{nw}"

# game/scenario/sc_en.rpy:336
translate russian s3_66090565:

    # " | For some reason I feel that it'd be bad for my mental health to see such a mismatch in the morning.{w} I might really collapse from anemia."
    " По какой-то причине мне кажется, что такое несоответсвие по утрам плохо скажется на моём метальном здоровье.{w} Я могу свалиться от анемии."

# game/scenario/sc_en.rpy:337
translate russian s3_b2e31320:

    # "\"No| let's drop it.{w} A greeting is a show of etiquette.{w} So, I'll try to be careful next time, too.\""
    "\"Нет, давай забудем об этом.{w} Приветствие — это демонстрация этикета.{w} Так что я буду осторожнее в следующий раз.\""

# game/scenario/sc_en.rpy:340
translate russian s3_7ea31736:

    # "\"It's good that you understand, Nii-san.{w} Please, don't make me say useless things in the morning.\""
    "\"Хорошо что ты понял это, Нии-сан.{w} Пожалуйста, не заставляй меня говорить такие бесполезные вещи по утрам.\""

# game/scenario/sc_en.rpy:341
translate russian s3_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:349
translate russian s3_4b42e6a1:

    # " Akiha elegantly drinks her black tea."
    " Акиха элегантно пъёт свой чёрный чай."

# game/scenario/sc_en.rpy:350
translate russian s3_5f5523ae:

    # " Basically, morning at the Tohno mansion doesn't change whether I wake up early or not.{w} Akiha and I both became used to it."
    " По сути, утро в особняке Тоно не изменится от того, проснусь я рано или нет.{w} Я с Акихой уже привык к этому."

# game/scenario/sc_en.rpy:351
translate russian s3_4ad75ac2:

    # " So it's just another morning."
    " Что ж, это всего лишь ещё одно утро."

# game/scenario/sc_en.rpy:352
translate russian s3_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:359
translate russian s3_025ad55c:

    # " I come to the lobby after I finish my preparations."
    " Я вышел в холл после того как завершил все приготовления."

# game/scenario/sc_en.rpy:360
translate russian s3_3895fecb:

    # " It's only seven o'clock.{w} I could probably drop by somewhere and still have time to make it to school."
    " Сейчас всего семь часов.{w} Я мог бы, наверное, зайти куда нибудь и у меня всё ещё останется время на школу."

# game/scenario/sc_en.rpy:361
translate russian s3_4a520362:

    # " Well, then|{nw}"
    " Ну ладно, тогда{nw}"

# game/scenario/sc_en.rpy:362
translate russian s3_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:375
translate russian s4_bdffb8b3:

    # "This scene is \"School: Arcueid's apartment.\"{nw}"
    "Эта сцена - \"Школа: Квартира Арквейд.\"{nw}"

# game/scenario/sc_en.rpy:376
translate russian s4_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:377
translate russian s4_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:395
translate russian s4_df4a5108:

    # " Since I have time, I should at least show my face."
    " Раз у меня есть немного времени, я должен хотя бы зайти."

# game/scenario/sc_en.rpy:396
translate russian s4_ad89dcbf:

    # " I didn't have time because of the school festival, and I just want to see Arcueid."
    " У меня нет слишком много времени из-за школьного фестиваля, но я хочу повидаться с Арквейд."

# game/scenario/sc_en.rpy:397
translate russian s4_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:404
translate russian s4_c30d4fcb:

    # " I go out before Akiha."
    " Я вышел раньше Акихи."

# game/scenario/sc_en.rpy:408
translate russian s4_939f9be4:

    # "\"Shiki-sama, did you forget anything?\""
    "\"Шики-сама, вы ничего не забыли?\""

# game/scenario/sc_en.rpy:409
translate russian s4_23dd00ab:

    # " Hisui surprises me with that question."
    " Хисуи ошарашила меня таким вопросом."

# game/scenario/sc_en.rpy:410
translate russian s4_ca6b47ff:

    # "\"Eh?{w} No, I thought I got everything.\""
    "\"Э?{w} Нет, я думаю что взял всё что нужно.\""

# game/scenario/sc_en.rpy:411
translate russian s4_531260f4:

    # " I open my bag to check the contents."
    " Я открыл свою сумку чтобы проверить содержимое."

# game/scenario/sc_en.rpy:412
translate russian s4_ea5260bc:

    # " Writing utensils, student ID, notes for today, and then the hidden knife."
    " Письменные принадлежности, студенческий билет, тетради на сегодня и припрятанный нож."

# game/scenario/sc_en.rpy:413
translate russian s4_e8d0bfa8:

    # nvl clear
    # "\"I don't think I forgot anything.{w} I'll be back later, then.{w} I might come back late, so don't worry too much.\""
    nvl clear
    "\"Вроде ничего не забыл.{w} Тогда до встречи.{w} Я могу вернуться поздно, так что особо не беспокойся.\""

# game/scenario/sc_en.rpy:418
translate russian s4_8714563b:

    # "\"Yes, it must be the cultural festival preparations.{w} If you decide to sleep over anywhere, please give me a call.\""
    "\"Да, наверное это из-за подготовки к культурному фестивалю.{w} Если вы решите переночевать где-нибудь, пожалуйста, отзвонитесь мне.\""

# game/scenario/sc_en.rpy:419
translate russian s4_66d57fba:

    # "\"Okay, then I'll see you later!\""
    "\"Хорошо, тогда увидимся позже!\""

# game/scenario/sc_en.rpy:423
translate russian s4_f18ef2ae:

    # " I turn away from the bowing Hisui and trod downhill."
    " Я отвернулся от кланяющейся Хисуи и побрёл вниз по склону."

# game/scenario/sc_en.rpy:424
translate russian s4_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:430
translate russian s4_8c06e2cd:

    # "\"Huu|\""
    "\"Хуух\""

# game/scenario/sc_en.rpy:431
translate russian s4_5301cf43:

    # " I break off from my running to take a deep breath."
    " Я прекратил бежать, чтобы сделать глубокий вдох."

# game/scenario/sc_en.rpy:432
translate russian s4_895d7edf:

    # " If I go south from the intersection, it'll be the school, east of the mansion.{w} If I don't hurry, I'll be late.{w} If I don't want to be late, I have to wake up and run all the way here."
    " Если я пойду на юг от перекрёстка, то попаду в школу, которая находится восточнее особняка.{w} \[Если я не потороплюсь, то опоздаю\].{w} Если я не хочу опаздывать, я встаю раньше и бегу по этому пути."

# game/scenario/sc_en.rpy:433
translate russian s4_176d27da:

    # " This is the normal routine when I want to go to Arcueid's place in the morning."
    " Это \[обыкновенная рутина\] когда я собираюсь зайти к Арквейд с утра."

# game/scenario/sc_en.rpy:434
translate russian s4_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:438
translate russian s4_43e776f8:

    # " \"Ring!\" The doorbell sounds."
    " \"Ррриинь!\" Звуки дверного звонка."

# game/scenario/sc_en.rpy:439
translate russian s4_172541d9:

    # " No answer."
    " Нет ответа."

# game/scenario/sc_en.rpy:440
translate russian s4_b01c389c:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:441
translate russian s4_f4ba9739:

    # " I open the door with my key and enter."
    " Я открываю дверь своим ключом и вхожу."

# game/scenario/sc_en.rpy:445
translate russian s4_1cdd04a3:

    # "\"Arcueid, you awake?\""
    "\"Арквейд, ты проснулась?\""

# game/scenario/sc_en.rpy:448
translate russian s4_04e639d6:

    # "\"Mmm| just now|\""
    "\"Ммм, только что\""

# game/scenario/sc_en.rpy:450
translate russian s4_8e68d96b:

    # " She rubs her sleepy eyes and motions toward me."
    " Она трёт свои сонные глаза и движется в мою сторону."

# game/scenario/sc_en.rpy:451
translate russian s4_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:456
translate russian s4_f9794e8e:

    # " Arcueid's room is as simple as always."
    " Комната Арквейд как всегда проста."

# game/scenario/sc_en.rpy:459
translate russian s4_d9140711:

    # " Maybe she went out last night, but she looks more tired than usual as she sits up on her bed."
    " Может быть она \[выходила\] прошлой ночью, она выглядит уставшей сильнее, чем обычно когда садится на кровати."

# game/scenario/sc_en.rpy:460
translate russian s4_feff3dfe:

    # nvl clear
    # "\"What is it?{w} You look like you're in pain.{w} Did something happen?\""
    nvl clear
    "\"Что такое?{w} Ты выглядишь так, будто тебе больно.{w} Что-то случилось?\""

# game/scenario/sc_en.rpy:462
translate russian s4_fa9b7241:

    # "\"Yeah| I dreamt a pretty boring dream last night.{w} It feels like I'll get swept away by myself.\""
    "\"Да, мне снился довольно \[утомительный\] сон ночью.{w} \[Такое чувство будто меня уносит прочь\].\""

# game/scenario/sc_en.rpy:463
translate russian s4_7cf9945e:

    # " Haa, she sighs.{w} It looks more like she's mentally tired, rather than physically tired."
    " Она вздохнула.{w} Больше походит на ментальную усталость, чем на физическую."

# game/scenario/sc_en.rpy:464
translate russian s4_f381af83:

    # "\"Boring dream|?{w} I thought you didn't dream often, Arcueid.\""
    "\"\[Утомительный сон\]?{w} Я думал ты не часто видишь сны, Арквейд.\""

# game/scenario/sc_en.rpy:465
translate russian s4_f8db5fcf:

    # "\"It's not often.{w} I never dream.{w} I stop my bodily functions when I sleep, so even thinking stops."
    "\"Не часто.{w} Мне никогда не снятся сны.{w} Я останавливаю процесссы в своём теле, когда сплю, поэтому мыслительная деятельность останавливается."

# game/scenario/sc_en.rpy:466
translate russian s4_a96c8d4a:

    # " Well, even then I sometimes remember things from the past, but that's just the past.{w} It's different from what you call dreaming, isn't it?\""
    " Однако, я иногда вспоминаю что-то из прошлого, но это всего лишь прошлое.{w} Это отличается от того, что вы назвываете снами, не так ли?\""

# game/scenario/sc_en.rpy:467
translate russian s4_ccbe56ff:

    # nvl clear
    # "\"Yeah| a dream is half reality, half wish.{w} It's different from remembering the past.\""
    nvl clear
    "\"Да, сны это наполовину реальность, наполовину \[желания\].{w} Это не просто воспоминания из прошлого.\""

# game/scenario/sc_en.rpy:469
translate russian s4_fb519966:

    # "\"Right.{w} I don't know what 'half wish' means, but you experience things you haven't, right?{w} Then I've never dreamt.\""
    "\"Верно.{w} Я не знаю что означает 'наполовину \[желания\]', но ты \[испытываешь\] вещи которых нет, верно?{w} Тогда я никогда не видела сны.\""

# game/scenario/sc_en.rpy:473
translate russian s4_d888fc91:

    # " Half asleep, she speaks as if she were lonely."
    " Полусонная, она говорила так, будто ей было одиноко."

# game/scenario/sc_en.rpy:474
translate russian s4_12540876:

    # nvl clear
    # "\"Hmm| then your 'dream' was just a remembrance of the past?\""
    nvl clear
    "\"Хм, тогда твой 'сон' это просто воспоминание из прошлого?\""

# game/scenario/sc_en.rpy:479
translate russian s4_0622c77a:

    # "\"Mmm| I haven't experienced anything like that, so I don't know.{w} Since it contained something I didn't know about, I guess you could call it a 'dream.'"
    "\"Ммм, я никогда не испытывала подобного, так что я не знаю.{w} Поскольку я видела то, о чём я не знала, я думаю это можно назвать 'сном'."

# game/scenario/sc_en.rpy:487
translate russian s4_0b5dc220:

    # " It was horrible.{w} I thought maybe I had dreamt at last, but I saw a future that I hate the most.{w} You didn't come, either.{w} I feel like I got cheated out of something.\""
    " Это ужасно.{w} Я подумала, что мне приснился сон, но я видела будущее, которое не хотела бы больше всего.{w} \[И в любом случае ты не пришёл\].{w} Я чувствовала себя в чём-то обманутой.\""

# game/scenario/sc_en.rpy:488
translate russian s4_016278ca:

    # " Haa, she sighs again."
    " Хаа, она снова вздохнула."

# game/scenario/sc_en.rpy:489
translate russian s4_61722a51:

    # " Her morning today doesn't seem much better than mine."
    " Её сегодняшнее утро, похоже, не лучше, чем моё."

# game/scenario/sc_en.rpy:490
translate russian s4_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:494
translate russian s4_db52c5c5:

    # "\"But, Shiki!{w} Since you're here, are you taking a break from school?\""
    "\"Но, Шики!{w} Раз ты здесь, то ты взял перерыв от школы?"

# game/scenario/sc_en.rpy:495
translate russian s4_07d812a5:

    # " With that, she gets up energetically.{w} Her eyes are telling me to take her out somewhere fun."
    " С этими словами, она энергично встала.{w} Её глаза говорили мне сходить с ней куда-нибудь повеселиться."

# game/scenario/sc_en.rpy:496
translate russian s4_8415b3d8:

    # "\"Ah| no, I just thought maybe I'd see you for a while.{w} After I hear what I came here for, I'm going to school.\""
    "\"А, нет, я просто решил заглянуть к тебе ненадолго.{w} После того как я \[услышу всё что хочу\], я пойду в школу.\""

# game/scenario/sc_en.rpy:499
translate russian s4_060d78e6:

    # "\"Ah| Shiki, you're so cold in that aspect.{w} You didn't come help me yesterday when I was in a pinch.\""
    "\"Ах, Шики, ты так \[холоден в этом плане\].{w} Ты не пришёл на помощь вчера, когда у меня были неприятности.\""

# game/scenario/sc_en.rpy:500
translate russian s4_fa9f875e:

    # " An unsatisfied look."
    " Недовольный взгляд."

# game/scenario/sc_en.rpy:501
translate russian s4_dea46636:

    # " Her dream is troubling, but at least getting trusted feels good."
    " Её сон тревожит, но, по крайней мере, \[когда тебе доверяют приятно\]."

# game/scenario/sc_en.rpy:502
translate russian s4_107044e4:

    # nvl clear
    # "\"Okay, okay.{w} If you're in a pinch next time, call.{w} But, unfortunately, I can't pop up in someone else's dream.{w} If it's a dream, you're on your own.\""
    nvl clear
    "\"Хорошо, хорошо.{w} Если ты в следующий раз попадёшь в неприятности, зови.{w} Но, к сожалению, я не могу попасть в чужой сон.{w} Если ты во сне, то ты сама по себе.\""

# game/scenario/sc_en.rpy:504
translate russian s4_5ad5ed7e:

    # "\"Eh?{w} Solve it on my own?{w} I can't do that.\""
    "\"Э?{w} Мне нужно разрешить всё самой?{w} Я не могу сделать это.\""

# game/scenario/sc_en.rpy:505
translate russian s4_90516e70:

    # " She tilts her head obliviously."
    " Она рассеяно наклоняет голову."

# game/scenario/sc_en.rpy:506
translate russian s4_0c63187b:

    # nvl clear
    # "\"Arcueid| a while ago, who was the person who messed around with my dreams?\""
    nvl clear
    "\"Арквейд, не ты ли некоторое время назад вмешивалась в мои сны?\""

# game/scenario/sc_en.rpy:511
translate russian s4_88263dc8:

    # "\"Ah| hehehe| that was me.\""
    "\"А, хехехе, это я.\""

# game/scenario/sc_en.rpy:512
translate russian s4_67fb4d70:

    # " Seriously| how can she forget something that would cause her trouble so quickly|"
    " Серьёзно, как она могла забыть что-то, что могло доставить ей хлопот, настолько быстро?"

# game/scenario/sc_en.rpy:513
translate russian s4_8a528a6f:

    # "\"I don't know about demon familiars, but nightmares aren't scary.{w} Ah, well, that is| you can't use something like Len to alter people's dreams.\""
    "\"Я не знаю о демонах-фамильярах, но кошмары - это не страшно.{w} А, кстати, разве ты не можешь использовать что-то вроде Лен, чтобы изменять сны людей.\""

# game/scenario/sc_en.rpy:514
translate russian s4_e75a16a7:

    # " Arcueid quickly consents.{w} It's good that she understands quickly."
    " \[Арквейд быстро соглашается\].{w} Хорошо, что она быстро поняла."

# game/scenario/sc_en.rpy:515
translate russian s4_c5248774:

    # nvl clear
    # "\"Yeah, I had Len.{w} I didn't use it too much, so I forgot.{w} I guess I can ask Len to do it for me, then!\""
    nvl clear
    "\"Точно, у меня есть Лен.{w} Я давно не пользовалась ей, поэтому забыла о ней.{w} Тогда я могу попросить её сделать это для меня!\""

# game/scenario/sc_en.rpy:517
translate russian s4_6f7efa2a:

    # "\"Hey!\""
    "\"Эй!\""

# game/scenario/sc_en.rpy:518
translate russian s4_164da422:

    # " You can't just abuse it like that|"
    " Ты не можешь злоупотреблять этим!"

# game/scenario/sc_en.rpy:519
translate russian s4_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:525
translate russian s4_17bf4d03:

    # "------If I don't leave for school right now, I'll be late."
    "------Если я сейчас не уйду в школу, я опоздаю"

# game/scenario/sc_en.rpy:526
translate russian s4_b28707a3:

    # " Even though the conversation drew on unexpectedly, I got what I came for, so I should leave for school now."
    " Несмотря на то, что разговор неожиданно затянулся, я получил то зачем пришёл, поэтому я должен идти в школу."

# game/scenario/sc_en.rpy:527
translate russian s4_fceda8cd:

    # nvl clear
    # "\"Arcueid, I have to ask something.\""
    nvl clear
    "\"Арквейд, я должен кое-что спросить.\""

# game/scenario/sc_en.rpy:529
translate russian s4_89cdb76f:

    # "\"Hmm?{w} What is it?{w} You suddenly became so serious.\""
    "\"Хм?{w} Что такое?{w} Ты внезапно стал таким серьёзным.\""

# game/scenario/sc_en.rpy:530
translate russian s4_ebe8f7ee:

    # "\"That's|{w} did you do something to me?\""
    "\"Насчёт этого,{w} ты что-то сделала со мной?\""

# game/scenario/sc_en.rpy:533
translate russian s4_8a4b8ed0:

    # "\"Eh?\""
    "\"Э?\""

# game/scenario/sc_en.rpy:534
translate russian s4_9324537d:

    # " Eh?{w} Arcueid tilts her head like a little pigeon."
    " Э?{w} Арквейд наклонила голову словно маленький голубь."

# game/scenario/sc_en.rpy:535
translate russian s4_198f5b46:

    # "\"What do you mean by 'do something?'{w} What did I do?\""
    "\"Что ты имеешь в виду под 'что-то сделала'?{w} Я что-то сделала?"

# game/scenario/sc_en.rpy:536
translate russian s4_85531965:

    # "\"No, I don't know myself.{w} I don't know what it is, but it just feels weird."
    "\"Нет, я и сам не знаю.{w} Я не знаю что это, но просто это кажется \[странным\]."

# game/scenario/sc_en.rpy:537
translate russian s4_3acbf169:

    # " And only you or Senpai would do something weird, so I came to ask about it.\""
    " И только ты или Семпай могут сделать что-то \[странное\], поэтому я и пришёл спросить об этом.\""

# game/scenario/sc_en.rpy:538
translate russian s4_76b2fe88_6:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:541
translate russian s4_fa5965c8:

    # "\"I can't let that slide by, but to answer first, it's a no.{w} I don't remember doing anything to you recently.\""
    "\"Я не могу оставить это без внимания, но сначала отвечу - нет.{w} Я не припомню чтобы делала с тобой что-то в последнее время.\""

# game/scenario/sc_en.rpy:542
translate russian s4_8779c4d2:

    # "\"Yeah| I don't remember your doing anything to me, either.\""
    "\"\[Да, я тоже не припомню, чтобы ты что-то делала со мной\].\""

# game/scenario/sc_en.rpy:543
translate russian s4_5ca56755:

    # " But I can't even tell what I did yesterday, so my memory isn't helping."
    " Но я даже не могу сказать что было вчера, так что моя память здесь не поможет."

# game/scenario/sc_en.rpy:544
translate russian s4_4c444e40:

    # " Eh| it actually feels like I asked this sort of question to someone yesterday, too|"
    " Э, на самом деле мне кажется я спрашивал подобное и вчера."

# game/scenario/sc_en.rpy:545
translate russian s4_0935f145:

    # nvl clear
    # "\"Ah, sorry, I guess it was just a mistake.{w} Well, I'll be leaving for school now.\""
    nvl clear
    "\"А, извини, думаю я просто ошибся.{w} Ладно, мне пора идти в школу.\""

# game/scenario/sc_en.rpy:549
translate russian s4_185f8c45:

    # "\"Eh|{w} Shiki, are you really going?{w} Since you came, I want you to stay a bit longer.\""
    "\"Э?{w} Шики, ты и правда уходишь?{w} Раз уж ты пришел, я хочу, чтобы ты остался еще немного.\""

# game/scenario/sc_en.rpy:550
translate russian s4_e29098dd:

    # "\"Uh|\""
    "\"Ух\""

# game/scenario/sc_en.rpy:551
translate russian s4_19ae0a3c:

    # " That pitiable asking face| I'm a healthy male| my determination is swaying."
    " Это жалостливо вопрошающее лицо. Я \[здоровый\] мужчина, моя решимость пошатнулась."

# game/scenario/sc_en.rpy:552
translate russian s4_76b2fe88_7:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:562
translate russian s4_6dd88e03:

    # " | What do I do|"
    " Что мне делать?"

# game/scenario/sc_en.rpy:563
translate russian s4_901ca095:

    # " In this case, I'll just--{nw}"
    " В таком случае, я просто--{nw}"

# game/scenario/sc_en.rpy:564
translate russian s4_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:577
translate russian s5_b03b4d06:

    # "This scene is \"School: Ciel-senpai's apartment.\"{nw}"
    "Эта сцена - \"Школа: Квартира Сиель-семпай.\"{nw}"

# game/scenario/sc_en.rpy:578
translate russian s5_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:579
translate russian s5_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:597
translate russian s5_f6ab9bc5:

    # " | Yeah, even though it'll be a detour, I guess I can go to school with her."
    " Хм, хотя это и будет \[обходной путь\], думаю я смогу пойти в школу с ней."

# game/scenario/sc_en.rpy:598
translate russian s5_16e62e31:

    # " If Akiha or Kohaku catch wind of it, I'll be dead, but be that as it may."
    " Если Акиха или Кохаку узнают об этом - я труп, но будь что будет."

# game/scenario/sc_en.rpy:599
translate russian s5_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:606
translate russian s5_b96525a3:

    # " I head out before Akiha."
    " Я вышел раньше Акихи."

# game/scenario/sc_en.rpy:610
translate russian s5_939f9be4:

    # "\"Shiki-sama, did you forget anything?\""
    "\"Шики-сама, вы ничего не забыли?\""

# game/scenario/sc_en.rpy:611
translate russian s5_44853f66:

    # " Hisui suddenly asks."
    " Внезапно спросила Хисуи."

# game/scenario/sc_en.rpy:612
translate russian s5_b2e76f83:

    # "\"Eh?{w} No.{w} I thought my preparations were complete.\""
    "\"Э?{w} Нет.{w} Я всё подготовил.\""

# game/scenario/sc_en.rpy:613
translate russian s5_17a0b88c:

    # " I open my bag to check its contents."
    " Я открываю сумку, чтобы проверить её содержимое."

# game/scenario/sc_en.rpy:614
translate russian s5_ba1c1272:

    # " Writing utensils, student ID, today's notes, and the hidden knife|{w} Everything seems to be normal."
    " Письменные принадлежности, студенческий билет, сегодняшние тетради и спрятанный нож{w} Кажется, всё в порядке."

# game/scenario/sc_en.rpy:615
translate russian s5_28dcc746:

    # nvl clear
    # "\"I don't think I forgot anything.{w} Then I'll see you later.{w} I might come back late, so don't worry too much.\""
    nvl clear
    "\"Не думаю, что что-нибудь забыл.{w} Тогда увидимся позже.{w} Я могу вернуться поздно, так что не волнуйся слишком сильно.\""

# game/scenario/sc_en.rpy:620
translate russian s5_8714563b:

    # "\"Yes, it must be the cultural festival preparations.{w} If you decide to sleep over anywhere, please give me a call.\""
    "\"Да, должно быть это из-за подготовки к культурному фестивалю.{w} Если вы решите переночевать где-нибудь ещё, отзвонитесь мне.\""

# game/scenario/sc_en.rpy:621
translate russian s5_66d57fba:

    # "\"Okay, then I'll see you later!\""
    "\"Хорошо, увидимся позже!\""

# game/scenario/sc_en.rpy:625
translate russian s5_f18ef2ae:

    # " I turn away from the bowing Hisui and trod downhill."
    " Я отвернулся от кланяющейся Хисуи и \[побрёл\] вниз по склону."

# game/scenario/sc_en.rpy:626
translate russian s5_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:632
translate russian s5_52a78f6c:

    # " I arrive at Senpai's apartment with time to spare."
    " Я пришёл к квартире Семпай \[с запасом времени\]."

# game/scenario/sc_en.rpy:633
translate russian s5_e487e04e:

    # "\"Maybe Ciel-senpai is having breakfast.\""
    "\"Возможно Сиель-семпай завтракает.\""

# game/scenario/sc_en.rpy:638
translate russian s5_1627cb3b:

    # " I knock on the door."
    " Я постучал в дверь."

# game/scenario/sc_en.rpy:639
translate russian s5_64f2a9d1:

    # "\"Hmm?{w} Who is it?\""
    "\"Хм?{w} Кто там?\""

# game/scenario/sc_en.rpy:640
translate russian s5_c0a1a366:

    # " Her courteous yet energetic voice greets me."
    " Ее вежливый, но энергичный голос приветствует меня."

# game/scenario/sc_en.rpy:641
translate russian s5_7f3fcc0b:

    # "\"It's Tohno, can I come in for a while?\""
    "\"Это Тоно, я могу зайти ненадолго?\""

# game/scenario/sc_en.rpy:642
translate russian s5_d4c16c64:

    # "\"Of course.{w} I can't stop what I'm doing now, so please help yourself in.\""
    "\"Конечно.{w} Я не могу оставить, то что делаю сейчас, так что \[войди самостоятельно\].\""

# game/scenario/sc_en.rpy:643
translate russian s5_ded2a86f:

    # " Assuming she's cooking, I go into her kitchen."
    " Предполагая, что она готовит, я иду на кухню."

# game/scenario/sc_en.rpy:644
translate russian s5_ad6bb1a5:

    # "\"Excuse me.\""
    "\"\[Простите\].\""

# game/scenario/sc_en.rpy:645
translate russian s5_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:648
translate russian s5_2ac0d182:

    # " Senpai is preparing breakfast in the kitchen."
    " Семпай готовит завтрак на кухне."

# game/scenario/sc_en.rpy:649
translate russian s5_03fdfa64:

    # " I feel like I went to sleep late last night and woke up late today."
    " Я чувствую будто поздно лёг спать вчера и поздно проснулся сегодня."

# game/scenario/sc_en.rpy:650
translate russian s5_cb2b552e:

    # nvl clear
    # " Senpai opens the door and comes in."
    nvl clear
    " Семпай открывает дверь и заходит."

# game/scenario/sc_en.rpy:652
translate russian s5_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:654
translate russian s5_a0953c46:

    # "\"Morning, Tohno-kun.{w} I thought we would both be late today, but you're pretty quick.\""
    "\"Утречка, Тоно-кун.{w} Я думала, \[мы оба сегодня опоздаем\], но ты довольно шустрый.\""

# game/scenario/sc_en.rpy:655
translate russian s5_3cb825c4:

    # "\"Eh?{w} Ah, yeah.{w} I woke up a bit early, so I have some spare time today.\""
    "\"Э?{w} А, да.{w} Я пронулся немного раньше, так что сегодня у меня есть \[запас врремени\].\""

# game/scenario/sc_en.rpy:656
translate russian s5_03a8b2d0:

    # "\"Is that so?{w} That's amazing.{w} I'm really tired so I thought maybe I would skip school.\""
    "\"Даже так?{w} Это прекрасно.{w} Я очень устала, поэтому подумала, что, может быть, пропущу школу.\""

# game/scenario/sc_en.rpy:657
translate russian s5_5b58fb46:

    # " She covers her mouth with her small hands and yawns in a cute way."
    " Она прикрывает рот своими маленькими руками и мило зевает."

# game/scenario/sc_en.rpy:658
translate russian s5_57804c79:

    # nvl clear
    # " Senpai is still doing her nocturnal patrols."
    nvl clear
    " Семпай всё ещё проводит ночные патрули."

# game/scenario/sc_en.rpy:660
translate russian s5_5bc8b8a4:

    # " Cleansing the streets once infected by vampires supposedly takes three months to a year, and it seems like Senpai is the only one doing it in this city."
    " \[Зачистка улиц\], однажды зараженных вампирами, предположительно занимает от трех месяцев до года, и кажется, что Семпай - единственная, кто занимается этим в этом городе."

# game/scenario/sc_en.rpy:661
translate russian s5_a6cc1fc6:

    # nvl clear
    # "\"Senpai, if you're tired, why don't you rest a bit?{w} You're doing something important.{w} I think you have an excuse to miss school.\""
    nvl clear
    "\"Семпай, если ты устала почему бы тебе немного не отдохуть?{w} Ты делаешь нечто важное.{w} Мне кажется у тебя есть оправдание, чтобы прогулять школу.\""

# game/scenario/sc_en.rpy:663
translate russian s5_10b90134:

    # "\"I was thinking that, but I decided to try a little harder.{w} What with the cultural festival preparations and all.{w} And seeing your face really cheered me up.\""
    "\"Я думала над этим, но я решила \[стараться усерднее\].{w} Что насчёт культурного фестиваля и остального.{w} К тому же увидев твоё лицо, я взбодрилась.\""

# game/scenario/sc_en.rpy:664
translate russian s5_fe3cc62d:

    # " She smiles a smile that would even beat down the morning sunshine."
    " На её лице такая улыбка, что затмила бы даже утреннее солнце."

# game/scenario/sc_en.rpy:665
translate russian s5_8627228d:

    # " | It's a true blessing I can be here to see it."
    " Это блаженство, что я здесь и вижу это."

# game/scenario/sc_en.rpy:666
translate russian s5_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:669
translate russian s5_8889de28:

    # "\"Well, if my face cheers you up, please look at it all you want.{w} I'll come whenever you call.\""
    "\"Ладно, если моё лицо бодрит тебя, пожалуйста смотри на него когда захочешь.{w} \[Я приду как позовёшь\].\""

# game/scenario/sc_en.rpy:670
translate russian s5_ce1312b5:

    # " I embarrassedly scratch my cheek to hide my face a little."
    " Я смущённо чешу щёку, чтобы немного скрыть своё лицо."

# game/scenario/sc_en.rpy:671
translate russian s5_c070e7d4:

    # nvl clear
    # "\"Ah, Senpai, you're helping the student government, too, right?{w} I heard that you're the coordinator.\""
    nvl clear
    "\"А, Семпай, ты же ещё помогаешь студенческому совету, верно?{w} Я слышал, что ты - координатор.\""

# game/scenario/sc_en.rpy:675
translate russian s5_075ce4f3:

    # "\"Yes| I didn't want to become a member, but I can't attend their events if I'm not, so I was forced to join."
    "\"Да, я не очень хочу быть членом совета, но я не смогу посещать заседания если я не в нём, так что мне пришлось вступить."

# game/scenario/sc_en.rpy:676
translate russian s5_2aa9577e:

    # "\"Oh?{w} I didn't know that.\""
    "\"О?{w} Я не знал об этом.\""

# game/scenario/sc_en.rpy:677
translate russian s5_f932593e:

    # nvl clear
    # "\"Yeah| it would be pretty bad to let someone who comes and goes like the wind participate in club activities.{w} One must have more than just the desire to be a part."
    nvl clear
    "\"Да, будет плохо если позволять кому-то, кто \[приходит и тут же уходит\], участвовать в клубных мероприятиях.{w} У человека должно быть нечто большее, чем просто желание быть частью совета."

# game/scenario/sc_en.rpy:679
translate russian s5_87ac7ac4:

    # " Oh, and the president is Maki-senpai, right?{w} It seemed like he relies on you a lot.\""
    " О, и президент Маки-семпай, верно?{w} Похоже, он очень полагается на тебя.\""

# game/scenario/sc_en.rpy:682
translate russian s5_1115f871:

    # " Eh?{w} Senpai suddenly takes on a depressed expression."
    " Э?{w} Лицо Сэмпай внезапно становится подавленным."

# game/scenario/sc_en.rpy:683
translate russian s5_2f00e9a9:

    # nvl clear
    # "\"Senpai| are you not getting along well with Maki-senpai?\""
    nvl clear
    "\"Семпай, ты плохо ладишь с Маки-семпай?\""

# game/scenario/sc_en.rpy:687
translate russian s5_a4c0b7b9:

    # "\"Eh?{w} No, I don't have any problems with the president.{w} But| he asked me out a few times|\""
    "\"Э?{w} Нет, у меня нет проблем с президентом.{w} Но он несколько раз приглашал меня на свидание.\""

# game/scenario/sc_en.rpy:688
translate russian s5_1b2163df:

    # " | Eh?"
    " Э?"

# game/scenario/sc_en.rpy:689
translate russian s5_4a584744:

    # "\"Ask?{w} Who asked whom?\""
    "\"Приглашал?{w} Кто кого приглашал?\""

# game/scenario/sc_en.rpy:690
translate russian s5_ff862dc2:

    # "\"Haa| the president asks me.\""
    "\"Аах, президент приглашает меня.\""

# game/scenario/sc_en.rpy:691
translate russian s5_34dfaa7a:

    # "\"Wha, what?!\""
    "\"Что?!\""

# game/scenario/sc_en.rpy:692
translate russian s5_f5089c78:

    # " I stand up suddenly and with force."
    " Я вскакиваю."

# game/scenario/sc_en.rpy:693
translate russian s5_4ab19a12:

    # nvl clear
    # "\"Se, Se, Senpai! What did you say!?\""
    nvl clear
    "\"Се-, Се-, Семпай! Что ты сказала!?\""

# game/scenario/sc_en.rpy:697
translate russian s5_cb3913bc:

    # "\"Well, I said what was expected.{w} I can't go out with anyone so casually given that I don't have time for it with the street patrols.\""
    "\"Я сказала то, что сказала.{w} Я не могу встречаться с кем-либо так непринужденно, учитывая, что у меня нет на это времени из-за уличных патрулей.\""

# game/scenario/sc_en.rpy:698
translate russian s5_b01c389c:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:699
translate russian s5_8e915427:

    # " Haa| I let out a sigh of relief."
    "Ахх, я вздохул с облегчением."

# game/scenario/sc_en.rpy:700
translate russian s5_5f96cb3b:

    # " Hmm| she denied."
    "Хмм, она отказала."

# game/scenario/sc_en.rpy:701
translate russian s5_2e427b1e:

    # " That's a relief| but why did she say it in the present tense and not in the past tense?"
    " Это облегчение, но почему она сказала это в настоящем времени, а не в прошедшем?"

# game/scenario/sc_en.rpy:702
translate russian s5_8218b729:

    # nvl clear
    # "\"Eh, Senpai.\""
    nvl clear
    "\"Э, Семпай.\""

# game/scenario/sc_en.rpy:704
translate russian s5_ef018020:

    # "\"Hmm?{w} I refuse everytime, but he doesn't seem to care.{w} Asking me out has replaced our greeting.\""
    "\"Хм?{w} Я каждый раз отказываюсь, но ему, похоже, все равно.{w} Приглашение на свидание заменило нам приветствие.\""

# game/scenario/sc_en.rpy:705
translate russian s5_8439bb9a:

    # "\"Every time?{w} You're not thinking of consenting, are you?{w} Maki-senpai is being stubborn!\""
    "\"Каждый раз?{w} Ты же не думаешь согласиться?{w} Маки-семпай настойчивый!\""

# game/scenario/sc_en.rpy:706
translate russian s5_b335ccc8:

    # nvl clear
    # "\"That may be so, but| I might fall for his perseverance.{w} Since I'm human, too, I'll probably like 'the one who likes me without a doubt,' rather than 'the one whom I like but who doesn't return my feelings.'\""
    nvl clear
    "\"Может быть и так, но я могла бы поддаться ему.{w} Я тоже человек, мне, вероятно, понравится 'тот, кому я нравлюсь без сомнения', а не 'тот, кто мне нравится, но не отвечает на мои чувства взаимностью.'\""

# game/scenario/sc_en.rpy:708
translate russian s5_92652779:

    # "\"------!\""
    "\"------!\""

# game/scenario/sc_en.rpy:709
translate russian s5_cd1135ba:

    # " She says it as if it were nothing."
    " Она говорит это так, словно это ничего не значит."

# game/scenario/sc_en.rpy:710
translate russian s5_a68b7c33:

    # nvl clear
    # "\"Senpai|{w} that's|{w} eh|\""
    nvl clear
    "\"Семпай,{w} это,{w} эх\""

# game/scenario/sc_en.rpy:712
translate russian s5_2eb93ff3:

    # "\"Hmm|{w} What is it, Tohno-kun?\""
    "\"Хмм?{w} В чём дело, Тоно-кун?\""

# game/scenario/sc_en.rpy:713
translate russian s5_912d5c5e:

    # "\"Eh| let's go out somewhere.{w} I'll take you where you want to go.\""
    "\"Давай куда-нибудь сходим.{w} Я отведу тебя куда захочешь.\""

# game/scenario/sc_en.rpy:714
translate russian s5_e3ac0d4c:

    # " Ah| my face is burning up."
    " Моё лицо горит."

# game/scenario/sc_en.rpy:715
translate russian s5_a1b960d0:

    # " Senpai looks at me with a smile."
    " Семпай смотрит на меня с улыбкой."

# game/scenario/sc_en.rpy:716
translate russian s5_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:719
translate russian s5_81f58413:

    # "\"Gladly.{w} But I'm sorry I kinda rushed you.\""
    "\"С удовольствием.{w} Но мне жаль, что поторопила тебя.\""

# game/scenario/sc_en.rpy:720
translate russian s5_0784fad1:

    # "\"| Prankster|\""
    "\"Проказница!\""

# game/scenario/sc_en.rpy:721
translate russian s5_4e4a60c9:

    # " I murmur quietly.{w} Senpai keeps on smiling as if she hasn't heard me."
    " Тихо пробормотал я.{w} Семпай продолжает улыбаться, как будто не слышала меня."

# game/scenario/sc_en.rpy:722
translate russian s5_24d8f4ae:

    # " Ah| I guess I'll have to hide away from Akiha and get a part-time job to finance what I just said|"
    " А, думаю, мне придется скрыться от Акихи и найти подработку, чтобы профинансировать то, что я только что сказал."

# game/scenario/sc_en.rpy:723
translate russian s5_76b2fe88_6:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:729
translate russian s5_1ef63d41:

    # "------Ah!{w} Time passed away so quickly while we chatted it up."
    "------А!{w} Время пролетело так быстро, пока мы болтали об этом."

# game/scenario/sc_en.rpy:730
translate russian s5_b761ac6c:

    # "\"Senpai, if we don't leave now, we'll be late for school.\""
    "\"Семпай, если мы не выйдем сейчас, мы опоздаем в школу.\""

# game/scenario/sc_en.rpy:731
translate russian s5_007b3de5:

    # " I shout as I pick up my bag."
    " Кричу я, хватая свою сумку."

# game/scenario/sc_en.rpy:734
translate russian s5_3e13683c:

    # "\"Mmm| that won't do.{w} I still haven't had my breakfast.\""
    "\"Ммм, так не пойдёт.{w} Я всё ещё не завтракала.\""

# game/scenario/sc_en.rpy:735
translate russian s5_b01c389c_1:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:736
translate russian s5_c50594c0:

    # " That is true."
    " И правда."

# game/scenario/sc_en.rpy:737
translate russian s5_edbdff5b:

    # " No| I must snap out of it| I don't have time for it."
    "Нет, я должен прийти в себя, у меня нет на это времени."

# game/scenario/sc_en.rpy:738
translate russian s5_fa703f1b:

    # nvl clear
    # "\"Sorry, Senpai, for bothering you.{w} But| are you really going to have breakfast?\""
    nvl clear
    "\"Извини, Семпай, что беспокою.{w} Но ты и правда собираешься завтракать?\""

# game/scenario/sc_en.rpy:742
translate russian s5_c15c1aa1:

    # "\"Yes.{w} Most definitely.{w} I'll collapse if I skip breakfast.\""
    "\"Да.{w} Абсолютно точно.{w} Я упаду в обморок, если пропущу завтрак.\""

# game/scenario/sc_en.rpy:743
translate russian s5_e9a2841b:

    # " She says that with such a serious expression."
    " Она говорит это с таким серьезным выражением лица."

# game/scenario/sc_en.rpy:746
translate russian s5_e959fa58:

    # "\"Ah| we haven't had breakfast in a while.{w} Why don't you eat with me?{w} I made a little too much.\""
    "\"А, мы давненько не завтракали.{w} Почему бы тебе не поесть со мной?{w} Я приготовила немного больше.\""

# game/scenario/sc_en.rpy:747
translate russian s5_3ba04c93:

    # "\"Senpai| are you saying I should be late?\""
    "\"Семпай, ты предлагаешь мне опоздать?\""

# game/scenario/sc_en.rpy:750
translate russian s5_137ee9bd:

    # "\"Sure.{w} Let's both be late.\""
    "\"Именно.{w} Давай оба опоздаем.\""

# game/scenario/sc_en.rpy:751
translate russian s5_76b2fe88_7:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:754
translate russian s5_a36ffde7:

    # " Uh| it should be a crime to use such cutesy eyes to sway a man's mind."
    " Ух, использовать такие милые глазки, чтобы повлиять на разум мужчины должно быть преступлением."

# game/scenario/sc_en.rpy:755
translate russian s5_f6926ead:

    # " Darn it| can I be seduced just like this!?{nw}"
    " Чёрт возьми, можно ли меня вот так просто соблазнить!?{nw}"

# game/scenario/sc_en.rpy:763
translate russian s5_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:775
translate russian s8_dee6b036:

    # "This scene is \"School - Morning: Arcueid's apartment.\"{nw}"
    "Эта сцена - \"Школа - Утро: квартира Арквейд.\"{nw}"

# game/scenario/sc_en.rpy:776
translate russian s8_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:777
translate russian s8_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:799
translate russian s8_1aef04fd:

    # "\"Say| Shiki, stay a little more.{w} People say that you don't have to go to school every day.{w} And plus, you did wake me up, too!\""
    "\"Шики, останься ещё немного.{w} Люди говорят, что тебе не обязательно ходить в школу каждый день.{w} И плюс ещё ты разбудил меня!\""

# game/scenario/sc_en.rpy:800
translate russian s8_30a410b0:

    # "\"Uuuuh|\""
    "\"Уууух|\""

# game/scenario/sc_en.rpy:808
translate russian s8_fccabcf1:

    # " I did wake her up.{w} I guess it's a bit rude to just say goodbye now and go to school."
    " Я разбудил её.{w} Думаю будет немного грубо просто сказать сейчас 'пока' и пойти в школу."

# game/scenario/sc_en.rpy:809
translate russian s8_aa1e1d8d:

    # " Well| today Arcueid doesn't seem to be feeling too well, and being one hour late won't be too bad."
    " Что ж, кажется сегодня Арквейд чувствует себя не очень хорошо, да и опоздать на час не так уж страшно."

# game/scenario/sc_en.rpy:810
translate russian s8_d2800121:

    # nvl clear
    # "\"Okay| I guess I could stay a bit longer.\""
    nvl clear
    "\"Хорошо, я думаю, что смогу остаться на подольше.\""

# game/scenario/sc_en.rpy:814
translate russian s8_a5cd16d7:

    # "\"You sure should!{w} Ah!{w} It feels like I just woke up from a fresh nap!\""
    "\"\[Конечно ты должен\]!{w} Ах!{w} Такое чувство, будто я проснулась от \[освежающей дремоты\]!\""

# game/scenario/sc_en.rpy:815
translate russian s8_661445b1:

    # " Ehehehe.{w} She makes a victory sign with baby-like happiness."
    " \[Ехехехе\].{w} Она сделала победный жест с детским восторгом."

# game/scenario/sc_en.rpy:816
translate russian s8_e5d2d488:

    # "\"It's good that you feel awake| but, didn't you just say something disturbing, Arcueid?\""
    "\"Хорошо, что ты наконец проснулась, но, мне кажется или ты сказала нечто тревожащее, Акрвейд?\""

# game/scenario/sc_en.rpy:820
translate russian s8_a4b900dd:

    # "\"Eh?{w} Something disturbing?{w} What?\""
    "\"Э?{w} Что-то тревожащее?{w} Что именно?\""

# game/scenario/sc_en.rpy:821
translate russian s8_f5f61860:

    # "\"You said people said you don't have to go to school everyday.{w} I don't remember saying anything like that.\""
    "\"Ты сказала люди говорят, что я не должен ходить в школу каждый день.{w} Я не помню чтобы говорил что-то такое.\""

# game/scenario/sc_en.rpy:822
translate russian s8_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:825
translate russian s8_4ad08ee0:

    # "\"Of course, it wasn't you.{w} It wasn't you who said you only have to go to school only about two thirds of the time.{w} No, it wasn't you.\""
    "\"Конечно, это был не ты.{w} Это не ты сказал, что тебе нужно ходить в школу только примерно две трети времени.{w} Нет, это не ты.\""

# game/scenario/sc_en.rpy:826
translate russian s8_e11f1337:

    # "\"Okay, well, that's all fine.{w} But| who did you hear it from?\""
    "\"Хорошо, тогда, всё в порядке.{w} Но от кого ты услышала это?\""

# game/scenario/sc_en.rpy:827
translate russian s8_dea63819:

    # " It wouldn't be Senpai who said it.{w} Akiha wouldn't forgive even a day of absence."
    " Этого не могла сказать Семпай.{w} Акиха не простит даже дня отсутствия."

# game/scenario/sc_en.rpy:828
translate russian s8_1aadc079:

    # " Then|{w} it probably isn't|{w} but|"
    " Тогда,{w} точно не они,{w} но"

# game/scenario/sc_en.rpy:829
translate russian s8_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:832
translate russian s8_90751bbc:

    # "\"Don't worry.{w} I don't go to your school randomly nor do I talk to your friends randomly.\""
    "\"Не волнуйся.{w} Я не ходила в твою школу и не разговаривала с твоими друзьями без разбора.\""

# game/scenario/sc_en.rpy:833
translate russian s8_1b216ccb:

    # " As if she's reading my mind, she says what I was going to ask."
    " Как будто прочитав мои мысли, она сказала то, о чём я хотел спросить."

# game/scenario/sc_en.rpy:834
translate russian s8_7e199f5f:

    # " Worst case scenario|{w} Arcueid got herself acquainted with someone at school who has the worst behavior and is very prone to spread rumors about me and her.{w} But I think that has been avoided."
    " Худший сценарий - {w} Арквейд познакомилась в школе с кем-то, кто ведет себя хуже всех и очень склонен распространять слухи обо мне и о ней.{w} Но я думаю, что этого удалось избежать."

# game/scenario/sc_en.rpy:835
translate russian s8_7968a1c2:

    # nvl clear
    # "\"Ah| I'm glad.{w} I was wondering how it would have turned out if you had met Arihiko or something.{w} But, then| where did you hear that, Arcueid?\""
    nvl clear
    "\"А, я рад.{w} Интересно, как бы все обернулось, если бы ты встретила Арихико или кого-то такого.{w} Но всё же, где ты услышала это, Арквейд?\""

# game/scenario/sc_en.rpy:839
translate russian s8_5eb9dd99:

    # "\"Oh, it's simple.{w} I was just walking around during the day and met some guy in your school's uniform.{w} But it was during the day, and so school was in.{w} I thought it weird that he'd be out and about.{w} So I just asked him why he wasn't in school.\""
    "\"О, всё просто.{w} Я просто гуляла по округе и втретила парня в униформе твоей школы.{w} Но это было днём, поэтому он должен быть в школе.{w} Я посчитала странным что он не там.{w} Поэтому я просто спросила почему он не в школе.\""

# game/scenario/sc_en.rpy:840
translate russian s8_b01c389c:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:841
translate russian s8_7b7e3152:

    # " For a moment, an image of something really really bad flashed through my mind."
    " На секунду картина чего-то очень очень плохого мелькнула в моёй голове."

# game/scenario/sc_en.rpy:842
translate russian s8_a96f7d62:

    # nvl clear
    # "\"That guy, he seemed to be around your age, so I thought it'd be okay to talk to him.{w} Then he taught me that you can skip a third of school.{w} And then he also said, 'Who would leave such a pretty Nee-san like you and go to school?{w} That guy can't even be called a guy!'{w} Or something like that.\""
    nvl clear
    "\"Тот парень был примерно твоего возраста, поэтому я подумала, что неплохо бы поговорить с ним.{w} Тогда он и сказал мне, что ты можешь пропустить треть школы.{w} Он ещё сказал, 'Кто бы бросил такую хорошенькую Нии-сан, как ты, и пошел в школу?{w} Этот парень не может называться парнем!'{w} Или как-то так.\""

# game/scenario/sc_en.rpy:844
translate russian s8_2774a6a3:

    # " Wow| to be able to say such a thing on his first meeting, and then to be able to walk around proudly during the day in school uniform|"
    " Вау, иметь возможность сказать такое при первой встрече, а потом гордо расхаживать в течение дня в школьной форме."

# game/scenario/sc_en.rpy:845
translate russian s8_ccce7d6b:

    # nvl clear
    # "\"Oh|{w} and did he say anything else?\""
    nvl clear
    "\"О,{w} он говорил что-нибудь ещё?\""

# game/scenario/sc_en.rpy:849
translate russian s8_1f14193a:

    # "\"Yeah.{w} He told me to say hi to you.{w} And then he shouted something like 'You traitor!' and left.\""
    "\"Да.{w} Он просил передать привет тебе.{w} Он крикнул что-то вроде 'Ты предатель!' и ушёл.\""

# game/scenario/sc_en.rpy:850
translate russian s8_b01c389c_1:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:851
translate russian s8_d04c6fbd:

    # " Ah| another reason for a headache|"
    " Ах, ещё одна причина головной боли."

# game/scenario/sc_en.rpy:852
translate russian s8_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:858
translate russian s8_66a0f8b4:

    # " Tick tock.{w} The hand of the clock points to 9 o'clock."
    " Тик так.{w} Стрелка часов показывает девять."

# game/scenario/sc_en.rpy:859
translate russian s8_9fe7a5d4:

    # " Homeroom's done.{w} First period should be in session.{w} I should just give up half of the morning periods, then."
    " Домашние задания проверены.{w} Первый урок должен быть в самом разгаре.{w} Тогда стоит просто отказаться от половины утренних занятий."

# game/scenario/sc_en.rpy:860
translate russian s8_4134c2a3:

    # nvl clear
    # "\"Ah!{w} That's right!\""
    nvl clear
    "\"А!{w} Точно!\""

# game/scenario/sc_en.rpy:863
translate russian s8_52f8163d:

    # " Arcueid suddenly clings on to my neck."
    " Акрвейд внезапно цепляется за мою шею."

# game/scenario/sc_en.rpy:864
translate russian s8_5cb2b7dc:

    # "\"Ack!{w} That's dangerous!{w} You don't do that when someone's drinking something!\""
    "\"Акх!{w} Это опасно!{w} Ты не должна делать так, когда кто-то что-то пьёт!\""

# game/scenario/sc_en.rpy:868
translate russian s8_12f777a1:

    # "\"Hmph!{w} It's your fault for relaxing by yourself and ditching me!\""
    "\"Хмпф!{w} Это твоя вина, что ты расслабляешься в одиночестве, \[бросив\] меня!\""

# game/scenario/sc_en.rpy:869
translate russian s8_0fac5be9:

    # "\"Relaxing by myself?{w} I was just drinking tea!{w} Are you going to go ballistic over such trivial things?\""
    "\"Расслабляюсь в одиночестве?{w} Я просто пью чай!{w} Ты выходишь из себя из-за таких тривиальный вещей?\""

# game/scenario/sc_en.rpy:870
translate russian s8_f558a92b:

    # nvl clear
    # "\"You don't take care of me, so I have to.{w} It's not nice of you to drink tea by yourself!\""
    nvl clear
    "\"Ты не заботишься обо мне, так что да.{w} Не хорошо с твой стороны пить чай в одиночестве!\""

# game/scenario/sc_en.rpy:879
translate russian s8_a6491b21:

    # " She looks at me as if her feelings were hurt."
    " Она смотрит на меня так, словно ее чувства задеты"

# game/scenario/sc_en.rpy:880
translate russian s8_e1f89db7:

    # " Well| this isn't all bad."
    " Что ж, всё не так плохо."

# game/scenario/sc_en.rpy:881
translate russian s8_aae6a275:

    # nvl clear
    # "\"Ah| really|{w} you're not satisfied whatever I do.{w} Okay, I'll do what you want.{w} Just tell me what you want me to do.\""
    nvl clear
    "\"\[А, и правда,{w} ты недовольна чем я занят\].{w} Хорошо, я сделаю что ты хочешь.{w} Просто скажи мне что ты хочешь, чтобы я сделал.\""

# game/scenario/sc_en.rpy:885
translate russian s8_9c66468c:

    # "\"Really?{w} Then I want you to make me breakfast!\""
    "\"Серьёзно?{w} Тогда я хочу, чтобы ты сделал мне завтрак!\""

# game/scenario/sc_en.rpy:886
translate russian s8_c11f546d:

    # " She lets go of my neck and bounces like a little child."
    " Она отпустила мою шею и подпрыгнула как маленький ребёнок."

# game/scenario/sc_en.rpy:887
translate russian s8_4a016e86:

    # "\"Eh| breakfast?\""
    "\"Э, завтрак?\""

# game/scenario/sc_en.rpy:888
translate russian s8_60d53907:

    # "\"Yeah!{w} The thing you made me last time!{w} I want to eat it again!\""
    "\"Да!{w} То, что ты готовил мне в прошлый раз!{w} \[Я снова хочу съесть это\]!\""

# game/scenario/sc_en.rpy:889
translate russian s8_55ff2a14:

    # " She looks so happy she might just explode."
    " Она выглядит такой счастливой, что готова просто взорваться."

# game/scenario/sc_en.rpy:890
translate russian s8_befcbb15:

    # nvl clear
    # " That's fine| but| is that really okay?"
    nvl clear
    " \[Это прекрасно, но действительно ли это нормально\]?"

# game/scenario/sc_en.rpy:892
translate russian s8_6aaf5a7e:

    # " I do remember making breakfast for her once, but I didn't think she would like it that much."
    " Я помню делал ей завтрак однажды, но я не думал, что ей так сильно это понравится."

# game/scenario/sc_en.rpy:900
translate russian s8_b01c389c_2:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:901
translate russian s8_353cb172:

    # " No reply comes to mind as I stand up."
    " Когда я встаю, в голову \[не приходит ответа\]."

# game/scenario/sc_en.rpy:902
translate russian s8_9bc0f468:

    # "\"Okay.{w} Then I'll go borrow your kitchen.\""
    "\"Окей.{w} Тогда я займу твою кухню.\""

# game/scenario/sc_en.rpy:903
translate russian s8_3fe7d3ca:

    # " Saying that, I head over to the kitchen."
    " Сказав это, я вхожу на кухню."

# game/scenario/sc_en.rpy:904
translate russian s8_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:908
translate russian s8_15a8a52b:

    # "\"Here you go!\""
    "\"Вот, пожалуйста!\""

# game/scenario/sc_en.rpy:909
translate russian s8_5111f10d:

    # " I put the bowl down on the table."
    " Я поставил чашу на стол."

# game/scenario/sc_en.rpy:910
translate russian s8_82c7e776:

    # "\"Wow!{w} I'll surely enjoy it!\""
    "\"Вау!{w} Мне наверняка это понравится!\""

# game/scenario/sc_en.rpy:911
translate russian s8_f25133f5:

    # " She brings her hand together to show gratitude and then picks up the chopsticks."
    " Она сложила ладони вместе в знак благодарности, а затем взяла палочки для еды."

# game/scenario/sc_en.rpy:912
translate russian s8_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:914
translate russian s8_4e12acde:

    # "\"Mmm!{w} Tasty!\""
    "\"Ммм!{w} Вкусно!\""

# game/scenario/sc_en.rpy:915
translate russian s8_b3d3fdfd:

    # " She moves her chopsticks lightly."
    " Она легко двигает своими палочками."

# game/scenario/sc_en.rpy:916
translate russian s8_2376c7e8:

    # " Well, if she's happy, I'm happy."
    " Чтож, если она счастлива, то и я счастлив."

# game/scenario/sc_en.rpy:917
translate russian s8_605591cc:

    # "\"I'm glad you like it.{w} I tried my best.{w} It was only a pre-cooked meal last time, but I took my time today.\""
    "\"Я рад, что тебе понравилось.{w} Я старался.{w} В прошлый раз была уже готовая еда, а сегодня я не торопился.\""

# game/scenario/sc_en.rpy:918
translate russian s8_b3c3ad9d:

    # "\"Really?{w} I think it's much better than last time! ♪\""
    "\"Серьёзно?{w} Это намного лучше, чем в прошлый раз! ♪\""

# game/scenario/sc_en.rpy:919
translate russian s8_71b365f1:

    # " She brings the noodles to her mouth."
    " Она подносит лапшу ко рту."

# game/scenario/sc_en.rpy:920
translate russian s8_2f9ae925:

    # nvl clear
    # "\"The base is the same, but I tried to make the taste soak in more.{w} I wasn't really satisfied last time.\""
    nvl clear
    "\"Основа та же самая, но я постарался, чтобы вкус сильнее впитался.{w} \[В прошлый раз я не был доволен этим\].\""

# game/scenario/sc_en.rpy:922
translate russian s8_df2dc75e:

    # "\"Nah, I just thought that I could eat more last time.{w} While thinking that you made it, I was already done with half of it.{w} So I could only enjoy the second half.\""
    "\"\[Нет, я просто подумала, что в прошлый раз могла бы съесть больше.\]{w} Пока думала что это приготовил ты, уже съела половину.{w} Поэтому я наслаждалась только второй половиной.\""

# game/scenario/sc_en.rpy:923
translate russian s8_4ebe6d28:

    # " Slurp, slurp."
    " Хлюп, хлюп."

# game/scenario/sc_en.rpy:924
translate russian s8_90d557c6:

    # " Saying that, she doesn't slow her chopsticks a single bit."
    " Говоря это, она ни на йоту не \[замедляет движение палочек\]."

# game/scenario/sc_en.rpy:925
translate russian s8_49bdf03d:

    # " Hmm| it looks like she really likes the dish."
    " Хмм, похоже ей и правда нравится это блюдо."

# game/scenario/sc_en.rpy:926
translate russian s8_a2bb52ba:

    # nvl clear
    # "\"Ah| this isn't good|{w} seeing you eat it so heartily, it makes me want to make you breakfast every day|{w} This isn't good.\""
    nvl clear
    "\"А, \[это плохо\],{w} видя как ты ешь это с таким аппетитом, мне хочется готовить тебе завтрак каждый день.{w} \[Это не хорошо\].\""

# game/scenario/sc_en.rpy:928
translate russian s8_1a119974:

    # "\"Don't worry.{w} Just come make me breakfast everyday.{w} I can wake up everyday if you make me breakfast--\""
    "\"Не волнуйся.{w} Просто приходи и готовь мне завтрак каждый день.{w} Я могу просыпаться каждый день, если ты будешь готовить мне завтрак--\""

# game/scenario/sc_en.rpy:929
translate russian s8_9588a2da:

    # " Looks like she's done with the noodles.{w} Arcueid now moves on to the soup by picking up the bowl and bringing it to her mouth."
    " Похоже, она покончила с лапшой.{w} Арквейд переходит к супу, берет миску и подносит её ко рту."

# game/scenario/sc_en.rpy:930
translate russian s8_5b320c7e:

    # " How do I say this| it's a sight to behold."
    " Как бы это сказать? Это зрелище, которое стоит видеть."

# game/scenario/sc_en.rpy:931
translate russian s8_fa5ac7bb:

    # nvl clear
    # "\"Mmm?{w} What is it, Shiki?{w} You seem surprised.\""
    nvl clear
    "\"Ммм?{w} Что такое Шики?{w} Ты выглядишь удивлённым.\""

# game/scenario/sc_en.rpy:933
translate russian s8_a9163e3d:

    # "\"Well, yeah, I'm surprised.{w} I thought that a princess like you would eat more elegantly.\""
    "\"Что ж, да, я удивлён.{w} Я думал, что такая принцесса как ты будет есть более элегантно.\""

# game/scenario/sc_en.rpy:934
translate russian s8_b01c389c_3:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:935
translate russian s8_b88ccc9b:

    # " Ah| Arcueid's cheeks redden.{w} This isn't like her."
    "Щёки Арквейд краснеют.{w} Это на неё не похоже."

# game/scenario/sc_en.rpy:936
translate russian s8_9ff21066:

    # nvl clear
    # "\"But| you made it for me.{w} So I wanted to eat it like that.\""
    nvl clear
    "\"Но, ты приготовил это для меня.{w} Так что мне захотелось съесть это именно так.\""

# game/scenario/sc_en.rpy:938
translate russian s8_7c04bf3d:

    # " She puts down her bowl."
    " Она опускает свою чашу."

# game/scenario/sc_en.rpy:939
translate russian s8_fc549e85:

    # " I ask again, but she seems embarrassed and looks away."
    " \[Я спрашиваю ещё раз\], но она кажется слишком смущённой и отводит взгляд."

# game/scenario/sc_en.rpy:940
translate russian s8_c92596ad:

    # "\"Well, this side has its cuteness, too|\""
    "\"Что ж, с этой стороны тоже есть своя прелесть.\""

# game/scenario/sc_en.rpy:941
translate russian s8_d72198a6:

    # " I murmur low enough so that she can't hear me."
    "Бормочу я достаточно тихо, чтобы она не услышала меня."

# game/scenario/sc_en.rpy:942
translate russian s8_22ef5faa:

    # " If you think about it, Arcueid has a lot of mismatched aspects about her."
    " Если подумать, у Арквейд есть много противоречащих аспектов."

# game/scenario/sc_en.rpy:943
translate russian s8_a01aedf0:

    # " She eats fast food, drinks canned drinks from vending machines|"
    " Она питается фастфудом, пьёт газировку из автоматов."

# game/scenario/sc_en.rpy:944
translate russian s8_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:946
translate russian s8_b4f2c652:

    # "\"Eh| eh?\""
    "\"Э, эх?\""

# game/scenario/sc_en.rpy:947
translate russian s8_ad8d3ba4:

    # " With that Arcueid starts acting strangely."
    " С этими звуками Арквейд начинает странно себя вести."

# game/scenario/sc_en.rpy:948
translate russian s8_e046a2e5:

    # " Like she's in pain|?{w} Maybe she wants to cry?"
    " Будто ей больно?{w} Может она хочет плакать?"

# game/scenario/sc_en.rpy:949
translate russian s8_89ee3fc9:

    # "\"Shi, Shi, Shiki|{w} I gotta ask|\""
    "\"Ши, ши, Шики,{w} я должна спросить.\""

# game/scenario/sc_en.rpy:950
translate russian s8_fbc569d9:

    # " She lets out in a tearful voice."
    " Выдавливает она с плачущим голосом."

# game/scenario/sc_en.rpy:951
translate russian s8_30713a94:

    # "\"Wh, what is it, Arcueid?{w} Is it painful to be awake so early in the morning?\""
    "\"Ч, что такое Арквейд?{w} Тебе больно просыпаться так рано по утрам?\""

# game/scenario/sc_en.rpy:952
translate russian s8_7eef5d14:

    # "\"Ah, no| nothing like that| but| what did you put in there?\""
    "\"А, нет, ничего подобного, но, что ты добавил сюда?\""

# game/scenario/sc_en.rpy:953
translate russian s8_67e2b0c0:

    # " She tilts her head like a little bird|{w} Somehow it matches her tearful expression well."
    " Она наклоняет голову, как маленькая птичка {w} Почему-то это хорошо сочетается с ее заплаканным выражением лица."

# game/scenario/sc_en.rpy:954
translate russian s8_8f1fa814:

    # nvl clear
    # "\"What ingredients?{w} I used only those necessary.\""
    nvl clear
    "\"Какие ингридиенты?{w} Я использовал только самое необходимое.\""

# game/scenario/sc_en.rpy:956
translate russian s8_77078103:

    # " I list them as they come up in my head."
    " Я перечисляю их по мере того, как они всплывают у меня в голове."

# game/scenario/sc_en.rpy:957
translate russian s8_a7eb8787:

    # " Arcueid puts down her pale face, but when she hears the last ingredient, she lies flat on the floor."
    " Арквейд опускает свое бледное лицо, но когда она слышит последний ингредиент, она ложится плашмя на пол."

# game/scenario/sc_en.rpy:958
translate russian s8_76b2fe88_6:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:961
translate russian s8_4b342c89:

    # "\"Arcueid!{w} What is it, Arcueid?!\""
    "\"Арквейд!{w} Что случилось, Арквейд?!\""

# game/scenario/sc_en.rpy:962
translate russian s8_581b2ac1:

    # "\"Kuuu|\""
    "\"Кххх\""

# game/scenario/sc_en.rpy:963
translate russian s8_ec55ef02:

    # " She goes limp."
    " Она обмякает."

# game/scenario/sc_en.rpy:964
translate russian s8_bdb0e40d:

    # " With her eyes closed, she murmurs as if she were in a nightmare."
    " С закрытыми глазами она бормочет что-то, как будто ей снится кошмар."

# game/scenario/sc_en.rpy:965
translate russian s8_2b45ff7c:

    # "\"|?\""
    "\"?\""

# game/scenario/sc_en.rpy:966
translate russian s8_2b027b10:

    # " What is it|?{w} I put my ears closer to her."
    " Что за?{w} Я приблизил уши к ней."

# game/scenario/sc_en.rpy:967
translate russian s8_c2bf470a:

    # "------And then|"
    "------А затем"

# game/scenario/sc_en.rpy:968
translate russian s8_76b2fe88_7:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:970
translate russian s8_bf4077af:

    # "\"Don't put that in|{w} No garlic|{w} please|{w} not that|{w} it's not tasty|\""
    "\"Не клади это{w} Никагого чеснока,{w} пожалуйста,{w} не его,{w} он не вкусный."

# game/scenario/sc_en.rpy:971
translate russian s8_cad6d77c:

    # " She repeats over and over again."
    " Она повторяла это снова и снова."

# game/scenario/sc_en.rpy:972
translate russian s8_76b2fe88_8:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:980
translate russian s8_8d8ab670:

    # "\"Then, I'm going to go.\""
    "\"Тогда я пойду.\""

# game/scenario/sc_en.rpy:981
translate russian s8_e5643055:

    # " I talk to Arcueid. who's lying on the bed."
    " Говорю я Арквейд, лежащей на кровати."

# game/scenario/sc_en.rpy:982
translate russian s8_b0de56c9:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:983
translate russian s8_6e9fc3a3:

    # " Arcueid doesn't answer and stares at me.{w} She glares at me with eyes like a cat's."
    " Арквейд не отвечает и пристально смотрит на меня.{w} Она смотрит на меня глазами, как у кошки."

# game/scenario/sc_en.rpy:984
translate russian s8_1048238b:

    # " A cat| that's it."
    " Кошка, вот и всё."

# game/scenario/sc_en.rpy:985
translate russian s8_cfe855a0:

    # " I can't tell whether she's interested, hostile, has feelings or not|"
    " Я не могу сказать, заинтересована она, враждебна, \[чувствует хоть что-то\] или нет."

# game/scenario/sc_en.rpy:986
translate russian s8_723fb66c:

    # nvl clear
    # "\"Sorry| I'll be more careful next time.\""
    nvl clear
    "\"Извини, я буду осторожнее в следующий раз.\""

# game/scenario/sc_en.rpy:988
translate russian s8_b0de56c9_1:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:989
translate russian s8_77ba5933:

    # " Arcueid stays in her cat-like state."
    " Арквейд осталась в своём состоянии кошки."

# game/scenario/sc_en.rpy:990
translate russian s8_9de54d9f:

    # " Uuuuuh|{w} The pressure from her eyes feels like I have hundreds of blades in my back.{w} I hurriedly turn my back to Arcueid and the room."
    " Ууууух{w} Давление ее глаз ощущается так, словно мне в спину вонзили сотни лезвий.{w} Я поспешно поворачиваюсь спиной к Арквейд и комнате."

# game/scenario/sc_en.rpy:991
translate russian s8_e27749a9:

    # " Or| I don't know what Arcueid will do to me, so I'll have to escape."
    " Или, я не знаю, что Арквейд сделает со мной, так что мне придется сбежать."

# game/scenario/sc_en.rpy:992
translate russian s8_76b2fe88_9:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:997
translate russian s8_1c92dcdd:

    # " Then, I'll head to school."
    " Тогда, я пойду в школу."

# game/scenario/sc_en.rpy:998
translate russian s8_fef69cd0:

    # " It's past ten now."
    " Уже за десять."

# game/scenario/sc_en.rpy:999
translate russian s8_a23e0aac:

    # " If I hurry I should be barely able to make it to third period."
    " Если я потороплюсь, то едва успею на третий урок."

# game/scenario/sc_en.rpy:1000
translate russian s8_76b2fe88_10:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1017
translate russian s9_0cddc2c0:

    # "This scene is \"School - Morning: Ciel-senpai's apartment - This is the famous curry jazz band!\"{nw}"
    "Эта сцена - \"Школа - Утро: квартира Сиель-семпай - \[Это знаменитый карри джаз-бэнд\]!\"{nw}"

# game/scenario/sc_en.rpy:1018
translate russian s9_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:1019
translate russian s9_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:1039
translate russian s9_c5f03a30:

    # "------Hmm, what am I stalling about?"
    "------Хмм, почему я тяну время?"

# game/scenario/sc_en.rpy:1040
translate russian s9_10874616:

    # " Senpai said that she'll make me breakfast.{w} If I refuse, I'm not even a man."
    " Семпай сказала, что она сделает мне завтрак.{w} Если я откажусь, я не мужик."

# game/scenario/sc_en.rpy:1041
translate russian s9_8e08e5e5:

    # " The one thing that worries me is whether I'll be able to finish it after eating Kohaku-san's meal back home."
    " Единственное, что меня беспокоит, - смогу ли я съесть его после того, как уже съел дома стряпню Кохаку-сан."

# game/scenario/sc_en.rpy:1045
translate russian s9_4438d1b6:

    # "\"Tohno-kun?{w} What are you suddenly thinking about?{w} Did I do something wrong?\""
    "\"Тоно-кун?{w} О чём ты внезапно задумался?{w} Я что-то делаю не так?\""

# game/scenario/sc_en.rpy:1046
translate russian s9_1fdd29e5:

    # "\"Ah, no, not at all!{w} I just decided.{w} I'll gladly eat the food you make me.\""
    "\"А, нет, ничего такого!{w} Я решил.{w} Я с радостью съем то, что ты мне приготовишь.\""

# game/scenario/sc_en.rpy:1047
translate russian s9_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1050
translate russian s9_eacee40b:

    # "\"I'm glad!{w} It was good that I made two persons' worth.{w} Then I'll make my preparations.{w} Please sit and rest."
    "\"Я рада!{w} \[Хорошо, что я принесу пользу двум людям\].{w} Тогда я всё подготовлю.{w} Пожалуйста, сядь и отдохни.\""

# game/scenario/sc_en.rpy:1058
translate russian s9_4a9e38a3:

    # " Senpai heads into the kitchen with an apron in one hand."
    " Семпай направилась на кухню с фартуком в одной руке."

# game/scenario/sc_en.rpy:1059
translate russian s9_b0de56c9:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:1060
translate russian s9_87fff8fc:

    # " Then, what should I do?"
    " Тогда, что делать мне?"

# game/scenario/sc_en.rpy:1061
translate russian s9_444ab0da:

    # " I could just sit and relax, but I really don't have anything to do right now."
    " Я могу просто сесть и расслабится, но \[сейчас мне и правда нечего делать\]."

# game/scenario/sc_en.rpy:1062
translate russian s9_c6eb0668:

    # " Although I'm not hungry, I'm fine without any more food.{w} Having recently had breakfast, the stomach doesn't require more of the brain.{w} If I eat Senpai's 'rough' menu, then it'll be suicidal."
    " Хотя я не голоден, я могу обойтись и без всякой еды.{w} После недавнего завтрака, \[желудок не просит больше, чем мозг\].{w} Если я съем 'грубое' меню Семпай, это будет самоубийством."

# game/scenario/sc_en.rpy:1063
translate russian s9_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1066
translate russian s9_66ca3725:

    # "\"------All right.\""
    "\"------Ладно.\""

# game/scenario/sc_en.rpy:1067
translate russian s9_f2fe27ca:

    # " I'll try to get my food down with pushups."
    " Я попробую проглотить еду после отжиманий."

# game/scenario/sc_en.rpy:1068
translate russian s9_93806888:

    # " Anyway, I'll have to work off some energy so that I can take in the food later."
    " \[В любом случае, мне нужно потратить немного энергии, которую я позже могу получить из еды.\]"

# game/scenario/sc_en.rpy:1069
translate russian s9_20cb1996:

    # "{w=0.5} 10, {w=0.5} 15, {w=0.5} 20, {w=0.5} 25, {w=0.5} 30, {w=0.5} 35||{w=0.5}! Huuu| it's simple."
    "{w=0.5} 10, {w=0.5} 15, {w=0.5} 20, {w=0.5} 25, {w=0.5} 30, {w=0.5} 35{w=0.5}! Хууу, это просто."

# game/scenario/sc_en.rpy:1070
translate russian s9_6af68a7e:

    # nvl clear
    # "\"Tohno-kun, please come here for a moment.\""
    nvl clear
    "\"Тоно-кун, пожалуйста подойди на минутку.\""

# game/scenario/sc_en.rpy:1072
translate russian s9_978cfa06:

    # " Senpai calls me from the kitchen."
    " Семпай звала меня с кухни."

# game/scenario/sc_en.rpy:1073
translate russian s9_1fcce7ae:

    # "\"Okay, I'm coming.\""
    "\"Хорошо, я иду.\""

# game/scenario/sc_en.rpy:1074
translate russian s9_e5a2be5d:

    # " Done with the pushups."
    " Закончим с отжиманиями."

# game/scenario/sc_en.rpy:1075
translate russian s9_827bdbfe:

    # " I wipe off my sweat and head over to the kitchen."
    " Я пытираю пот и вхожу на кухню."

# game/scenario/sc_en.rpy:1076
translate russian s9_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1078
translate russian s9_23cbaa25:

    # "\"What is it, Senpai?\""
    "\"Что такое, Семпай?\""

# game/scenario/sc_en.rpy:1079
translate russian s9_40313f89:

    # "\"Well, I just didn't know how much you would eat for breakfast.{w} You don't seem to not eat anything in particular, but you also don't eat too much, do you?\""
    "\"Ну, я просто не знаю как мого ты ешь на завтрак.{w} Не похоже, что ты воротишь нос от чего-то необычного, но ты не ешь слишком много, не так ли?\""

# game/scenario/sc_en.rpy:1080
translate russian s9_e959a26f:

    # "\"Ah, not really|{w} I don't really eat that little, either|\""
    "\"А, не совсем,{w} на самом деле я не так уж и мало ем.\""

# game/scenario/sc_en.rpy:1081
translate russian s9_f39fc307:

    # " Her consideration is welcome right now."
    " Сейчас она соображает в верном направлении."

# game/scenario/sc_en.rpy:1082
translate russian s9_ba477fda:

    # " I don't want to waste food that Senpai's prepared, so I'll just get a small amount."
    " Я не хочу переводить еду, которую приготовила Семпай, так что я просто \[возьму немного\]."

# game/scenario/sc_en.rpy:1083
translate russian s9_ebbd24de:

    # nvl clear
    # "\"I don't think I can eat too much for breakfast.{w} Please give me half a person's serving.\""
    nvl clear
    "\"Не думаю, что смогу съесть слишком много на завтрак.{w} Пожалуйста дай мне только половину порции.\""

# game/scenario/sc_en.rpy:1085
translate russian s9_88936310:

    # "\"Okay, then I'll do that.\""
    "\"Ладно, тогда так и сделаю.\""

# game/scenario/sc_en.rpy:1086
translate russian s9_78dce1b2:

    # " She smiles while nodding and churns the food on the stove."
    " Кивая, она улыбается и перемешивает еду на плите."

# game/scenario/sc_en.rpy:1087
translate russian s9_b01c389c:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:1088
translate russian s9_d51a2e19:

    # nvl clear
    # " I catch myself unconsciously going limp after seeing Senpai in an apron cooking breakfast."
    nvl clear
    " Я ловлю себя на том, что бессознательно обмякаю, увидев Сэмпай в фартуке, готовящую завтрак."

# game/scenario/sc_en.rpy:1090
translate russian s9_f262be6d:

    # " This is a great blessing and happiness."
    " Это \[великое благословение\] и счастье."

# game/scenario/sc_en.rpy:1091
translate russian s9_b445995d:

    # " A person who prepares breakfast with a smiling face."
    " Человек, который готовит завтрак с улыбающимся лицом."

# game/scenario/sc_en.rpy:1092
translate russian s9_f5898829:

    # " The table that emanates the smell of breakfast."
    " Стол, от которого исходит запах завтрака."

# game/scenario/sc_en.rpy:1093
translate russian s9_40c70ed7:

    # " It's too bad that I'm not hungry right now, but this spicy aroma|"
    " Плохо, что я сейчас не голоден, но этот пряный аромат..."

# game/scenario/sc_en.rpy:1094
translate russian s9_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1097
translate russian s9_9397760f:

    # "------Wait| spice?"
    "------Стоп, пряный?"

# game/scenario/sc_en.rpy:1098
translate russian s9_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1100
translate russian s9_81465f22:

    # "\"Hold on| wait a sec|\""
    "\"Постой, подожди секунду!\""

# game/scenario/sc_en.rpy:1101
translate russian s9_11f35160:

    # " I realize now that I was daydreaming a ridiculous dream."
    " Теперь я понимаю, что мне \[приснился нелепый сон наяву\]."

# game/scenario/sc_en.rpy:1102
translate russian s9_4c203e16:

    # "\"Hey, Senpai|\""
    "\"Эй, Семпай\""

# game/scenario/sc_en.rpy:1103
translate russian s9_a0db6f3c:

    # " I call Senpai who happily tastes the soup from the bowl."
    " Я окликнул Семпай, которая счасливо пробовала суп из миски."

# game/scenario/sc_en.rpy:1104
translate russian s9_b983e30d:

    # "\"Yes?{w} Something wrong, Tohno-kun?\""
    "\"Да?{w} Что-то не так, Тоно-кун?\""

# game/scenario/sc_en.rpy:1105
translate russian s9_3a2435f6:

    # "\"That's|{w} I forgot to say, but in the morning my body's not stable.\""
    "\"\[Это\],{w} я забыл сказать, что по утрам моё тело \[не стабильно\].\""

# game/scenario/sc_en.rpy:1106
translate russian s9_3bf521bb:

    # "\"I know. But today you look well.\""
    "\"Я знаю. Но сегодня ты выглядишь хорошо.\""

# game/scenario/sc_en.rpy:1107
translate russian s9_8d991e66:

    # " She smiles widely."
    " Она широко улыбается."

# game/scenario/sc_en.rpy:1108
translate russian s9_e4f0d2a8:

    # " Uuuh| if she smiles like that it becomes difficult for me to tell her what I was just going to|"
    " Ууух, если она так улыбается, то становится сложнее сказать ей, что я только что собирался."

# game/scenario/sc_en.rpy:1109
translate russian s9_a13c7105:

    # nvl clear
    # "\"So, I was just going to say|{w} I can eat a normal breakfast, but anything that stimulates the taste buds is kinda| bad|{w} How do I say this|{w} I can only eat Japanese food.\""
    nvl clear
    "\"Я просто собирался сказать,{w} что могу съесть обычный завтрак, но все, что стимулирует вкусовые рецепторы, в некотором роде - вредно.{w} Как бы это сказать.{w} Я могу есть только Японскую еду.\""

# game/scenario/sc_en.rpy:1111
translate russian s9_a00bdeb6:

    # "\"?\""
    "\"?\""

# game/scenario/sc_en.rpy:1112
translate russian s9_41cbd38e:

    # " She tilts her head questioningly."
    " Она вопросительно наклоняет голову."

# game/scenario/sc_en.rpy:1113
translate russian s9_c1ec0bdb:

    # "\"Well, I can eat non-Japanese food, but Japanese would probably be the best.{w} I definitey can't take anything spicy or greasy|\""
    "\"Вообще, я могу есть не Японскую еду, но Япоская, вероятно, была бы лучше всего.{w} Но я точно не могу есть ничего острого или жирного.\""

# game/scenario/sc_en.rpy:1114
translate russian s9_40df7121:

    # "\"Haa?{w} What do you want to say, Tohno-kun?\""
    "\"Ааа?{w} Что ты хочешь сказать, Тоно-кун?\""

# game/scenario/sc_en.rpy:1115
translate russian s9_91e9dfee:

    # nvl clear
    # "\"So|{w} what I'm saying is that I don't like curry for breakfast|\""
    nvl clear
    "\"\[Ну\],{w} я хочу сказать, что не люблю карри на завтрак.\""

# game/scenario/sc_en.rpy:1121
translate russian s9_f88f4416:

    # "\"------------\""
    "\"------------\""

# game/scenario/sc_en.rpy:1122
translate russian s9_b5390663:

    # " Whoosh|"
    " \[Свист\]"

# game/scenario/sc_en.rpy:1123
translate russian s9_e2b1c92a:

    # " The air in the kitchen freezes solid."
    " Воздух на кухне намертво застывает."

# game/scenario/sc_en.rpy:1124
translate russian s9_a87f2979:

    # nvl clear
    # "\"Tohno-kun|{w} that means|\""
    nvl clear
    "\"Тоно-кун,{w} это значит,\""

# game/scenario/sc_en.rpy:1126
translate russian s9_7094e250:

    # "\"Ah, no, no!{w} I didn't say I don't like your curry!{w} And I'm definitely not looking down on curry!{w} It's just that eating curry at a time like this would probably upset my stomach|{w} or something|\""
    "\"А, нет, нет!{w} Я не говорю, что не люблю карри!{w} Я определённо \[не смотрю низко\] на карри!{w} Просто употребление карри в такое время, скорее всего, расстроит мой желудок,{w} или что-то в этом роде.\""

# game/scenario/sc_en.rpy:1132
translate russian s9_f88f4416_1:

    # "\"------------\""
    "\"------------\""

# game/scenario/sc_en.rpy:1133
translate russian s9_60d8b5e1:

    # " Heeeeeee!!!!{w} She's definitely mad!"
    " Хеееееее!!!!{w} Она сошла с ума!"

# game/scenario/sc_en.rpy:1134
translate russian s9_d79dc5f2:

    # nvl clear
    # "\"Tohno-kun|\""
    nvl clear
    "\"Тоно-кун!\""

# game/scenario/sc_en.rpy:1136
translate russian s9_b4c0c408:

    # "\"Yes, yes!{w} I'll eat it!{w} I like curry that I don't like!"
    "\"Да, да!{w} Я съем это!{w} \[Мне нравится карри, которое мне не нравится\]!"

# game/scenario/sc_en.rpy:1137
translate russian s9_98751b81:

    # " What do I have to hide?{w} As you can see, I came to eat curry!\""
    " Что мне скрывать?{w} Как ты видишь, я пришёл поесть карри!\""

# game/scenario/sc_en.rpy:1138
translate russian s9_a3ba5a8f:

    # "\"Don't say such strange things.{w} To eat curry for breakfast| that sounds like an Indian!{w} Did you see me that way, Tohno-kun?\""
    "\"Не говори таких странных вещей.{w} Есть карри на завтрак звучит как что-то Индиское!{w} \[Ты представляешь меня такой\], Тоно-кун?\""

# game/scenario/sc_en.rpy:1139
translate russian s9_ecd6b1fe:

    # " She seems a little upset."
    " Она выглядит немного расстроенной."

# game/scenario/sc_en.rpy:1140
translate russian s9_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1142
translate russian s9_e30b6e36:

    # "\"Eh|?\""
    "\"Э?\""

# game/scenario/sc_en.rpy:1143
translate russian s9_076e4834:

    # "\"Today's breakfast is bread, bean stalk soup and salad.{w} A normal breakfast, I would say.\""
    "\"Сегодняшний завтрак - булочка, бобовый суп и салат.{w} Обычный завтрак, я бы сказала.\""

# game/scenario/sc_en.rpy:1144
translate russian s9_012c9bf6:

    # "\"Eh|{w} EH?\""
    "\"Э?{w} Э?\""

# game/scenario/sc_en.rpy:1145
translate russian s9_e191305c:

    # " That's strange."
    " Это странно."

# game/scenario/sc_en.rpy:1146
translate russian s9_70a693f0:

    # " Because|{w} Senpai is|{w} Senpai|"
    " Потому как{w} Семпай это{w} Семпай."

# game/scenario/sc_en.rpy:1147
translate russian s9_b262997f:

    # " Ciel-senpai equates to curry, and curry equates to Ciel-senpai."
    " Сиель-семпай отождествляется с карри, а карри отождествляется с Сиель-семпай."

# game/scenario/sc_en.rpy:1148
translate russian s9_7cb0507f:

    # " No| no curry means no Senpai!{w} I can't think of Senpai wanting bread!"
    " Нет карри - значит нет Семпай!{w} Не могу представить, чтобы Семпай захотела булочек!"

# game/scenario/sc_en.rpy:1149
translate russian s9_72505271:

    # nvl clear
    # "\"I|{w} I get it|{w} you placate me like this and then later it turns out to be curry bread, right?!\""
    nvl clear
    "\"Я,{w} я понял,{w} ты вот так успокаиваешь меня, а потом окажется, что это булочка с карри, верно?!\""

# game/scenario/sc_en.rpy:1151
translate russian s9_bed69206:

    # "\"Unfortunately, it's not curry bread, either.{w} It would be a great mistake to eat curry for breakfast!\""
    "\"К сожалению это не булочка с карри.{w} Было бы большой ошибкой есть карри на завтрак!\""

# game/scenario/sc_en.rpy:1152
translate russian s9_a4edce4e:

    # " She hands out the bean stalk soup."
    " Она расставляет бобовый суп."

# game/scenario/sc_en.rpy:1153
translate russian s9_8ebdea1e:

    # "\"Tohno-kun, please get the marmalade and margarine from the refrigerator.{w} And we'll have our breakfast.\""
    "\"Тоно-кун, пожалуйста достань мармелад и маргарин из холодильника.{w} И мы можем начинать завтракать.\""

# game/scenario/sc_en.rpy:1154
translate russian s9_684b86ce:

    # " Senpai prepares the table for two."
    " Семпай накрывает стол на двоих."

# game/scenario/sc_en.rpy:1155
translate russian s9_5133bc1d:

    # " She places the wares neatly and refreshingly.{w} The food looks like something even I could eat."
    " Она аккуратно и \[свежо\] расставляет приборы.{w} Еда выглядит так, будто даже я мог бы её съесть."

# game/scenario/sc_en.rpy:1156
translate russian s9_e6a27ca0:

    # nvl clear
    # "\"Wow| it really isn't curry!\""
    nvl clear
    "\"Вау, это и правда не карри!\""

# game/scenario/sc_en.rpy:1158
translate russian s9_a08bfac9:

    # "\"Of course!{w} It's usual to leave the best food for dinner, isn't it?{w} It'd be profane to have curry rice for anything other than dinner!{w} Really!\""
    "\"Конечно!{w} Обычно лучшие блюда оставляют на ужин, разве нет?{w} Было бы кощунством есть рис с карри на что-либо, кроме ужина!{w} \[Воистину\]!"

# game/scenario/sc_en.rpy:1159
translate russian s9_3eeff8ec:

    # " She really looks a bit offended."
    " Она действительно выглядит немного обиженной."

# game/scenario/sc_en.rpy:1160
translate russian s9_25d39436:

    # "\"Ah|{w} is that the reason|\""
    "\"А,{w} вот в чём дело.\""

# game/scenario/sc_en.rpy:1161
translate russian s9_ccef9700:

    # " For some reason I accept it as it is."
    " Почему-то я принял это как есть."

# game/scenario/sc_en.rpy:1162
translate russian s9_766ebc30:

    # nvl clear
    # "\"Senpai, you can really restrain yourself from indulging yourself to enjoy things more.\""
    nvl clear
    "\"Семпай, ты действительно можешь сдерживать себя от того, чтобы потакать себе, чтобы получать больше удовольствия.\""

# game/scenario/sc_en.rpy:1164
translate russian s9_2f266e27:

    # " I get the margarine and marmalade from the refrigerator."
    " Я достал маргарин и мармелад из холодильника."

# game/scenario/sc_en.rpy:1165
translate russian s9_c21dea79:

    # " I'm the opposite.{w} Senpai puts an egg in the stewpot."
    " Я - полная противоположность.{w} Семпай кладёт яйца в кастрюлю."

# game/scenario/sc_en.rpy:1166
translate russian s9_e1cb52cf:

    # "\"Mmm?{w} Is that so?{w} I didn't really think about it much, either.\""
    "\"Ммм?{w} Это так?{w} Я даже особо не задумывалась об этом.\""

# game/scenario/sc_en.rpy:1167
translate russian s9_7d8a2355:

    # " Curry gets transported from the stewpot to the plate."
    " Карри перемещается из кастрюли в тарелку."

# game/scenario/sc_en.rpy:1168
translate russian s9_e0a38099:

    # nvl clear
    # "\"Senpai| what did you just put on the plate?\""
    nvl clear
    "\"Семпай, что ты только что положила на тарелку?\""

# game/scenario/sc_en.rpy:1170
translate russian s9_1339e6bc:

    # "\"Ah, this isn't Tohno-kun's.{w} It's mine, so don't worry.{w} Eating a night old ruro bread makes me feel like I can go the whole day without eating anything else!\""
    "\"А, не для Тоно-куна.{w} Это мне, так что не волнуйся.{w} \[Съедая хлеб руро лежавший с ночи, я чувствую, что могу целый день ничего больше не есть\]!\""

# game/scenario/sc_en.rpy:1171
translate russian s9_a49891ee:

    # " Is that so|{w} is that why the kitchen smells of curry?"
    " Вот как,{w} вот почему на кухне такой запах карри?"

# game/scenario/sc_en.rpy:1172
translate russian s9_6c04c340:

    # nvl clear
    # "\"Uh|{w} Senpai|{w} weren't you going to put off on eating curry until dinner?\""
    nvl clear
    "\"Ух,{w} Семпай,{w} разве ты не собиралась отложить карри до ужина?\""

# game/scenario/sc_en.rpy:1174
translate russian s9_4356d5c8:

    # " I knew the answer, but I still asked."
    " Я знал ответ, но всё равно спросил."

# game/scenario/sc_en.rpy:1175
translate russian s9_761d6ea9:

    # "\"Hmm? It's bread with curry, not curry with bread.\""
    "\"Хм? Это хлеб с карри, а не карри с хлебом.\""

# game/scenario/sc_en.rpy:1176
translate russian s9_4b9bf8a1:

    # "\"Ah|{w} I thought as much|{w} I thought you were going to say that.\""
    "\"А,{w} я так и думал,{w} я так и думал, что ты это скажешь.\""

# game/scenario/sc_en.rpy:1177
translate russian s9_76b2fe88_6:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1183
translate russian s9_182084ca:

    # " I rest a bit after breakfast and then start on my way to school."
    " Я немного отдохнул после завтрака и начал собираться в школу."

# game/scenario/sc_en.rpy:1187
translate russian s9_1c14b65b:

    # "\"Hmm| if we go now, then it'll be third period.{w} Eh, we took a lot of time.\""
    "\"Хмм, если мы пойдём сейчас, тогда успеем на третий урок.{w} Эх, у нас ушло много времени.\""

# game/scenario/sc_en.rpy:1188
translate russian s9_998f72d2:

    # "\"Yes, but, isn't it okay to do this from time to time?\""
    "\"Да, но разве это не нормально - делать это время от времени?\""

# game/scenario/sc_en.rpy:1191
translate russian s9_928954ba:

    # "\"Hehe, you're right.{w} It felt like a family, so I just relaxed a bit.\""
    "\"Хехе, ты права.{w} \[Мы чувствовали себя как одна семья\], так что я просто немного расслабился.\""

# game/scenario/sc_en.rpy:1192
translate russian s9_3627735f:

    # " Uuuuh|{w} I don't know whether she knows or not, but what she said had meaning."
    " Уууух,{w} я не знаю, знает она или нет, но то, что она сказала, имело смысл."

# game/scenario/sc_en.rpy:1193
translate russian s9_5c8ecd4f:

    # nvl clear
    # "\"Then, shall we go?{w} We'll split up at school, but on the way there we should be able to be together.\""
    nvl clear
    "\"Тогда, может, пойдём?{w} Мы расстанемся в школе, но по дороге туда мы можем быть вместе.\""

# game/scenario/sc_en.rpy:1197
translate russian s9_55be0f58:

    # "\"Okay.{w} Then let's head out, Shiki-kun.\""
    "\"Хорошо.{w} Тогда отправляемся, Тоно-кун.\""

# game/scenario/sc_en.rpy:1205
translate russian s9_322cf920:

    # " Calling me Shiki-kun instead of Tohno-kun, she starts heading out."
    " Называя меня Шики-кун вместо Тоно-кун, она направляется к выходу"

# game/scenario/sc_en.rpy:1206
translate russian s9_f88f4416_2:

    # "\"------------\""
    "\"------------\""

# game/scenario/sc_en.rpy:1207
translate russian s9_b053a72b:

    # " I can't even answer coolly and just stand there dumbfounded."
    " Я даже не могу спокойно ответить и просто ошеломлённо стою."

# game/scenario/sc_en.rpy:1208
translate russian s9_76b2fe88_7:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1225
translate russian s10_9720be8c:

    # "This scene is \"School - Morning: With Akiha.\"{nw}"
    "Эта сцена - \"Школа - Утро: с Акихой.\"{nw}"

# game/scenario/sc_en.rpy:1226
translate russian s10_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:1227
translate russian s10_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:1247
translate russian s10_4b4cbb70:

    # "\"I'm tired of waiting!{w} I'm tired!{w} I'm going!{w} I'm going on ahead, Akiha!\""
    "\"Я устал ждать!{w} Я устал!{w} Я пошёл!{w} Я выхожу, Акиха!\""

# game/scenario/sc_en.rpy:1248
translate russian s10_8f7040cb:

    # " I shout to the second floor."
    "Кричу я на второй этаж."

# game/scenario/sc_en.rpy:1249
translate russian s10_f53e3541:

    # " A surprised voice."
    " Удивлённый голос."

# game/scenario/sc_en.rpy:1250
translate russian s10_91a02242:

    # "\"Wa, wait just a moment, Nii-san!{w} I'll be out right now!\""
    "\"Ва, подожди минутку, Нии-сан!{w} Я сейчас спущусь!\""

# game/scenario/sc_en.rpy:1251
translate russian s10_a116eac6:

    # " The sound of hurried footsteps comes from the second floor."
    " Звуки поспешных шагов раздались на втором этаже."

# game/scenario/sc_en.rpy:1252
translate russian s10_f2ac3e89:

    # nvl clear
    # " A long while after breakfast."
    nvl clear
    " Прошло много времени после завтрака."

# game/scenario/sc_en.rpy:1254
translate russian s10_24971ba7:

    # " I hurry Akiha who was up until now relaxedly drinking tea in the living room."
    " Я поторопил Акиху, которая до этого момента спокойно пила чай в гостиной."

# game/scenario/sc_en.rpy:1255
translate russian s10_c2d93ec9:

    # " I say to myself that Akiha deserves punishment for leisurely taking time instead of changing and smile a dark smile."
    " Я говорю себе, что Акиха заслуживает наказания за то, что не торопилась, вместо того чтобы переодеться, и улыбаюсь мрачной улыбкой."

# game/scenario/sc_en.rpy:1256
translate russian s10_8e772577:

    # " And within three short minutes Akiha comes down to the lobby."
    " Через три \[короткие\] минуты Акиха спустилась в холл."

# game/scenario/sc_en.rpy:1257
translate russian s10_b7b786e0:

    # " Really| she doesn't run even when she's short on time.{w} Her lifestyle really is different from mine."
    " И правда, она не спешит, даже если у неё мало времени.{w} Её образ жизни дествительно отличается от моего."

# game/scenario/sc_en.rpy:1258
translate russian s10_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1262
translate russian s10_a19cccf2:

    # "\"I'm sorry|{w} I made you wait.{w} How are we doing on time?\""
    "\"Я извиняюсь,{w} я заставила тебя ждать.{w} Сколько у нас времени?\""

# game/scenario/sc_en.rpy:1263
translate russian s10_8f05b950:

    # "\"Yeah, we still have about twenty minutes to blow.{w} So no worries.\""
    "\"Эмм, у нас в запасе есть ещё двадцать минут.{w} Так что не беспокойся.\""

# game/scenario/sc_en.rpy:1264
translate russian s10_528de25d:

    # " Akiha's troubled panting stops altogether."
    " \[Сбитое дыхание Акихи полностью прекращается\]."

# game/scenario/sc_en.rpy:1268
translate russian s10_a8192cdc:

    # "\"Nii-san| then you didn't have to say that you were going on ahead.\""
    "\"Нии-сан, тогда тебе не нужно было говорить, что ты выходишь.\""

# game/scenario/sc_en.rpy:1269
translate russian s10_15367f50:

    # "\"I know, but I found it weird that someone like you would make others wait.{w} So I just had to
    "\"Я знаю, но мне показалось странным, что кто-то вроде тебя заставляет других ждать.{w} Так что мне пришлось так сделать.\""

# game/scenario/sc_en.rpy:1270
translate russian s10_8570ff58:

    # " I had to play a little trick on you.{w} I probably should keep that to myself."
    " Я решил немного подшутить над тобой.{w} Наверное мне не стоит говорить этого."

# game/scenario/sc_en.rpy:1271
translate russian s10_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1274
translate russian s10_b3fc022c:

    # "\"Nii-san, so what is it?\""
    "\"Нии-сан, так в чём дело?\""

# game/scenario/sc_en.rpy:1275
translate russian s10_797d79d5:

    # "\"Eh| I guess nothing beats going to school early.{w} So let's head on out together.\""
    "\"Э, \[я думаю, нет ничего лучше, чем выйти\] в школу пораньше.{w} Так что пошли вместе.\""

# game/scenario/sc_en.rpy:1276
translate russian s10_c3471582:

    # " I storm out with an upset Akiha."
    " Я выбегаю вместе с расстроенной Акихой."

# game/scenario/sc_en.rpy:1277
translate russian s10_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1283
translate russian s10_899262fb:

    # " We go downhill."
    " Мы спускаемся по склону."

# game/scenario/sc_en.rpy:1284
translate russian s10_985d6d98:

    # " Now that I think about it, we've been going to school together for a while now."
    " \[Теперь, когда я думаю об этом\], мы уже некоторое время ходим в школу вместе."

# game/scenario/sc_en.rpy:1285
translate russian s10_a0e07fdc:

    # " After Akiha transferred, we went to school together when I woke up in time.{w} Although that happens only about twice a week|"
    " После того, как Акиха перевелась, мы ходим в школу вместе, когда я просыпаюсь вовремя.{w} Хотя обычно это происходит всего дважды в неделю."

# game/scenario/sc_en.rpy:1286
translate russian s10_c3a98c8a:

    # nvl clear
    # "\"------\""
    nvl clear
    "\"------\""

# game/scenario/sc_en.rpy:1288
translate russian s10_273380ba:

    # " Stop."
    " Стоп."

# game/scenario/sc_en.rpy:1289
translate russian s10_daf36674:

    # " I stop after hitting a stone on the road."
    " Я остановился после удара о камень на дороге."

# game/scenario/sc_en.rpy:1292
translate russian s10_0e27530a:

    # "\"Nii-san, what is it?{w} You look like you've seen a ghost.\""
    "\"Нии-сан, что случилось?{w} Ты выглядишь так, будто призрака увидел.\""

# game/scenario/sc_en.rpy:1293
translate russian s10_6cb09322:

    # " Akiha looks at me from the side."
    " Акиха вполоборота посмотрела на меня."

# game/scenario/sc_en.rpy:1294
translate russian s10_ad6b98b1:

    # " The scent from her black hair on her shoulder sways me a bit."
    " Аромат, исходящий от её черных волос на плече, немного завораживает меня"

# game/scenario/sc_en.rpy:1295
translate russian s10_7714d978:

    # nvl clear
    # "\"Ah, hmm|{w} I feel like I went to school with you yesterday, too.\""
    nvl clear
    "\"А, хм,{w} Мне кажется я и вчера ходил с тобой в школу.\""

# game/scenario/sc_en.rpy:1297
translate russian s10_a1c57fa1:

    # " Akiha just smiles happily, as if she's happy to hear such a thing from me."
    " Акиха просто счастливо улыбнулась, как будто она счастлива слышать такое от меня."

# game/scenario/sc_en.rpy:1300
translate russian s10_c6e71cb0:

    # "\"You've been waking up early nowadays.{w} I find it really enjoyable that I can go to school with you.\""
    "\"В последнее время ты рано просыпаешься.{w} Мне действительно приятно, что я могу ходить с тобой в школу.\""

# game/scenario/sc_en.rpy:1301
translate russian s10_30a410b0:

    # "\"Uuuuh|\""
    "\"Уууух\""

# game/scenario/sc_en.rpy:1302
translate russian s10_21123581:

    # " If she says that to me so frankly, I really don't know what to say."
    " \[Если она говорит мне это так откровенно, я действительно не знаю, что сказать.\]"

# game/scenario/sc_en.rpy:1303
translate russian s10_7fbef32b:

    # "\"Oh| oh?{w} If you're happy, then I'm happy, too.\""
    "\"О, о?{w} Если ты счастлива, тогда я тоже счастлив.\""

# game/scenario/sc_en.rpy:1304
translate russian s10_c60dc40d:

    # " Maybe it's just my feeling, but I feel as if there is a trap today."
    " Может быть это просто моё воображение, но я чувствую, что \[это ловушка\]."

# game/scenario/sc_en.rpy:1305
translate russian s10_aeffd470:

    # nvl clear
    # "\"Ah, I know what you want to say.{w} There's nothing wrong happening at school and the mansion.{w} It's almost scary that these days are so peaceful.\""
    nvl clear
    "\"А, я знаю что ты хочешь сказать.{w} В школе и особняке \[не происходит\] ничего плохого.{w} Немного пугает, что эти дни такие мирные.\""

# game/scenario/sc_en.rpy:1314
translate russian s10_b01c389c:

    # "\"------\""
    "\"------\""

# game/scenario/sc_en.rpy:1315
translate russian s10_539338b0:

    # " Aah| that is true."
    "Аах, это правда."

# game/scenario/sc_en.rpy:1316
translate russian s10_71bc2d3a:

    # " I worry when happy things continue to happen.{w} I'm not afraid of painful times becoming more painful because of the happy times, but of happy times ending and going back to normal."
    " Я начинаю волноваться, \[когда счастливые времена продолжаются\].{w} Я боюсь не того, что плохие времена станут еще более болезненными из-за \[счастливых времен\], а того, что \[счастливые времена\] закончатся и все вернется на круги своя."

# game/scenario/sc_en.rpy:1317
translate russian s10_e852e0ad:

    # " A feeling of depression at the end of a festival, for example."
    " Чувство подавленности в конце фестиваля, например."

# game/scenario/sc_en.rpy:1318
translate russian s10_8a43c1ac:

    # " The happiest moment of life is the happiest| but it's also a flash.{w} I subconsciously fear it."
    " Самый счастливый момент в жизни обычно ещё и самый короткий.{w} Я подсознательно боюсь этого."

# game/scenario/sc_en.rpy:1319
translate russian s10_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1322
translate russian s10_57046d9c:

    # "\"But, it's okay.{w} Soon it'll be the cultural festival.{w} Happy times will continue.{w} And after the festival will be winter break.{w} We can all travel during the break.{w} And when we return it'll be around New Year's Day."
    "\"Но, это нормально.{w} Скоро будет культурный фестиваль.{w} \[Счастливые времена\] продолжатся.{w} А после фестиваля будут зимние каникулы.{w} Мы все сможем попутешествовать на каникулах.{w} А вернёмся мы уже под Новый Год."

# game/scenario/sc_en.rpy:1323
translate russian s10_6795c831:

    # " See, Nii-san?{w} If an important person is by your side, then happy times will continue.\""
    " Видишь, Нии-сан?{w} Если рядом с тобой важный человек, то \[счастливые времена продолжатся\].\""

# game/scenario/sc_en.rpy:1324
translate russian s10_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1332
translate russian s10_ffbc8e35:

    # " Akiha walks ahead with light steps."
    " Акиха, лёгкими шагами, идёт впереди."

# game/scenario/sc_en.rpy:1333
translate russian s10_e49b457c:

    # "\"Yeah| you're definitely right, Akiha.\""
    "\"Да, ты \[действительно права\], Акиха.\""

# game/scenario/sc_en.rpy:1334
translate russian s10_df759ff3:

    # " Even my small worries are no match for Akiha."
    " Даже мои небольшие заботы не идут ни в какое сравнение с Акихой."

# game/scenario/sc_en.rpy:1335
translate russian s10_ee049944:

    # " I have Akiha, Hisui and Kohaku-san.{w} I have a home to return to, so I have nothing to worry about."
    " У меня есть Акиха, Хисуи и Кохаку-сан.{w} У меня есть дом, в который можно вернуться, так что не о чем беспокоиться."

# game/scenario/sc_en.rpy:1336
translate russian s10_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1343
translate russian s10_42ceb2ee:

    # "\"Then, let's meet during lunch break!\""
    "\"Тогда, увидимся на обеденном перерыве!\""

# game/scenario/sc_en.rpy:1351
translate russian s10_e0968205:

    # " Akiha breaks off and happily goes to the freshman section."
    " Акиха прерывается и радостно направляется в \[секцию\] для первокурсников."

# game/scenario/sc_en.rpy:1352
translate russian s10_76b2fe88_6:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1366
translate russian s11_8caec6cf:

    # "This scene is \"School - Morning: Preparations.\"{nw}"
    "Эта сцена - \"Школа - Утро: \[Подготовка\].\"{nw}"

# game/scenario/sc_en.rpy:1367
translate russian s11_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:1368
translate russian s11_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:1386
translate russian s11_a266c98e:

    # " But even if I decide to go to school, I have two choices|"
    " Но даже если я пойду в школу у меня есть два варианта:"

# game/scenario/sc_en.rpy:1387
translate russian s11_d2cf607f:

    # " I can go with Akiha, or go alone hoping to meet someone new."
    " Я могу пойти с Акихой, или пойти один, надеясь повстречать кого-то нового."

# game/scenario/sc_en.rpy:1388
translate russian s11_86d2796d:

    # "\"If I'm with Akiha, then I definitely won't meet a new person|\""
    "\"Если я с Акихой, то я определённо не встречу \[нового человека\]\""

# game/scenario/sc_en.rpy:1389
translate russian s11_b29c6157:

    # " Then, what should I do?{nw}"
    " Тогда, что мне делать?{nw}"

# game/scenario/sc_en.rpy:1390
translate russian s11_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:1402
translate russian s12_0f2816ea:

    # "This scene is \"School - Morning: The Inui Family.\"{nw}"
    ""

# game/scenario/sc_en.rpy:1403
translate russian s12_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:1404
translate russian s12_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1422
translate russian s12_9276e975:

    # "\"Hmm| Arihiko's house, eh?{w} But| why|?\""
    ""

# game/scenario/sc_en.rpy:1423
translate russian s12_56487bf6:

    # " I wonder at myself as I head over to Arihiko's without any particular reason."
    ""

# game/scenario/sc_en.rpy:1424
translate russian s12_2bcec2d5:

    # " I do sometimes visit after school to pass time, but in the morning?"
    ""

# game/scenario/sc_en.rpy:1425
translate russian s12_bace11f4:

    # "\"Well| I guess once in a while won't hurt|\""
    ""

# game/scenario/sc_en.rpy:1426
translate russian s12_ccd4d08f:

    # " Life is interesting because of these random impulses."
    ""

# game/scenario/sc_en.rpy:1427
translate russian s12_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1434
translate russian s12_c206017d:

    # " So here I am.{w} The house of the shy and happy Inui Arihiko-kun."
    ""

# game/scenario/sc_en.rpy:1435
translate russian s12_25bd5781:

    # nvl clear
    # " I ring the bell."
    nvl clear
    ""

# game/scenario/sc_en.rpy:1437
translate russian s12_0ef3f234:

    # " Ding dong!{w} After a few minutes the main door opens with a ghastly creak."
    ""

# game/scenario/sc_en.rpy:1438
translate russian s12_499c92b2:

    # "\"Who?\""
    ""

# game/scenario/sc_en.rpy:1439
translate russian s12_ab71338a:

    # " Asks the woman who only pokes half her face out of the door."
    ""

# game/scenario/sc_en.rpy:1440
translate russian s12_c01a6f31:

    # nvl clear
    # "\"Hello, Ichigo-san.{w} If you're coming out, does that mean Arihiko-kun's already left?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1442
translate russian s12_4d3d9dcd:

    # "\"Ah?{w} That idiot didn't come home yesterday.\""
    ""

# game/scenario/sc_en.rpy:1443
translate russian s12_3e745a42:

    # " The woman, in deep thought now, steps out of the doorway."
    ""

# game/scenario/sc_en.rpy:1444
translate russian s12_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1447
translate russian s12_27042e30:

    # "\"What|{w} I was wondering who it was, and it turns out to be you, Arima.{w} Do excuse me.{w} I finally fell asleep after three days of work.\""
    ""

# game/scenario/sc_en.rpy:1448
translate russian s12_eba8c589:

    # "\"I'm sorry to wake you up.{w} It's been a while, Ichigo-san.\""
    ""

# game/scenario/sc_en.rpy:1449
translate russian s12_4be5e531:

    # "\"Yeah.{w} You haven't changed much from being an awkward teenager.\""
    ""

# game/scenario/sc_en.rpy:1450
translate russian s12_ea6feefc:

    # " She puffs out a smoke.{w} And smiles a mysteriously indescribable smile."
    ""

# game/scenario/sc_en.rpy:1451
translate russian s12_bd376221:

    # nvl clear
    # "------Inui Ichigo-san."
    nvl clear
    ""

# game/scenario/sc_en.rpy:1453
translate russian s12_bbe1c72c:

    # " Inui.{w} Her last name implies that she's Arihiko's big sister."
    ""

# game/scenario/sc_en.rpy:1454
translate russian s12_00d52f69:

    # " Profession unknown, says Arihiko."
    ""

# game/scenario/sc_en.rpy:1455
translate russian s12_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1462
translate russian s12_040ba766:

    # "\"Hey, Tohno, I know you're in a special deal with Takada-kun, but listen to me.\""
    ""

# game/scenario/sc_en.rpy:1463
translate russian s12_24f4dd33:

    # "\"Okay, what is it?\""
    ""

# game/scenario/sc_en.rpy:1464
translate russian s12_46e564f1:

    # "\"The truth is|{w} I thought my sister was a detective when I was in elementary school.\""
    ""

# game/scenario/sc_en.rpy:1465
translate russian s12_cd4f542d:

    # "\"Heeeeh?{w} Meaning she's not?{w} That's unexpected.\""
    ""

# game/scenario/sc_en.rpy:1466
translate russian s12_6c53c886:

    # "\"Aaah| so from middle school onward I thought maybe she was a novelist|{w} but that wasn't it, either.\""
    ""

# game/scenario/sc_en.rpy:1467
translate russian s12_e3a6533c:

    # "\"Hmm?{w} She's not a novelist, either?{w} Your second guess failed as well? Then| what does she do?\""
    ""

# game/scenario/sc_en.rpy:1468
translate russian s12_838ec8e7:

    # nvl clear
    # "\"Yeah| from high school on I thought maybe she was a journalist.{w} What do you think?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1470
translate russian s12_d03f8fe0:

    # "\"Mmm| anyway, don't cross into my territory|{w} A special deal doesn't work that way.\""
    ""

# game/scenario/sc_en.rpy:1471
translate russian s12_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1476
translate russian s12_6c05d9ca:

    # " | Or something like that."
    ""

# game/scenario/sc_en.rpy:1477
translate russian s12_c5d2ccfe:

    # " Anyway, the point is that what Ichigo-san does will always remain a mystery."
    ""

# game/scenario/sc_en.rpy:1478
translate russian s12_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1481
translate russian s12_d1e9a426:

    # "\"So, what's up today?{w} Did the family idiot do something stupid again?\""
    ""

# game/scenario/sc_en.rpy:1482
translate russian s12_048c584c:

    # "\"No.{w} I don't know whether he's in trouble.{w} Anyway, I wasn't affected by it.{w} Well, I just thought maybe it'd be good to go to school together once in a while.\""
    ""

# game/scenario/sc_en.rpy:1483
translate russian s12_cdb14020:

    # "\"Mmm.{w} Sorry that you got to know him.{w} He only confides in you, and you don't brush him off, either.\""
    ""

# game/scenario/sc_en.rpy:1484
translate russian s12_05ec96eb:

    # "\"Aah|{w} I wouldn't brush him off."
    ""

# game/scenario/sc_en.rpy:1485
translate russian s12_0682bc69:

    # " So| where is Arihiko?\""
    ""

# game/scenario/sc_en.rpy:1486
translate russian s12_289624a8:

    # nvl clear
    # "\"I don't really know.{w} I think he was at Dachi's.{w} I'll tell him to go to school, so if he skips, please let me know.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1488
translate russian s12_8afa9f92:

    # " I consent."
    ""

# game/scenario/sc_en.rpy:1489
translate russian s12_b01c389c:

    # "\"------\""
    ""

# game/scenario/sc_en.rpy:1490
translate russian s12_4d9a4ce7:

    # " Ichigo-san looks my way for a while and then,{nw}"
    ""

# game/scenario/sc_en.rpy:1491
translate russian s12_19d1332c:

    # "\"Arima, let me see your hand.\""
    ""

# game/scenario/sc_en.rpy:1492
translate russian s12_cae79e5d:

    # " She says curtly."
    ""

# game/scenario/sc_en.rpy:1493
translate russian s12_d26ffb59:

    # "\"What is it?\""
    ""

# game/scenario/sc_en.rpy:1494
translate russian s12_b66a0bda:

    # " I show her my hand unconditionally."
    ""

# game/scenario/sc_en.rpy:1495
translate russian s12_ce428403:

    # " And on the outstretched hand, Ichigo-san puts something."
    ""

# game/scenario/sc_en.rpy:1496
translate russian s12_a797e23f:

    # nvl clear
    # "\"Here.{w} When you're free, come again.{w} I'll buy you dinner.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1505
translate russian s12_c16af2a0:

    # " She waves and goes into the house."
    ""

# game/scenario/sc_en.rpy:1506
translate russian s12_b0de56c9:

    # "\"||||||\""
    ""

# game/scenario/sc_en.rpy:1507
translate russian s12_76fbfb12:

    # " Alone, I look at my hand."
    ""

# game/scenario/sc_en.rpy:1508
translate russian s12_84e3c951:

    # " And on there was a half-empty tube of gold acrylic paint."
    ""

# game/scenario/sc_en.rpy:1509
translate russian s12_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1525
translate russian s13_3240346d:

    # "This scene is \"School - Morning: The Inui Family, Part II.\"{nw}"
    ""

# game/scenario/sc_en.rpy:1526
translate russian s13_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:1527
translate russian s13_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1545
translate russian s13_528bb640:

    # "\"Hmm|{w} Arihiko's house, eh?{w} But| why|?\""
    ""

# game/scenario/sc_en.rpy:1546
translate russian s13_56487bf6:

    # " I wonder at myself as I head over to Arihiko's without any particular reason."
    ""

# game/scenario/sc_en.rpy:1547
translate russian s13_2bcec2d5:

    # " I do sometimes visit after school to pass time, but in the morning?"
    ""

# game/scenario/sc_en.rpy:1548
translate russian s13_bace11f4:

    # "\"Well| I guess once in a while won't hurt|\""
    ""

# game/scenario/sc_en.rpy:1549
translate russian s13_ccd4d08f:

    # " Life is interesting because of these random impulses."
    ""

# game/scenario/sc_en.rpy:1550
translate russian s13_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1557
translate russian s13_c206017d:

    # " So here I am.{w} The house of the shy and happy Inui Arihiko-kun."
    ""

# game/scenario/sc_en.rpy:1558
translate russian s13_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1561
translate russian s13_66c8bf1a:

    # " Ding dong!{w} The doorbell rings."
    ""

# game/scenario/sc_en.rpy:1562
translate russian s13_2451efb7:

    # " And after a few minutes, it looks like no one will come out."
    ""

# game/scenario/sc_en.rpy:1563
translate russian s13_bbda4ea0:

    # "\"Is he not in?{w} Arihiko|{w} Where's he staying now?\""
    ""

# game/scenario/sc_en.rpy:1564
translate russian s13_344389a4:

    # " I stand there for about thirty seconds, and then decide to go to school."
    ""

# game/scenario/sc_en.rpy:1575
translate russian s13_acd3b479:

    # "\"Wha, what is it?!\""
    ""

# game/scenario/sc_en.rpy:1576
translate russian s13_092d89fc:

    # " Just now there was a loud noise from the second floor."
    ""

# game/scenario/sc_en.rpy:1577
translate russian s13_42996637:

    # " Sound as if something fell, or maybe a burglar.{w} Some weird noise."
    ""

# game/scenario/sc_en.rpy:1578
translate russian s13_8186581a:

    # nvl clear
    # "\"Uh| is this house safe|?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1580
translate russian s13_0b67ef57:

    # " I put my hands on the door handle."
    ""

# game/scenario/sc_en.rpy:1581
translate russian s13_b67f9371:

    # "\"Uaaaah!{w} It's open!\""
    ""

# game/scenario/sc_en.rpy:1582
translate russian s13_810f9a24:

    # " I open it all the way."
    ""

# game/scenario/sc_en.rpy:1583
translate russian s13_a86f778e:

    # " It's unfortunate, but now that I'm in, I should check things out."
    ""

# game/scenario/sc_en.rpy:1584
translate russian s13_b6cf23f9:

    # " Maybe a burglar came in while Arihiko's absent.{w} If the burglar were to steal Ichigo-san's precious panda guitar, then it'll be the end of Arihiko."
    ""

# game/scenario/sc_en.rpy:1585
translate russian s13_8e1674fd:

    # " I should look around for Arihiko|{w} As a friend!"
    ""

# game/scenario/sc_en.rpy:1586
translate russian s13_1a14c12b:

    # nvl clear
    # "\"What am I talking about|{w} it's so unlike me|\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1588
translate russian s13_6ec8d49f:

    # " I excuse myself into the house."
    ""

# game/scenario/sc_en.rpy:1589
translate russian s13_7beeaa27:

    # " I've been in his house many times while he was gone, so it doesn't feel unnatural at all."
    ""

# game/scenario/sc_en.rpy:1590
translate russian s13_399c083e:

    # " Then, to the second floor, the origin of the noise."
    ""

# game/scenario/sc_en.rpy:1591
translate russian s13_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1596
translate russian s13_0807a94e:

    # " The stairs creak under my feet, but I make it to the second floor."
    ""

# game/scenario/sc_en.rpy:1597
translate russian s13_7b39c72a:

    # " On the second floor is only Arihiko's room and a storage room."
    ""

# game/scenario/sc_en.rpy:1598
translate russian s13_54747998:

    # " Some sort of rustling noise."
    ""

# game/scenario/sc_en.rpy:1599
translate russian s13_a93a62d6:

    # " It's coming from Arihiko's room."
    ""

# game/scenario/sc_en.rpy:1600
translate russian s13_6174df41:

    # "\"Uuuuh| there is definitely something going on.\""
    ""

# game/scenario/sc_en.rpy:1601
translate russian s13_dc3b47be:

    # " I don't like it.{w} From the sound I would say it's a two meter tall roach.{w} And since it is Arihiko's room after all, a magical beast might pop out, too."
    ""

# game/scenario/sc_en.rpy:1602
translate russian s13_df6ccc6a:

    # nvl clear
    # " Rustle, rustle, rustle, rustle| CLICK!{w}　|| Rustle, rustle, rustle, munch, munch, swallow, aaaaa, water, rustle, rustle, swallow|"
    nvl clear
    ""

# game/scenario/sc_en.rpy:1604
translate russian s13_c3c6c88c:

    # nvl clear
    # "\"Eh|?{w} Was there a word mixed in there?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1606
translate russian s13_ae39aebd:

    # " Ma, maybe a talking roach?!{w} Even Nrvnqsr wouldn't have such a thing!"
    ""

# game/scenario/sc_en.rpy:1607
translate russian s13_a8d5534e:

    # "\"Ah|{w} I really don't like it|{w} Maybe it'll eat me as soon as I open the door|\""
    ""

# game/scenario/sc_en.rpy:1608
translate russian s13_5827c9fa:

    # " I gather up my courage to put my hand on the door handle."
    ""

# game/scenario/sc_en.rpy:1609
translate russian s13_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1610
translate russian s13_2cd06d91:

    # " Then, one, two, THREE!"
    ""

# game/scenario/sc_en.rpy:1611
translate russian s13_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1616
translate russian s13_d0bceb5f:

    # " |||||| I don't know what to say|"
    ""

# game/scenario/sc_en.rpy:1617
translate russian s13_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1619
translate russian s13_83e97755:

    # "\"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAH!!!!\""
    ""

# game/scenario/sc_en.rpy:1620
translate russian s13_83e97755_1:

    # "\"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAH!!!!\""
    ""

# game/scenario/sc_en.rpy:1621
translate russian s13_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1623
translate russian s13_de6e9eb6:

    # "\"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAH!\""
    ""

# game/scenario/sc_en.rpy:1624
translate russian s13_168f0ba7:

    # " With unnatural speed I escape the abyss."
    ""

# game/scenario/sc_en.rpy:1627
translate russian s13_6be81ae6:

    # "\"What are you doing, Arima?\""
    ""

# game/scenario/sc_en.rpy:1628
translate russian s13_921e51a3:

    # "\"AAAAAH!!!{w} Another weird mons|{w} No|{w} Ichigo-san?\""
    ""

# game/scenario/sc_en.rpy:1629
translate russian s13_4bb4a375:

    # "\"Yeah, it's not a weird monster.\""
    ""

# game/scenario/sc_en.rpy:1630
translate russian s13_f874abdd:

    # "\"Ah| hah, hah| Really|\""
    ""

# game/scenario/sc_en.rpy:1631
translate russian s13_416ccb2c:

    # " Huuu| I catch my breath again and calm down."
    ""

# game/scenario/sc_en.rpy:1632
translate russian s13_76b2fe88_6:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1635
translate russian s13_8f7f6eac:

    # "\"So, what did the idiot do this time?\""
    ""

# game/scenario/sc_en.rpy:1636
translate russian s13_b8d2f5c5:

    # " It looks like she's just come home from work."
    ""

# game/scenario/sc_en.rpy:1637
translate russian s13_437a48ed:

    # "\"Ah, no|{w} it's|{w} I thought I saw a white spirit.{w} Did Arihiko-kun do something recently?{w} Maybe he hit a horse or a cow with a motorcycle or a truck?\""
    ""

# game/scenario/sc_en.rpy:1638
translate russian s13_4768c48d:

    # "\"Ah, I asked, too, but he said he's innocent."
    ""

# game/scenario/sc_en.rpy:1641
translate russian s13_587d47ce:

    # " | Hmm| if you've seen it, too, then this might be troublesome.\""
    ""

# game/scenario/sc_en.rpy:1649
translate russian s13_6aa91d2f:

    # " With that, she boldly steps into the house."
    ""

# game/scenario/sc_en.rpy:1650
translate russian s13_fe754410:

    # nvl clear
    # "\"Ichigo-san| are you going in?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1654
translate russian s13_84b7e651:

    # "\"It's my house.{w} I can't sleep unless I go in.{w} And that won't do any harm to you, so don't worry.{w} It only harms Arihiko, it seems.\""
    ""

# game/scenario/sc_en.rpy:1655
translate russian s13_ac49a20c:

    # "\"Haa| only Arihiko, eh?\""
    ""

# game/scenario/sc_en.rpy:1656
translate russian s13_a17dbc16:

    # " I don't know what's going on| and don't want to.{w} I wonder what he'll say|"
    ""

# game/scenario/sc_en.rpy:1659
translate russian s13_bdcfbef5:

    # "\"His hobby's changed, hmm?{w} It's a hobby that makes me worry as a family member.\""
    ""

# game/scenario/sc_en.rpy:1667
translate russian s13_c8c1154a:

    # " She waves and disappears."
    ""

# game/scenario/sc_en.rpy:1668
translate russian s13_ffc92800:

    # nvl clear
    # "\"||||||\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1670
translate russian s13_12d7339c:

    # " Then|"
    ""

# game/scenario/sc_en.rpy:1671
translate russian s13_e39f9bad:

    # " I'll just forget about this and go to school."
    ""

# game/scenario/sc_en.rpy:1672
translate russian s13_76b2fe88_7:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1689
translate russian s15_888eeba0:

    # "This scene is \"School - Morning.\"{nw}"
    ""

# game/scenario/sc_en.rpy:1690
translate russian s15_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:1691
translate russian s15_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1709
translate russian s15_3ff856df:

    # " But I still have a bit of time before homeroom."
    ""

# game/scenario/sc_en.rpy:1710
translate russian s15_89833734:

    # " Then|{nw}"
    ""

# game/scenario/sc_en.rpy:1711
translate russian s15_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1724
translate russian s16_3d6e4263:

    # "This scene is \"School - Morning: Tea Ceremony Club room.\"{nw}"
    ""

# game/scenario/sc_en.rpy:1725
translate russian s16_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:1726
translate russian s16_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1744
translate russian s16_2615ae07:

    # " I'll go to the Tea Ceremony Club room."
    ""

# game/scenario/sc_en.rpy:1745
translate russian s16_2cbe07b0:

    # " Maybe Senpai came early and is drinking tea right now."
    ""

# game/scenario/sc_en.rpy:1746
translate russian s16_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1753
translate russian s16_05b23cf3:

    # "\"SENPAI!{w} HIIIIII!!!!!\""
    ""

# game/scenario/sc_en.rpy:1754
translate russian s16_0ef85731:

    # " What am I|{w} an elementary school kid|?"
    ""

# game/scenario/sc_en.rpy:1755
translate russian s16_8bb3f760:

    # "\"Ah| I remember now.{w} She said she'll be absent before and after school because of the cultural festival|\""
    ""

# game/scenario/sc_en.rpy:1756
translate russian s16_a498a8b5:

    # " I think she said something like that a few days ago."
    ""

# game/scenario/sc_en.rpy:1757
translate russian s16_82450134:

    # "\"Mmm|{w} I forgot even that|{w} Maybe I'm becoming too loose.\""
    ""

# game/scenario/sc_en.rpy:1758
translate russian s16_f57be0b4:

    # " I shake my head at my own absentmindedness and leave the tea room."
    ""

# game/scenario/sc_en.rpy:1759
translate russian s16_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1776
translate russian s17_e0edcdec:

    # "This scene is \"School - Morning: Class - Secret mission: Make the sign.\"{nw}"
    ""

# game/scenario/sc_en.rpy:1777
translate russian s17_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:1778
translate russian s17_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1800
translate russian s17_89a81f5c:

    # "\"Hello!\""
    ""

# game/scenario/sc_en.rpy:1801
translate russian s17_7a319c99:

    # " With or without energy, I don't know, but I go into to the classroom."
    ""

# game/scenario/sc_en.rpy:1802
translate russian s17_c6375179:

    # " None of the girls are present.{w} A few of the boys are in the middle working on something."
    ""

# game/scenario/sc_en.rpy:1803
translate russian s17_c39d6588:

    # "\"Oooh!{w} Reinforcements!\""
    ""

# game/scenario/sc_en.rpy:1804
translate russian s17_741230e1:

    # " One of the boys who were scrunched over gets up.{w} He's the ace of the Judo club, Kitsuyoshi Yoshinobu-kun."
    ""

# game/scenario/sc_en.rpy:1805
translate russian s17_fcc1b23e:

    # nvl clear
    # "\"Ah?{w} Who?{w} Who is it?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1807
translate russian s17_12d1f9dd:

    # " The members of the mysterious gathering look up, following the example of Kitsuyoshi."
    ""

# game/scenario/sc_en.rpy:1808
translate russian s17_e8d2db92:

    # "\"What, it's Tohno.{w} You seem to have a lot of free time.\""
    ""

# game/scenario/sc_en.rpy:1809
translate russian s17_1067c287:

    # "\"Hey, you came early, Tohno.\""
    ""

# game/scenario/sc_en.rpy:1810
translate russian s17_068beb9b:

    # "\"What, then you're saying I have a lot of free time, too?\""
    ""

# game/scenario/sc_en.rpy:1811
translate russian s17_04722a2c:

    # "\"No, that's|{w} we can't leave this up to a non-artistic farce.\""
    ""

# game/scenario/sc_en.rpy:1812
translate russian s17_57ec19c5:

    # "\"No, what am I hiding|{w} I even won a bronze medal in elementary school!\""
    ""

# game/scenario/sc_en.rpy:1813
translate russian s17_b9bd8ef3:

    # "\"It's not that we have free time|{w} it's just|{w} we're weird|{w} And the girls are so cold."
    ""

# game/scenario/sc_en.rpy:1814
translate russian s17_a1feb111:

    # nvl clear
    # "\"Hey!{w} I thought I told one of you to keep Inui quiet!\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1816
translate russian s17_ff92bb5b:

    # "\"No!{w} Because only a sports club member can hold him down, he gets to do the dirty work.\""
    ""

# game/scenario/sc_en.rpy:1817
translate russian s17_19dee514:

    # "\"If that's your logic, then Tohno can go either way, since he's neither athletic nor artistic?\""
    ""

# game/scenario/sc_en.rpy:1818
translate russian s17_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1819
translate russian s17_3d4b017e:

    # "\"No|{w} I don't remember being that lucky|\""
    ""

# game/scenario/sc_en.rpy:1820
translate russian s17_7c320d67:

    # " I answer the last question and sigh.{w} These kids are fantastically out of control|"
    ""

# game/scenario/sc_en.rpy:1821
translate russian s17_7aea29ff:

    # " Including Kitsuyoshi, there were six rowdy guys armed with paint and markers."
    ""

# game/scenario/sc_en.rpy:1822
translate russian s17_4ab5ae69:

    # nvl clear
    # "\"What|{w} I was wondering what five, six grown men were doing, and all you guys are doing is making a weird sign|\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1827
translate russian s17_9b649968:

    # "\"I-told-you-to-leave-the-coloring-to-me!{w} I'll show you the talent of the disregarded Inui Arihiko!{w} It'll be as if you were dreaming!\""
    ""

# game/scenario/sc_en.rpy:1828
translate russian s17_cad3d41a:

    # " So says the class delinquent, tied up and with a pink paint bucket in one of his hands."
    ""

# game/scenario/sc_en.rpy:1829
translate russian s17_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1837
translate russian s17_f1aacfea:

    # "\"Ah| then you'll have to die, no?{w} Geniuses only get recognized after they die.\""
    ""

# game/scenario/sc_en.rpy:1838
translate russian s17_fdadfabc:

    # " A munching sound."
    ""

# game/scenario/sc_en.rpy:1839
translate russian s17_6f69c34c:

    # " In one corner of the classroom is the silhouette of a figure resembling a rice cake.{w} Takada-kun looks like he has nothing to do but eat his bread and watch the raggedy group of males."
    ""

# game/scenario/sc_en.rpy:1840
translate russian s17_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1844
translate russian s17_42da6781:

    # "\"All right!{w} My beautiful calligraphy!{w} The next famous line of the century!{w} Let's see|{w} 'Panda eats a bamboo and drinks a ball| -Arihiko'\""
    ""

# game/scenario/sc_en.rpy:1845
translate russian s17_556e2e13:

    # "\"Uuuuaaaaah!{w} Arihiko's writing nonsense, again!\""
    ""

# game/scenario/sc_en.rpy:1846
translate russian s17_c99f072f:

    # "\"Heeeeeeee!!!{w} Stop him!{w} Stop Inui!\""
    ""

# game/scenario/sc_en.rpy:1847
translate russian s17_f00b3eb5:

    # "\"Ah| definitely feels like a dream|\""
    ""

# game/scenario/sc_en.rpy:1848
translate russian s17_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1852
translate russian s17_b19546a6:

    # "\"All right!{w} It's perfect!{w} It's on par with Hokusai!{w} No one will think it's a mere cultural festival sign!\""
    ""

# game/scenario/sc_en.rpy:1853
translate russian s17_4962a40f:

    # "\"No| it's not even a sign|\""
    ""

# game/scenario/sc_en.rpy:1854
translate russian s17_81f7c2a3:

    # "\"Aaah| that's enough.{w} Snap out of your dream!\""
    ""

# game/scenario/sc_en.rpy:1866
translate russian s17_70fdc80b:

    # " With a crash Arihiko goes airborne."
    ""

# game/scenario/sc_en.rpy:1867
translate russian s17_f795940c:

    # " I watch him fall over in slow motion and know that it was a critical hit."
    ""

# game/scenario/sc_en.rpy:1868
translate russian s17_17afa1fc:

    # nvl clear
    # "\"Erase it with white paint!{w} Hey, Tohno, if you're free, help!{w} We have to finish this by homeroom!\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1870
translate russian s17_5acf9ef6:

    # " The loud voice of Kitsuyoshi really suits his big body.{w} It probably reached the next classroom, and maybe even the one next to that one."
    ""

# game/scenario/sc_en.rpy:1871
translate russian s17_615e3e2e:

    # " All right.{w} I roll up my sleeves and join the group."
    ""

# game/scenario/sc_en.rpy:1872
translate russian s17_f587b095:

    # nvl clear
    # "\"I was going to bother you even if you hadn't asked.{w} But| weren't we doing something else yesterday?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1874
translate russian s17_798dc56a:

    # "\"Yeah, that got scrapped pretty quickly.\""
    ""

# game/scenario/sc_en.rpy:1875
translate russian s17_2aadaf90:

    # "\"Oh?{w} But we made it pretty well.{w} Even the student council thought it was good.\""
    ""

# game/scenario/sc_en.rpy:1876
translate russian s17_9fd69ac3:

    # "\"Well, the thing is|{w} There was this idiot who wanted to 'brighten' it up with some red paint|\""
    ""

# game/scenario/sc_en.rpy:1877
translate russian s17_b03a0b4a:

    # " Kitsuyoshi Yoshinobu is complaining."
    ""

# game/scenario/sc_en.rpy:1878
translate russian s17_80439d78:

    # " Really|{w} I realize why Arihiko's here so early in the morning: he's here to get punished."
    ""

# game/scenario/sc_en.rpy:1879
translate russian s17_76e4629b:

    # nvl clear
    # "\"Mm, Kitsuyoshi, what are you going to do with this?{w} If we're to attract customers, then we'll have to use one style.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1881
translate russian s17_2fd64efb:

    # " Tsuneiwa-kun, who seems to have the most artistic talent of the group, speaks."
    ""

# game/scenario/sc_en.rpy:1882
translate russian s17_a3b82ec2:

    # " And to think that he's in the same Judo club as Kitsuyoshi|"
    ""

# game/scenario/sc_en.rpy:1883
translate russian s17_2031e879:

    # nvl clear
    # "\"Ah| I wouldn't know much about it."
    nvl clear
    ""

# game/scenario/sc_en.rpy:1885
translate russian s17_b920770b:

    # " Oh! That's nice.{w} Tohno, what do you think?\""
    ""

# game/scenario/sc_en.rpy:1886
translate russian s17_38370853:

    # "\"Hmmm, if it's Shiki-kun, then I'll trust him.{w} So, how do you want us to end this?{w} Maids?{w} Or aprons?\""
    ""

# game/scenario/sc_en.rpy:1887
translate russian s17_b2477f67:

    # "\"Uh| why are you asking such a pointed question?\""
    ""

# game/scenario/sc_en.rpy:1888
translate russian s17_b96a9d2a:

    # " Tsunewai-kun just has an expression that seems to say 'I wonder why that is.'"
    ""

# game/scenario/sc_en.rpy:1889
translate russian s17_9f9a04d8:

    # nvl clear
    # "\"Well, it doesn't matter much, but if you ask, I think the apron's better.{w} It has a more| settled feeling.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1891
translate russian s17_d0898092:

    # "\"Haha| so it's true that you like the Japanese style.\""
    ""

# game/scenario/sc_en.rpy:1892
translate russian s17_251cd7dc:

    # " Kitsunewai-kun happily moves his brush up and down.{w} The drawings are left up to him, and we decide to do the letters."
    ""

# game/scenario/sc_en.rpy:1893
translate russian s17_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1896
translate russian s17_5beaaefd:

    # " Well, I like maid uniforms, too|"
    ""

# game/scenario/sc_en.rpy:1897
translate russian s17_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1912
translate russian s19_ac706c86:

    # "This scene is \"School - Morning: Courtyard - Shiki and the black cat.\"{nw}"
    ""

# game/scenario/sc_en.rpy:1913
translate russian s19_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:1914
translate russian s19_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:1932
translate russian s19_0f737bc9:

    # " I'll stop by the courtyard."
    ""

# game/scenario/sc_en.rpy:1933
translate russian s19_cbaa4b95:

    # " I still have time, and the fresh air there will be refreshing."
    ""

# game/scenario/sc_en.rpy:1934
translate russian s19_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1941
translate russian s19_0622658c:

    # " As expected, the air in the courtyard is fantastic."
    ""

# game/scenario/sc_en.rpy:1942
translate russian s19_9afb0483:

    # " Most students go straight into the main building from the gate, so the inner courtyard is empty most of the time."
    ""

# game/scenario/sc_en.rpy:1943
translate russian s19_e27f52b4:

    # " Space of solitude."
    ""

# game/scenario/sc_en.rpy:1944
translate russian s19_5125db20:

    # " When I'm there I can enjoy a moment of peace and watch the students going to school, like a parent."
    ""

# game/scenario/sc_en.rpy:1945
translate russian s19_b53802d7:

    # nvl clear
    # " While I'm thinking such unnecessary thoughts something comes out of the grass nearby."
    nvl clear
    ""

# game/scenario/sc_en.rpy:1947
translate russian s19_6ac3266c:

    # "\"Hmm?\""
    ""

# game/scenario/sc_en.rpy:1950
translate russian s19_37582c74:

    # "\"A cat|{w} Hey, maybe you're thinking like me.{w} Let's enjoy this together, then.\""
    ""

# game/scenario/sc_en.rpy:1951
translate russian s19_b4a927e5:

    # " I motion to the cat with my fingers."
    ""

# game/scenario/sc_en.rpy:1960
translate russian s19_e03835f0:

    # " As if disinterested, the black cat turns its head and curls up."
    ""

# game/scenario/sc_en.rpy:1961
translate russian s19_eb233a72:

    # nvl clear
    # "\"Mm, you're picky, aren't you?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1963
translate russian s19_deb02d40:

    # " Well, I guess cats are like that."
    ""

# game/scenario/sc_en.rpy:1964
translate russian s19_62907e3f:

    # " Begging and pleading from this end will only drive away the cat.{w} So, regrettably, I can't do anything."
    ""

# game/scenario/sc_en.rpy:1965
translate russian s19_cad2b2f9:

    # "\"Well, then|{w} I'll just be patient.\""
    ""

# game/scenario/sc_en.rpy:1966
translate russian s19_6c8698ff:

    # " I sit on the bench and blankly stare at the clock."
    ""

# game/scenario/sc_en.rpy:1967
translate russian s19_773d9350:

    # " It's not good to force something.{w} I should be happy that a cat who acts like a high-class lady is with me this morning in the inner courtyard.{w} I should be punished for wanting anything more than that."
    ""

# game/scenario/sc_en.rpy:1968
translate russian s19_582b9d96:

    # nvl clear
    # " The black cat seems disinterested, but still looks at me from its serene position on the road without any dislike."
    nvl clear
    ""

# game/scenario/sc_en.rpy:1970
translate russian s19_440999cb:

    # " Hmm|"
    ""

# game/scenario/sc_en.rpy:1971
translate russian s19_3db505ee:

    # " Looking at this scene alone creates a peace of mind."
    ""

# game/scenario/sc_en.rpy:1972
translate russian s19_da1fd53c:

    # nvl clear
    # "\"Hmm| you're from a classy family, eh?{w} You have an aura of calmness and softness or something.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:1974
translate russian s19_87214396:

    # " Uuuh|{w} I want to pet it!"
    ""

# game/scenario/sc_en.rpy:1975
translate russian s19_495da8f4:

    # " I want to pet it| but I shouldn't."
    ""

# game/scenario/sc_en.rpy:1976
translate russian s19_a5dc8245:

    # " It wouldn't be good to ruin this moment of peace we're both enjoying."
    ""

# game/scenario/sc_en.rpy:1977
translate russian s19_d16e0c28:

    # " I watch the cat on the road."
    ""

# game/scenario/sc_en.rpy:1978
translate russian s19_ea778aa8:

    # " The cat watches me on the bench."
    ""

# game/scenario/sc_en.rpy:1979
translate russian s19_a84f703e:

    # " That's more than enough."
    ""

# game/scenario/sc_en.rpy:1980
translate russian s19_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:1984
translate russian s19_7d0f8bab:

    # "\"Hmm| it's time already?\""
    ""

# game/scenario/sc_en.rpy:1985
translate russian s19_ea2342ef:

    # " The black cat seems surprised at the school bell.{w} It looks at me once and disappears into the grass."
    ""

# game/scenario/sc_en.rpy:1986
translate russian s19_95cc77c9:

    # "\"Well, then I'll go back to my nest, too.\""
    ""

# game/scenario/sc_en.rpy:1987
translate russian s19_5049dea8:

    # " I hurriedly stand up from the bench."
    ""

# game/scenario/sc_en.rpy:1988
translate russian s19_b9abd4f8:

    # " My nest| would be Class 2-3's classroom, full of my classmates."
    ""

# game/scenario/sc_en.rpy:1989
translate russian s19_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2006
translate russian s20_37f6dc24:

    # "This scene is \"School - Morning: Inner Courtyard - No one.\"{nw}"
    "Эта сцена - \"Школа - Утро: Внутренний двор - Никого.\"{nw}"

# game/scenario/sc_en.rpy:2007
translate russian s20_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:2008
translate russian s20_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2026
translate russian s20_9202b90e:

    # " I'll drop by the courtyard."
    " \[Заскочу\] во внутренний двор."

# game/scenario/sc_en.rpy:2027
translate russian s20_a1587a7e:

    # " Maybe I'll meet that child again."
    " Может быть я снова встречу того ребёнка."

# game/scenario/sc_en.rpy:2028
translate russian s20_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2035
translate russian s20_5fbf0c0c:

    # " As expected, the air of the courtyard is refreshing."
    " Как и ожидалось, воздух двора освежает."

# game/scenario/sc_en.rpy:2036
translate russian s20_3e026f24:

    # " One might call it morning tranquility."
    " Можно было бы назвать это утренним спокойствием."

# game/scenario/sc_en.rpy:2037
translate russian s20_9962f314:

    # " Since most students go straight to the main building from the gate, there's usually no one in the inner courtyard."
    " Поскольку большая часть учеников ходят к главному зданию через ворота, обычно во внутреннем дворе никого нет."

# game/scenario/sc_en.rpy:2038
translate russian s20_e27f52b4:

    # " Space of solitude."
    " Одинокое пространство."

# game/scenario/sc_en.rpy:2039
translate russian s20_b41dee5b:

    # " The courtyard looks as serene as usual."
    " Внутренний двор выглядит таким же безмятежным, как обычно."

# game/scenario/sc_en.rpy:2040
translate russian s20_b0de56c9:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:2041
translate russian s20_1d831a9f:

    # " But it lacks the presence of the child."
    " Но здесь не хватает того ребёнка."

# game/scenario/sc_en.rpy:2042
translate russian s20_a1f06034:

    # nvl clear
    # "\"What am I doing here|\""
    nvl clear
    "\"Что я здесь делаю?\""

# game/scenario/sc_en.rpy:2044
translate russian s20_415f4b2a:

    # " I shake my head at my own worthless action."
    " \[Я качаю головой над своим собственным никчемным поступком\]."

# game/scenario/sc_en.rpy:2045
translate russian s20_266ed642:

    # " I can't help the child by coming to this place."
    " Я не смогу помочь ребёнку, придя \[сюда\]."

# game/scenario/sc_en.rpy:2046
translate russian s20_68f9a09c:

    # " That place."
    " \[Сюда\]."

# game/scenario/sc_en.rpy:2047
translate russian s20_e4bdd856:

    # " If I don't meet that child in the dark forest, it won't do."
    " \[Если я не встречу этого ребенка в темном лесу, ничего не выйдет\]."

# game/scenario/sc_en.rpy:2048
translate russian s20_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2065
translate russian s22_5dc08a50:

    # "This scene is \"School - Lunch time: Choices.\"{nw}"
    "Эта сцена - \"Школа - Обед: Выборы.\"{nw}"

# game/scenario/sc_en.rpy:2066
translate russian s22_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:2067
translate russian s22_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2085
translate russian s22_51ba409b:

    # " Homeroom starts."
    " Начинается классный час."

# game/scenario/sc_en.rpy:2086
translate russian s22_fad1f7b9:

    # " Mr. Kujihuni stands on the podium.{w} After the roll call, the announcements of the day are made."
    " Мистер Куджихуни стоит на подиуме.{w} После переклички делаются дневные объявления."

# game/scenario/sc_en.rpy:2087
translate russian s22_a04c2aac:

    # " Nothing special."
    " Ничего необычного."

# game/scenario/sc_en.rpy:2088
translate russian s22_76d5e991:

    # " The tranquil day today starts as usual."
    " Сегодняшний безмятежный день начался как обычно."

# game/scenario/sc_en.rpy:2089
translate russian s22_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2099
translate russian s22_8ac0521e:

    # " It's lunchtime."
    " Время обедать."

# game/scenario/sc_en.rpy:2100
translate russian s22_c48ed8f7:

    # " The classroom gets loud, and most students head off to the cafeteria."
    " В классе станомится шумно, и большиство учеников уходят в столовую."

# game/scenario/sc_en.rpy:2104
translate russian s22_1061e728:

    # "\"Tohno, where will you have lunch today?\""
    "\"Тоно, где ты будешь сегодня обедать?\""

# game/scenario/sc_en.rpy:2105
translate russian s22_9f5c48ec:

    # " The hungry Arihiko asks."
    " Спрашивает голодный Арихико."

# game/scenario/sc_en.rpy:2106
translate russian s22_4668e17e:

    # "\"Hmm| what should I do|{w} What are you going to do?\""
    "\"Хмм, \[что мне делать\]?{w} А куда ты собираешься?\""

# game/scenario/sc_en.rpy:2107
translate russian s22_7ab2c2a2:

    # "\"Me?{w} I'm going to have some bread.{w} If you're going to the cafeteria, I'll have to bid goodbye.\""
    "\"Я?{w} Я собираюсь поесть булочек.{w} Если ты пойдёшь в столовую, мне придётся попрощаться."

# game/scenario/sc_en.rpy:2108
translate russian s22_b72ee415:

    # "\"Oh?{w} I was actually thinking of going there|\""
    "\"О?{w} На самом деле я хотел пойти именно туда."

# game/scenario/sc_en.rpy:2109
translate russian s22_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2117
translate russian s22_87e51704:

    # " Unfortunately, the food in the cafeteria is expensive.{w} If I'm to use my lunch money of five-hundred yen wisely, I'll have to buy some bread and eat in the courtyard, or leech off Senpai in the tea room|"
    " К сожалению, еда в столовой дорогая.{w} Если я хочу мудро потратить свои пятьсот йен на обед, я лучше куплю немного булок и поем во дворе, или подсяду к Семпай в чайной комнате."

# game/scenario/sc_en.rpy:2118
translate russian s22_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2122
translate russian s22_3b0c293d:

    # "\"Ha!{w} What ridiculous thoughts are you having, Tohno?{w} Really| the man who saved a hundred thousand yen from elementary school is a miser.\""
    "\"А!{w} Что за нелепые мысли приходят тебе в голову, Тоно?{w} Серьёзно, человек, который сэкономил сто тысяч иен на начальной школе, - скряга.\""

# game/scenario/sc_en.rpy:2131
translate russian s22_b0de56c9:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:2132
translate russian s22_1c53a2bd:

    # " I get angry for some reason.{w} Senpai or the courtyard are fine, but today I want to have some donkatsu rice at the cafeteria."
    " По какой-то причине я начинаю злиться.{w} Семпай или двор прекрасный выбор, но сегодня я хочу поесть рис тонкацу в столовой."

# game/scenario/sc_en.rpy:2133
translate russian s22_fcef5b57:

    # " Damn it|{w} I'll just|{nw}"
    " Чёрт!{w} Я просто{nw}"

# game/scenario/sc_en.rpy:2134
translate russian s22_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2148
translate russian s24_d13bb33d:

    # "This scene is \"School - Lunch time: Tea room - Scene with Arcueid.\"{nw}"
    "Эта сцена - \"Школа - Обед: Чайная комната - Сцена с Арквейд.\"{nw}"

# game/scenario/sc_en.rpy:2149
translate russian s24_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:2150
translate russian s24_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2168
translate russian s24_819d8a63:

    # " I'll just go to the Tea Ceremony Club room and steal from Senpai."
    " Я просто пойду в комнату Чайного кружка и украду что-нибудь у Семпай."

# game/scenario/sc_en.rpy:2169
translate russian s24_205eba0a:

    # " I half-fill my stomach with two snack breads from the store.{w} The rest will be up to Senpai.{w} Just like exploiting a loophole."
    " Я уже наполовину набил желудок двумя булками из магазина.{w} Остальное будет зависеть от Семпай.{w} Это как \[использовать лазейку.\]"

# game/scenario/sc_en.rpy:2170
translate russian s24_df290ae2:

    # nvl clear
    # "\"All right.{w} I'll go to the tea room and drink some tea.\""
    nvl clear
    "\"Ладно.{w} Пойду в чайную комнату и попью чаю.\""

# game/scenario/sc_en.rpy:2175
translate russian s24_cfbf0e2d:

    # "\"Well, then|{w} I'll just eat my bread|{w} alone and lonely|\""
    "\"Ну, тогда,{w} я просто съем свою булочку,{w} в одного и в одиночестве.\""

# game/scenario/sc_en.rpy:2183
translate russian s24_2b45ff7c:

    # "\"|?\""
    "\"?\""

# game/scenario/sc_en.rpy:2184
translate russian s24_ac055c88:

    # " Arihiko rushes out, as if thinking that now's the chance."
    " Арихико выбегает, словно думая, что сейчас самый подходящий момент."

# game/scenario/sc_en.rpy:2185
translate russian s24_3ffc284c:

    # "\"What's up with him|{w} I thought he'd come with me.\""
    "\"Что это с ним?{w} Я думал он пойдёт со мной.\""

# game/scenario/sc_en.rpy:2186
translate russian s24_0053e86b:

    # " Well, I guess I won't have to share anything with him.{w} I'll go to the tea room quickly before he changes his mind."
    " Что ж, думаю мне не придётся с ним ничем делиться.{w} Пойду ка в Чайную комнату побыстрее, пока он не передумал."

# game/scenario/sc_en.rpy:2187
translate russian s24_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2194
translate russian s24_086bafb0:

    # "\"Eh?{w} It's| locked.\""
    "\"Э?{w} Закрыто.\""

# game/scenario/sc_en.rpy:2195
translate russian s24_ab215661:

    # " Locked|{w} Ciel-senpai isn't here yet|"
    " Закрыто,{w} Сиель-семпай ещё нет."

# game/scenario/sc_en.rpy:2196
translate russian s24_266d3f79:

    # " The Tea Ceremony Club is just a facade that Ciel-senpai made up to fool the school, so she's the only member.{w} So the only person to have the key to the room would be Senpai as well."
    " Комната Чайного кружка просто прикрытие, которое сделала Сиель-семпай, чтобы обмануть школу, поэтому она единственный его участник.{w} Так что, единственным человеком, у которого был бы ключ от комнаты, тоже была бы Сэмпай."

# game/scenario/sc_en.rpy:2197
translate russian s24_d5eb4744:

    # "\"Strange| she always eats lunch in the tea room.\""
    "\"Странно, она всегда обедает в чайной комнате.\""

# game/scenario/sc_en.rpy:2198
translate russian s24_242884ae:

    # " Maybe her plans changed."
    " Может быть её планы изменились."

# game/scenario/sc_en.rpy:2199
translate russian s24_a28863a0:

    # nvl clear
    # "{fast}{nw}"
    nvl clear
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2201
translate russian s24_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2202
translate russian s24_0af6a5fa_2:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2203
translate russian s24_0af6a5fa_3:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2204
translate russian s24_0af6a5fa_4:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2205
translate russian s24_0af6a5fa_5:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2206
translate russian s24_0af6a5fa_6:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2207
translate russian s24_0af6a5fa_7:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2208
translate russian s24_0af6a5fa_8:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2209
translate russian s24_0af6a5fa_9:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2210
translate russian s24_0af6a5fa_10:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2211
translate russian s24_0af6a5fa_11:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2212
translate russian s24_0af6a5fa_12:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2213
translate russian s24_8c92c558:

    # "------Rattle, rattle."
    "------Грохот."

# game/scenario/sc_en.rpy:2214
translate russian s24_c3618ed3:

    # nvl clear
    # "\"Eh? I think I heard something.\""
    nvl clear
    "\"Э? Кажется я что-то слышал.\""

# game/scenario/sc_en.rpy:2216
translate russian s24_a28863a0_1:

    # nvl clear
    # "{fast}{nw}"
    nvl clear
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2218
translate russian s24_0af6a5fa_13:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2219
translate russian s24_0af6a5fa_14:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2220
translate russian s24_0af6a5fa_15:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2221
translate russian s24_0af6a5fa_16:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2222
translate russian s24_0af6a5fa_17:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2223
translate russian s24_0af6a5fa_18:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2224
translate russian s24_0af6a5fa_19:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2225
translate russian s24_0af6a5fa_20:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2226
translate russian s24_0af6a5fa_21:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2227
translate russian s24_0af6a5fa_22:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2228
translate russian s24_0af6a5fa_23:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2229
translate russian s24_0af6a5fa_24:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2230
translate russian s24_7870fff7:

    # "------Some more noises."
    "------Ещё какие-то звуки."

# game/scenario/sc_en.rpy:2231
translate russian s24_229f2ecc:

    # nvl clear
    # "\"Is it just some people enjoying lunch?\""
    nvl clear
    "\"Это просто кто-то наслаждается обедом?\""

# game/scenario/sc_en.rpy:2233
translate russian s24_ad8563f5:

    # " There's a lot of noise coming from the school building.{w} And I could distinctly hear one of the noises."
    " Из здания школы доносится сильный шум.{w} И я отчетливо услышал один из этих звуков."

# game/scenario/sc_en.rpy:2234
translate russian s24_e4410dc7:

    # "\"Senpai doesn't seem to be here|{w} I'll go to the cafeteria.\""
    "\"Не похоже, что семпай здесь.{w} Я пойду в столовую.\""

# game/scenario/sc_en.rpy:2235
translate russian s24_1a620b04:

    # " I turn my back to the tea room with the two breads I bought from the store."
    " Я отвернулся от чайной комнаты с двумя булками, которые я купил в магазине."

# game/scenario/sc_en.rpy:2236
translate russian s24_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2252
translate russian s25_ea84bb28:

    # "This scene is \"School - Lunch time: Tea room - Scene with Ciel-senpai.\"{nw}"
    "Эта сцена - \"Школа - Обед: Чайная комната - Сцена с Сиель-семпай.\"{nw}"

# game/scenario/sc_en.rpy:2253
translate russian s25_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:2254
translate russian s25_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2270
translate russian s25_71149ad6:

    # " I'll just go steal some food from Ciel-senpai in the Tea Ceremony Club room.{nw}"
    " Я просто украду немного еды у Сиель-семпай в комнате Чайного кружка.{nw}"

# game/scenario/sc_en.rpy:2271
translate russian s25_205eba0a:

    # " I half-fill my stomach with two snack breads from the store.{w} The rest will be up to Senpai.{w} Just like exploiting a loophole."
    " Я уже наполовину набил свой желудок двумя \[булками\] из магазина.{w} Остальное за Семпай.{w} Это как использовать \[лазейку\]."

# game/scenario/sc_en.rpy:2272
translate russian s25_ae618235:

    # nvl clear
    # "\"I've decided.{w} I'll go to the tea room and drink some tea.\""
    nvl clear
    "\"Я решил.{w} Я пойду в чайную комнату и попью чаю.\""

# game/scenario/sc_en.rpy:2277
translate russian s25_8d9e1e89:

    # "\"Well|{w} I guess I'll just eat some bread|{w} alone and lonely|\""
    "\"Ладно,{w} тогда я поем булочек,{w} в одного и в одиночестве.\""

# game/scenario/sc_en.rpy:2285
translate russian s25_2b45ff7c:

    # "\"|?\""
    "\"?\""

# game/scenario/sc_en.rpy:2286
translate russian s25_ac055c88:

    # " Arihiko rushes out, as if thinking that now's the chance."
    " Акихико выбегает, словно думая, что сейчас самый подходящий момент."

# game/scenario/sc_en.rpy:2287
translate russian s25_3ffc284c:

    # "\"What's up with him|{w} I thought he'd come with me.\""
    "\"Что это с ним?{w} Я думал он пойдёт со мной.\""

# game/scenario/sc_en.rpy:2288
translate russian s25_0053e86b:

    # " Well, I guess I won't have to share anything with him.{w} I'll go to the tea room quickly before he changes his mind."
    " Что ж, думаю мне не придётся ничем с ним делиться.{w} Пойду ка в Чайную комнату побыстрее, пока он не передумал."

# game/scenario/sc_en.rpy:2289
translate russian s25_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2296
translate russian s25_d9b85d72:

    # "\"Ah, Senpai!{w} You're already here!\""
    "\"А, Семпай!{w} Ты уже здесь!\""

# game/scenario/sc_en.rpy:2297
translate russian s25_3eeb59b9:

    # " The Tea Ceremony Club room is open.{w} Since the only Tea Ceremony Club member is Senpai, she has to be the one inside."
    " Комната Чайного кружка открыта.{w} Поскольку единственным членом кружка является Сэмпай, она должна быть внутри."

# game/scenario/sc_en.rpy:2298
translate russian s25_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2303
translate russian s25_74505627:

    # "\"Cha!\""
    "\"\[Чай!\]\""

# game/scenario/sc_en.rpy:2304
translate russian s25_6344e241:

    # " I announce my presence as I go in."
    " Объявил я о своём присутствии, когда вошёл."

# game/scenario/sc_en.rpy:2308
translate russian s25_24d05162:

    # "\"Ah!{w} Tohno-kun!{w} How very sudden of you to come!\""
    "\"А!{w} Тоно-кун!{w} Как неожиданно ты пришёл!\""

# game/scenario/sc_en.rpy:2309
translate russian s25_8d6aea84:

    # "\"Sudden?{w} I just thought maybe we could eat lunch together.\""
    "\"Неожиданно?{w} Я просто подумал может быть мы пообедаем вместе.\""

# game/scenario/sc_en.rpy:2312
translate russian s25_679d2759:

    # "\"Ah|{w} I see.{w} I was going to eat right now, actually.\""
    "\"А,{w} вижу.{w} Вообще я собиралась поесть прямо сейчас.\""

# game/scenario/sc_en.rpy:2313
translate russian s25_346bdd58:

    # " Senpai stretches as if tired."
    " Сэмпай потягивается, как будто устала."

# game/scenario/sc_en.rpy:2314
translate russian s25_87cb1175:

    # nvl clear
    # "\"Senpai| did something happen?\""
    nvl clear
    "\"Семпай, что-то случилось?\""

# game/scenario/sc_en.rpy:2318
translate russian s25_2ef67b23:

    # "\"N, no!{w} There is absolutely nothing wrong.{w} Let's just eat lunch!\""
    "\"Н, нет!{w} Абсолютно ничего плохого.{w} Просто давай пообедаем!\""

# game/scenario/sc_en.rpy:2319
translate russian s25_1fde8c2f:

    # " What's with this definite suspiciousness|"
    " Что-то это подозрительно."

# game/scenario/sc_en.rpy:2320
translate russian s25_b3cef979:

    # " If Ciel-senpai, who's usually calm, is disturbed like that, then it must be something really big."
    " Если Сиель-семпай, которая обычно спокойна, так взволнована, значит случилось что-то и правда \[значимое\]."

# game/scenario/sc_en.rpy:2321
translate russian s25_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2324
translate russian s25_7d93b0c5:

    # "\"Tohno-kun, if you don't eat, lunch will be over.{w} I'll give you some of mine, so let's enjoy.\""
    "\"Тоно-кун, если ты не будешь есть, обед закончится.{w} Я дам тебе немного своего, так что наслаждайся.\""

# game/scenario/sc_en.rpy:2325
translate russian s25_35a07402:

    # " She speaks with a loud voice."
    " Говорит она громким голоосм."

# game/scenario/sc_en.rpy:2326
translate russian s25_cadfc0b8:

    # " And I instinctively react."
    " И я инстинктивно реагирую."

# game/scenario/sc_en.rpy:2327
translate russian s25_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2328
translate russian s25_cbbe7baf:

    # "------Rattle, rattle！{nw}"
    "------Грохот！{nw}"

# game/scenario/sc_en.rpy:2329
translate russian s25_0af6a5fa_2:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2330
translate russian s25_bf0ff037:

    # " The noise comes from the closet."
    " Шум доносится из шкафа."

# game/scenario/sc_en.rpy:2331
translate russian s25_99ba69d0:

    # nvl clear
    # "\"Wait, Senpai!\""
    nvl clear
    "\"Подожди, Семпай!\""

# game/scenario/sc_en.rpy:2335
translate russian s25_9d3c29d6:

    # "\"A rat?{w} Or maybe a big cat?{w} Well, either way, it doesn't concern us, so let's keep the lock on the closet locked.\""
    "\"Крыса?{w} Или может большая кошка?{w} Что ж, в любом случае, нас это не касается, так что давай оставим шкаф закрытым.\""

# game/scenario/sc_en.rpy:2336
translate russian s25_391a02a4:

    # " And with that she forcibly sits me down."
    " С этими словами она силой посадила меня обратно."

# game/scenario/sc_en.rpy:2337
translate russian s25_0af6a5fa_3:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2338
translate russian s25_74c2ff03:

    # " Rattle, rattle."
    "Грохот."

# game/scenario/sc_en.rpy:2339
translate russian s25_0af6a5fa_4:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2340
translate russian s25_3bba4b0d:

    # " The noise from the cabinet gets louder."
    " Шум из шкафа становится громче."

# game/scenario/sc_en.rpy:2341
translate russian s25_05ac3171:

    # " Now that I look closer, Senpai has many scratches on her arm, and one is beautifully laid across the tatami mats."
    " Теперь, когда я пригляделся повнимательнее, у Семпай на руке, которая грациозно лежит на татами, много царапин."

# game/scenario/sc_en.rpy:2342
translate russian s25_64d1bf82:

    # nvl clear
    # "\"Senpai, did Arcueid perhaps co-\""
    nvl clear
    "\"Семпай, может это Аркв-\""

# game/scenario/sc_en.rpy:2346
translate russian s25_1bdac25b:

    # " Uuuh|{w} I get the feeling that if I say any more, something unspeakable will be done to me."
    " Ууух,{w} у меня такое чувство, что если я скажу еще что-нибудь, со мной сделают что-то невыразимое."

# game/scenario/sc_en.rpy:2347
translate russian s25_a3335eb9:

    # "\"N, no| never mind.{w} Let's just enjoy our meal.\""
    "\"Н, нет, забудь.{w} Давай просто насладжаться едой.\""

# game/scenario/sc_en.rpy:2350
translate russian s25_f7373c50:

    # "\"Yes.{w} Today, the tempura came out well.{w} And there's definitely nothing in the closet, so pay absolutely no attention.{w} If you do, I'll put you in there or something.\""
    "\"Да.{w} Сегодня темпура получилась вкусной.{w} И в шкафу точно ничего нет, так что не обращай абсолютно никакого внимания.{w} Если ты это сделаешь, я посажу тебя туда или сделаю ещё чего-нибудь.\""

# game/scenario/sc_en.rpy:2351
translate russian s25_f55be6be:

    # " With that, she opens her lunch box in a friendly way and offers me some."
    " С этими словами она дружелюбно открывает свой ланч-бокс и предлагает немного мне."

# game/scenario/sc_en.rpy:2352
translate russian s25_396757c3:

    # nvl clear
    # "\"Haa| if I do get in there, how long will it be before I'm let out?\""
    nvl clear
    "\"Хаа, если я все-таки попаду туда, сколько времени пройдет, прежде чем меня выпустят?\""

# game/scenario/sc_en.rpy:2354
translate russian s25_c9e22ed8:

    # "\"Of course, until I feel like it.\""
    "\"Конечно же, пока я не захочу.\""

# game/scenario/sc_en.rpy:2355
translate russian s25_5424f6b4:

    # " She smiles."
    " Улыбнулась она."

# game/scenario/sc_en.rpy:2356
translate russian s25_684944db:

    # " Haa|{w} I don't know why Arcueid is locked up, but maybe she'll never be let out|"
    " Хаа,{w} не знаю почему Арквейд заперта, но может быть её никогда и не выпустят."

# game/scenario/sc_en.rpy:2357
translate russian s25_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2374
translate russian s26_f455c37a:

    # "This scene is \"School - Lunch time: Ominous Krokakaunta! Clash of Dehurikashiel!\"{nw}"
    "Эта сцена - \"Школа - Обед: \[Зловещая Крокакаунта! Битва при Дехурикашиэле\]!\"{nw}"

# game/scenario/sc_en.rpy:2375
translate russian s26_5a857b50:

    # "Would you like to skip?{nw}"
    "Хотите пропустить?{nw}"

# game/scenario/sc_en.rpy:2376
translate russian s26_0af6a5fa:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2394
translate russian s26_21f96c88:

    # " I'll just go to the tea room and steal from Senpai."
    " Я просто пойду в чайную и украду немного у Семпай."

# game/scenario/sc_en.rpy:2395
translate russian s26_205eba0a:

    # " I half-fill my stomach with two snack breads from the store.{w} The rest will be up to Senpai.{w} Just like exploiting a loophole."
    " Я уже наполовину набил свой желудок булочками из магазина.{w} Остальное зависит от Семпай.{w} Это как использовать \[лазейку\]."

# game/scenario/sc_en.rpy:2396
translate russian s26_602d1624:

    # nvl clear
    # "\"I've decided.{w} I'll go to the Tea Ceremony Club room.\""
    nvl clear
    "\"Я решил.{w} Я пойду в комнату Чайного клуба.\""

# game/scenario/sc_en.rpy:2401
translate russian s26_45606533:

    # "\"Well, then|{w} I guess I'll eat my bread|{w} alone and lonely|\""
    "\"Ладно, тогда,{w} думаю я поем булок,{w} в одного и в одиночестве.\""

# game/scenario/sc_en.rpy:2409
translate russian s26_2b45ff7c:

    # "\"|?\""
    "\"?\""

# game/scenario/sc_en.rpy:2410
translate russian s26_ac055c88:

    # " Arihiko rushes out, as if thinking that now's the chance."
    " Акихико выбежал, словно думая, что сейчас самый подходящий момент."

# game/scenario/sc_en.rpy:2411
translate russian s26_3ffc284c:

    # "\"What's up with him|{w} I thought he'd come with me.\""
    "\"Что это с ним?{w} Я думал он пойдёт со мной.\""

# game/scenario/sc_en.rpy:2412
translate russian s26_0053e86b:

    # " Well, I guess I won't have to share anything with him.{w} I'll go to the tea room quickly before he changes his mind."
    " Чтож, думаю мне не придётся ничем с ним делиться.{w} Пойду в чайную побыстрее, пока он не передумал."

# game/scenario/sc_en.rpy:2413
translate russian s26_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2420
translate russian s26_fd9f59e7:

    # "\"Ah, Senpai, you're already here.\""
    "\"А, Семпай, ты уже здесь.\""

# game/scenario/sc_en.rpy:2421
translate russian s26_3a6ef448:

    # " The tea room is open.{w} Senpai is the only member of the Tea Ceremony Club.{w} And since the room is unlocked, she has to be inside."
    " Чайная открыта.{w} Семпай единственный участник Чайного кружка.{w} А раз комната открыта, "

# game/scenario/sc_en.rpy:2422
translate russian s26_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2427
translate russian s26_454dc8a6:

    # "\"Hello.\""
    "\"Привет.\""

# game/scenario/sc_en.rpy:2428
translate russian s26_122dd2a4:

    # " I go in with a greeting."
    " Вошёл я с приветствием."

# game/scenario/sc_en.rpy:2432
translate russian s26_59080d98:

    # "\"Welcome, Tohno-kun.{w} Are you eating lunch here today?\""
    "\"Привет, Тоно-кун.{w} Сегодня ты обедаешь здесь?\""

# game/scenario/sc_en.rpy:2433
translate russian s26_519d4e6e:

    # "\"Oui.{w} I wanted to have some tea and share some lunch, too.\""
    "\"Да.{w} Я хотел попить чаю и пообедать вместе с тобой.\""

# game/scenario/sc_en.rpy:2434
translate russian s26_05af56df:

    # " With that I whip out the bread I bought in the store."
    " С этими словами я достал булочки, которые я купил в магазине."

# game/scenario/sc_en.rpy:2435
translate russian s26_8ff47dc3:

    # " Sausage bread and curry bread.{w} The sausage bread will be mine, and the curry bread is for material exchange."
    " Сосиска в тесте и булочка с карри.{w} Сосиска в тесте будет моя, а булочку с карри я обменяю."

# game/scenario/sc_en.rpy:2436
translate russian s26_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2440
translate russian s26_b0c32009:

    # "\"Ah!{w} It's curry bread from the store in front of the office.\""
    "\"А!{w} Это булочка с карри из магазина перед \[офисом\].\""

# game/scenario/sc_en.rpy:2441
translate russian s26_6357a76a:

    # "\"Oui.{w} I thought you might be sick of the curry bread from the store in front of the cafeteria.\""
    "\"Да.{w} Я подумал, что тебя уже тошнит от булочек с карри из магазина у кафетерия.\""

# game/scenario/sc_en.rpy:2444
translate russian s26_6f1f6427:

    # "\"Haa, Tohno-kun, you think deeply!{w} But I never get sick of curry bread, so you don't have to worry about such a thing.\""
    "\"Хаа, Тоно-кун, ты глубоко подумал!{w} Но меня никогда не тощшнило от хлеба с карри, так что не волнуйся о таких вещах.\""

# game/scenario/sc_en.rpy:2445
translate russian s26_fc00bc57:

    # "\"Oui.{w}I kinda thought of that, too|\""
    "\"Да.{w} Я вроде как тоже думал об этом.\""

# game/scenario/sc_en.rpy:2448
translate russian s26_a0e861fd:

    # "\"Okay, I'll bring some tea, so please wait here.\""
    "\"Ладно, я принесу чая, так что подожди здесь.\""

# game/scenario/sc_en.rpy:2456
translate russian s26_465f471a:

    # " Senpai energetically goes to make tea."
    " Семпай энергично пошла делать чай."

# game/scenario/sc_en.rpy:2457
translate russian s26_0b109e21:

    # " I pull out some cushions, open the window to let some fresh air in, and sit down."
    " Я поставил подушек, открыл окно, чтобы впустить немного свежего воздуха, и сел."

# game/scenario/sc_en.rpy:2458
translate russian s26_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2463
translate russian s26_6dfd78a4:

    # " So lunch starts as usual."
    " Чтож, обед начался как обычно."

# game/scenario/sc_en.rpy:2464
translate russian s26_4b591fae:

    # " Senpai's lunchbox is jumbo-sized today, too.{w} Even if she were to offer me a hamburger or tempura, it'd be enough to feed one whole person."
    " Ланчбокс Семпай сегодня тоже огромен.{w} Даже если бы она предложила мне гамбургер или темпуру, этого было бы достаточно, чтобы накормить целого человека."

# game/scenario/sc_en.rpy:2465
translate russian s26_123d72dd:

    # " And then after all that, the curry bread|{w} How does she fit all that in?"
    " И после всего этого хлеб с карри.{w} Как она вмещает всё в себя?"

# game/scenario/sc_en.rpy:2469
translate russian s26_a6390ab4:

    # "\"It's the autumn of appetite|\""
    "\"Это осенний аппетит.\""

# game/scenario/sc_en.rpy:2470
translate russian s26_e2e83ba2:

    # " Haa, she sighs and eats."
    " Хаа, она вздыхает и начинает есть."

# game/scenario/sc_en.rpy:2471
translate russian s26_96e88e42:

    # " | I don't think the season has anything to do with a person's appetite, but I'll say nothing here."
    " Я не думаю, что время года имеет какое-либо отношение к аппетиту человека, но здесь я ничего не скажу."

# game/scenario/sc_en.rpy:2472
translate russian s26_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2481
translate russian s26_b2c61ba4:

    # "\"I don't think it has anything to do with it-nya, Fatty Ciel.\""
    "\"Я не думаю, что это имеет к этому какое-то отношение-ня, Жиро-Сиель.\""

# game/scenario/sc_en.rpy:2482
translate russian s26_f99a91c6:

    # "\"|!\""
    "\"!\""

# game/scenario/sc_en.rpy:2483
translate russian s26_3011bc8c:

    # " I almost drop the the tea cup."
    " Я чуть не выронил чашку с чаем."

# game/scenario/sc_en.rpy:2489
translate russian s26_93b8324e:

    # "\"| Did you say something, Tohno-kun?{w} It was soft, so I didn't hear it clearly enough.\""
    "\"Ты что-то сказал, Тоно-кун?{w} Это было тихо, так что я не очень расслышала.\""

# game/scenario/sc_en.rpy:2490
translate russian s26_402865b9:

    # "\"N, no, I didn't say anyth|!\""
    "\"Н, нет, я ничего не говорил!\""

# game/scenario/sc_en.rpy:2491
translate russian s26_433b9ee7:

    # "\"Ah| good, great, but I was wondering what I should do if what I heard were not hallucinations.\""
    "\"А, прекрасно, замечательно, но мне интересно, что мне делать, если бы то, что я услышала, не было галлюцинациями.\""

# game/scenario/sc_en.rpy:2492
translate russian s26_4750a182:

    # " Senpai smiles as usual, but I think I can feel the poisonous intent in her voice."
    " По-обыкновенному улыбнулась Семпай, но думаю в интонации её голоса чувствовалась ядовитость."

# game/scenario/sc_en.rpy:2493
translate russian s26_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2496
translate russian s26_41b39909:

    # "\"Anyway, Tohno-kun, are you done with your lunch?{w} If you're not full, please take some of mine.\""
    "\"В любом случае, Тоно-кун, ты уже закончил с обедом?{w} Если ты не наелся, пожалуйста, возьми немного у меня.\""

# game/scenario/sc_en.rpy:2497
translate russian s26_cdb4922e:

    # "\"Ah, no|{w} I don't want to eat any of yours.{w} I can take maybe a hamburger or a tempura.{w} Senpai| wouldn't you be less full because of me?\""
    "\"А, нет.{w} \[Я не хочу ничего твоего\].{w} Я может быть возьму гамбургер или темпуру.{w} Семпай, не не наешься ли ты из-за меня?\""

# game/scenario/sc_en.rpy:2498
translate russian s26_d81626d3:

    # "\"That's not the case.{w} I did get the curry bread from you.{w} I might leave some, but it will definitely fill me up.\""
    "\"Не в этом случае.{w} Я возьму у тебя булочку с карри.{w} наверное Немного останется, но ей я определённо наемся.\""

# game/scenario/sc_en.rpy:2499
translate russian s26_25df017c:

    # " Hmm| her chopsticks move without resting, but I really shouldn't say anything."
    " Хмм, её палочки двигаются без устали, но я не должен ничего говорить."

# game/scenario/sc_en.rpy:2500
translate russian s26_76b2fe88_6:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2507
translate russian s26_a47437a6:

    # "\"You can't bluff your way through|{w} a big eater like you won't be satisfied with even two persons' worth-nyang.\""
    "\"Ты не сможешь блефом пробить себе дорогу,{w} такой заядлый едок, как ты, не насытится даже едой на двоих-ня.\""

# game/scenario/sc_en.rpy:2509
translate russian s26_0af6a5fa_1:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2510
translate russian s26_1b2f8a5b:

    # "----------------Snap！"
    "----------------Хрусть! "

# game/scenario/sc_en.rpy:2511
translate russian s26_0af6a5fa_2:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2512
translate russian s26_776de6cd:

    # " A fracture appears on the tea cup Senpai is holding."
    " На чашке с чаем, которую держит Семпай появились трещины."

# game/scenario/sc_en.rpy:2515
translate russian s26_2d2834a1:

    # "\"Ahahaha|{w} Tohno-kun| what did you say now?{w} I still couldn't hear, because it was soft.\""
    "\"Ахахаха,{w} Тоно-кун, что ты сейчас сказал?{w} Я всё ещё не расслышала, это было тихо.\""

# game/scenario/sc_en.rpy:2516
translate russian s26_3cea1156:

    # "\"Aaaaa!{w} I didn't say anything!{w} I really didn't!\""
    "\"Ааааа!{w} Я ничего не говорил!{w} Я правда ничего не говорил!\""

# game/scenario/sc_en.rpy:2517
translate russian s26_76b2fe88_7:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2524
translate russian s26_7aec4e11:

    # "\"You eat so well even though it all goes to the butt-nya!{w} At this rate, you'll be approaching sixties in the winter-nya!\""
    "\"Ты так хорошо ешь, хотя все это идет в зад-ня!{w} Такими темпами, ты будешь уже за шесдесят к зиме-ня!\""

# game/scenario/sc_en.rpy:2526
translate russian s26_0af6a5fa_3:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2527
translate russian s26_410caeb0:

    # " Snap, snap, snap."
    " Хрусть, хрусть, хрусть."

# game/scenario/sc_en.rpy:2528
translate russian s26_0af6a5fa_4:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2529
translate russian s26_d6197e65:

    # " With an unfaltering smile, she puts down the tea cup that's trying its best to not fall apart."
    " С неизменной улыбкой она ставит на стол чашку с чаем, которая изо всех сил старается не рассыпаться."

# game/scenario/sc_en.rpy:2530
translate russian s26_ffc92800:

    # nvl clear
    # "\"||||||\""
    nvl clear
    "\"||||||\""

# game/scenario/sc_en.rpy:2532
translate russian s26_6bd4f24d:

    # " She stands up without another word."
    " Встала она без единого слова."

# game/scenario/sc_en.rpy:2533
translate russian s26_3278b4e0:

    # "\"Ah| Senpai?\""
    "\"А, Семпай?\""

# game/scenario/sc_en.rpy:2534
translate russian s26_b0de56c9:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:2535
translate russian s26_d11e5160:

    # " There is no answer."
    " Нет ответа."

# game/scenario/sc_en.rpy:2536
translate russian s26_8349fcb8:

    # " Senpai marches toward me and stands right in front."
    " Семпай направилась ко мне и встала передо мной."

# game/scenario/sc_en.rpy:2540
translate russian s26_a62a06e3:

    # "\"Excuse me!{w} How can you say such horrible things!\""
    "\"Простите!{w} Как ты можешь говорить такие ужасные вещи!\""

# game/scenario/sc_en.rpy:2541
translate russian s26_ed09a8de:

    # " And with a serious face, she monstrously pulls on my cheeks."
    " И с серьезным лицом она чудовищно натягивает мне щеки."

# game/scenario/sc_en.rpy:2543
translate russian s26_735a2dc4:

    # "\"Aaaah!{w} It hurts!{w} It hurts, Senpai!!!\""
    "\"Ааааа!{w} Больно!{w} Больно, Семпай!!!\""

# game/scenario/sc_en.rpy:2544
translate russian s26_da1f68dc:

    # nvl clear
    # "\"How do you know my weight, Tohno-kun?{w} If you don't answer truthfully, bad things will happen!\""
    nvl clear
    "\"Откуда ты узнал про мой вес, Тоно-кун?{w} Если ты не ответишь честно, произойдут страшные вещи!\""

# game/scenario/sc_en.rpy:2546
translate russian s26_5ca190f8:

    # "\"I don't know!{w} I don't know such a thing!\""
    "\"Я не знаю!{w} Я не знаю о таких вещах!\""

# game/scenario/sc_en.rpy:2547
translate russian s26_b9c742c6:

    # " Even though I retaliate with all my might, nothing goes through."
    " Даже несмотря на то, что я сопротивляюсь изо всех сил, \[ничего не проходит.\]"

# game/scenario/sc_en.rpy:2548
translate russian s26_4af4a0ee:

    # "\"You mustn't say a girl's weight so blatantly!{w} What if you get it right by accident?{w} How would the girl feel then?!\""
    "\"Ты не можешь говорит о весе девушки так откровенно!{w} Что если ты угадаешь?{w} Что тогда будет чувствовать девушка?!\""

# game/scenario/sc_en.rpy:2550
translate russian s26_e2f2ab1b:

    # " She puts more spin into her pinch."
    " Она сильнее скрутила мои щёки."

# game/scenario/sc_en.rpy:2551
translate russian s26_352bd949:

    # "\"It hurts!{w} Senpai, it's gonna rip!{w} My mouth is going to|!\""
    "\"Больно!{w} Семпай, оно порвётся!{w} Мой рот сейчас...!\""

# game/scenario/sc_en.rpy:2552
translate russian s26_6b4bc112:

    # nvl clear
    # "\"It's better for a liar's mouth to be ripped.{w} Close to sixty kilograms?{w} What nonsense!{w} A person who speaks such nonsense deserves this.\""
    nvl clear
    "\"Лучше, чтобы рот лжеца порвался.{w} Близка к шестидесяти килограммам?{w} Что за чушь!{w} Человек, который говорит такую чушь, заслуживает этого.\""

# game/scenario/sc_en.rpy:2555
translate russian s26_f730cf61:

    # " !!!!"
    " !!!!"

# game/scenario/sc_en.rpy:2556
translate russian s26_0d4c0de2:

    # " Even more force!"
    " Ещё сильнее!"

# game/scenario/sc_en.rpy:2557
translate russian s26_a28863a0:

    # nvl clear
    # "{fast}{nw}"
    nvl clear
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2559
translate russian s26_aa471458:

    # " There."
    " Там."

# game/scenario/sc_en.rpy:2560
translate russian s26_0af6a5fa_5:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2570
translate russian s26_f1cdbd06:

    # "\"Ahahaha!{w} I can tell with one look-nya.{w} Who's speaking nonsense?{w} Seriously, who's the liar-nyang?\""
    "\"Ахахаха!{w} Могу сказать только одно-ня!.{w} Кто несёт чушь?{w} Серьёзно, кто здесь лжец?\""

# game/scenario/sc_en.rpy:2571
translate russian s26_0af6a5fa_6:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2572
translate russian s26_1aa1e318:

    # " The voice sounds so transparent."
    " Голос звучит так \[прозрачно\]."

# game/scenario/sc_en.rpy:2573
translate russian s26_76b2fe88_8:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2576
translate russian s26_3034ddf4:

    # "\"Tohno-kun|\""
    "\"Тоно-кун!\""

# game/scenario/sc_en.rpy:2577
translate russian s26_9fd9d1e0:

    # " And her fingers stop suddenly."
    " Её пальцы внезапно остановились."

# game/scenario/sc_en.rpy:2578
translate russian s26_07940165:

    # "\"Yes| I heard it, too.\""
    "\"Да, я тоже это слышал.\""

# game/scenario/sc_en.rpy:2579
translate russian s26_2f454d16:

    # "\"Really| the rumor that there's a cat demon in the tea room is true.\""
    "\"Действительно, слух о том, что в чайной комнате обитает кошачий демон, правдив.\""

# game/scenario/sc_en.rpy:2587
translate russian s26_48a2f64c:

    # " Senpai closes her eyes and catches her breath."
    " Семпай закрыла глаза и задержала дыхание."

# game/scenario/sc_en.rpy:2588
translate russian s26_38931638:

    # " And then with a deep breath|{nw}"
    "А затем, глубоко вздохнув...{nw}"

# game/scenario/sc_en.rpy:2589
translate russian s26_0af6a5fa_7:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2590
translate russian s26_8a0fc686:

    # "\"Come out, you millennium-old cat demon vampire!\""
    "\"Выходи, ты тысячелетний кошачий демон вампир!\""

# game/scenario/sc_en.rpy:2592
translate russian s26_0af6a5fa_8:

    # "{fast}{nw}"
    "{fast}{nw}"

# game/scenario/sc_en.rpy:2593
translate russian s26_b95d33bb:

    # " It almost rips my ear drums apart."
    " Это почти разрывает мои барабанные перепонки на части."

# game/scenario/sc_en.rpy:2594
translate russian s26_76b2fe88_9:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2597
translate russian s26_e03de0a5:

    # "\"Ooonya!\""
    "\"Оооня!\""

# game/scenario/sc_en.rpy:2598
translate russian s26_f7a49f09:

    # " A mysterious object lightly lands with a tap."
    " С лёгким касанием приземляется таинственный предмет."

# game/scenario/sc_en.rpy:2599
translate russian s26_76b2fe88_10:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2604
translate russian s26_58910db4:

    # "\"Mmph|{w} I was caught off guard.{w} I didn't think you would use such a traditional method.\""
    "\"Ммпф,{w} меня застали врасплох.{w} Я не думала, что вы будете использовать такой традиционный метод.\""

# game/scenario/sc_en.rpy:2605
translate russian s26_23a28e00:

    # " With a pained expression, Arcueid gets up slowly."
    " Медленно поднялась Арквейд с болезненным видом."

# game/scenario/sc_en.rpy:2609
translate russian s26_8c71232b:

    # "\"What were you thinking?{w} This is a school for humans.{w} It's not a place for someone unnatural, non-standard, ridiculous like you to be around normal people.\""
    "\"О чём ты думаешь?{w} Эта школа для людей.{w} Это не место для кого-то неестественного, нестандартного, вроде тебя, тебе нелепо находиться среди нормальных людей."

# game/scenario/sc_en.rpy:2612
translate russian s26_b161bf05:

    # "\"Uuuh.{w} Hmph!{w} I don't have to hear something like that from you, Ciel!{w} I don't even want to come to this territory of yours.\""
    "\"Ууух.{w} Хмпф!{w} Я не обязана слушать от тебя такое, Сиель!{w} Я даже не хочу заходить на эту вашу территорию.\""

# game/scenario/sc_en.rpy:2613
translate russian s26_76b2fe88_11:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2617
translate russian s26_b0cbc293:

    # "\"Is that so|{w} Then get out without any delay.{w} Of course not just the tea room but the whole school.{w} Actually, leave the town and go back to where you came from.\""
    "\"Это правда?{w} Тогда сейчас же убирайся от сюда.{w} Конечно, не только из чайной комнаты, но и вовсе из школы.{w} А вообще, покинь город и возвращайся туда, откуда пришла.\""

# game/scenario/sc_en.rpy:2620
translate russian s26_349b0318:

    # "\"Heee?{w} You can't even revive now.{w} Can you really say such a thing, my opponent?\""
    "\"Эээ?{w} Сейчас ты даже не пришла в себя.{w} \[Ты действительно говоришь такое, мой оппонент\]?\""

# game/scenario/sc_en.rpy:2621
translate russian s26_727dd9ef:

    # "\"A body that doesn't die| I don't actually need that.\""
    "\"Тело, которое не может умереть не нуждается в этом.\""

# game/scenario/sc_en.rpy:2622
translate russian s26_94324fd5:

    # " The tension rises as they try to stare each other down."
    " Напряжение растет, когда они пытаются смотреть друг на друга сверху вниз."

# game/scenario/sc_en.rpy:2623
translate russian s26_76b2fe88_12:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2631
translate russian s26_5d8a2456:

    # "------It's dangerous|"
    "------Это опасно."

# game/scenario/sc_en.rpy:2632
translate russian s26_8fb03c64:

    # " I don't know how Arcueid got here, but if they start a fight here, then it'll turn out really bad."
    " Я не знаю как Арквейд попала сюда, но если они начнуть здесь драться, это может закончится плохо."

# game/scenario/sc_en.rpy:2633
translate russian s26_32b40c15:

    # "\"Both of you, wait|{w} You do know this is a school?\""
    "\"Вы обе, подождите!{w} Вы знаете, что вы в школе?\""

# game/scenario/sc_en.rpy:2634
translate russian s26_0bb4bf06:

    # " I carefully try my stopping spell on them."
    " Аккуратно попробовал я своё останавливающее заклинание."

# game/scenario/sc_en.rpy:2635
translate russian s26_76b2fe88_13:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2639
translate russian s26_8dd20ebc:

    # "\"I know.{w} Well, this vampire might not.\""
    "\"Я знаю.{w} Но, это вампир возможно не.\""

# game/scenario/sc_en.rpy:2643
translate russian s26_8e02e873:

    # "\"Ah, it's okay.{w} I didn't come to kill Ciel.\""
    "\"А, всё нормально.{w} Я не собираюсь убивать Сиель.\""

# game/scenario/sc_en.rpy:2644
translate russian s26_a4bfe933:

    # " Unlike Senpai, who emits an aura of death, Arcueid is relaxed."
    " В отличие от Семпай, которая излучала ауру смерти, Арквейд была расслаблена."

# game/scenario/sc_en.rpy:2645
translate russian s26_2b45ff7c_1:

    # "\"|?\""
    "\"?\""

# game/scenario/sc_en.rpy:2646
translate russian s26_e2a7af6c:

    # " Senpai and I tilt our heads in confusion."
    " В смятении наклонили голову я и Семпай."

# game/scenario/sc_en.rpy:2647
translate russian s26_5d792382:

    # " Arcueid looks around the room once or twice and then,"
    " Арквейд оглядела комнату раз или два и затем,"

# game/scenario/sc_en.rpy:2652
translate russian s26_f742ae4b:

    # " In the blink of an eye intercepts Senpai's lunchbox."
    " В мгновение ока хватает коробку с завтраком Сэмпай."

# game/scenario/sc_en.rpy:2653
translate russian s26_76b2fe88_14:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2657
translate russian s26_54b78b54:

    # "\"Wh, what are you doing?!\""
    "\"Чт, что ты творишь?!\""

# game/scenario/sc_en.rpy:2661
translate russian s26_af29b498:

    # "\"Itadakimasu!\""
    "\"Итадакимасу!\""

# game/scenario/sc_en.rpy:2665
translate russian s26_9696607e:

    # " As Senpai watches on blankly, Arcueid gulps down Senpai's lunch in one go."
    " Пока Семаай безучастно наблюдает за происходящим, Арквейд за раз глотает обед Семпай."

# game/scenario/sc_en.rpy:2666
translate russian s26_5f5ece55:

    # " Well| not the lunchbox itself, of course."
    " Ну, без ланчбокса, конечно."

# game/scenario/sc_en.rpy:2667
translate russian s26_e6b598ac:

    # nvl clear
    # "\"Ah|{w} ah|{w} AH!\""
    nvl clear
    "\"А!{w} а!{w} АА!\""

# game/scenario/sc_en.rpy:2669
translate russian s26_f3757557:

    # " Ciel-senpai shakes."
    " Затрясло Сиель-семпай."

# game/scenario/sc_en.rpy:2670
translate russian s26_911c2a02:

    # " Arcueid chews a while and then gulps it down cleanly."
    " Аркуэйд некоторое время жует, а затем аккуратно проглатывает еду."

# game/scenario/sc_en.rpy:2673
translate russian s26_48463153:

    # "\"You|{w} You ate it!{w} You|{w} ate my whole LUNCH!\""
    "\"Ты!{w} Ты съела его!{w} Ты,{w} съела весь мой ОБЕД!\""

# game/scenario/sc_en.rpy:2674
translate russian s26_d3374061:

    # " There's a fiery aura coming from Senpai.{w} Wooow| it's the fiercest yet."
    " От Сэмпай исходит огненная аура.{w} Вааау, эта жестче всего, чем все до этого."

# game/scenario/sc_en.rpy:2675
translate russian s26_9e189e72:

    # nvl clear
    # " Arcueid, on the other hand|"
    nvl clear
    " А Арквейд по другую сторону:"

# game/scenario/sc_en.rpy:2681
translate russian s26_18ca5926:

    # "\"Blech!{w} What is this?!{w} It's terrible!\""
    "\"Блин!{w} Что это?!{w} Это ужасно!\""

# game/scenario/sc_en.rpy:2682
translate russian s26_d6cf8f38:

    # " She pours oil on the fire and clutches her stomach."
    " Подлила она масла в огонь и схватилась за живот."

# game/scenario/sc_en.rpy:2683
translate russian s26_f8249355:

    # nvl clear
    # "\"Te, terrible?!{w} You steal someone's lunch, and then you say that?!{w} What are you?!\""
    nvl clear
    "\"У, ужасно?!{w} Ты крадёшь чей-то обед, и затем ты говоришь такое?!{w} Да кто ты такая?!\""

# game/scenario/sc_en.rpy:2685
translate russian s26_3f52e4d2:

    # " Kuuuaaaah, cries Senpai."
    " Ууууаааах, плачет Семпай."

# game/scenario/sc_en.rpy:2693
translate russian s26_b0de56c9_1:

    # "\"||||||\""
    "\"||||||\""

# game/scenario/sc_en.rpy:2694
translate russian s26_7f43d9ad:

    # " There's only one thing I can do now."
    " Есть только одна вещь, которую я сейчас могу сделать."

# game/scenario/sc_en.rpy:2695
translate russian s26_6e38a3f6:

    # " Hurriedly, I hide in the corner to watch as the story unfolds."
    " Я поспешно прячусь в углу, чтобы понаблюдать за развитием событий."

# game/scenario/sc_en.rpy:2696
translate russian s26_76b2fe88_15:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2701
translate russian s26_3a64f1ea:

    # "\"Huuu| I'm okay now.{w} Really, what Ciel makes is terrible.{w} It's unnecessarily big, and has so many calories.{w} Did you get that, Shiki?{w} Don't eat anything Ciel makes ever again.{w} If you do, you'll become fat just like her.\""
    ""

# game/scenario/sc_en.rpy:2704
translate russian s26_b7009dde:

    # "\"Ah|{w} haha|{w} AHAHAHAHAHAHA!\""
    ""

# game/scenario/sc_en.rpy:2705
translate russian s26_05cbcbc7:

    # " Ah| Senpai's sanity just snapped."
    ""

# game/scenario/sc_en.rpy:2706
translate russian s26_76b2fe88_16:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2709
translate russian s26_56518351:

    # "\"Now|{w} who's fat?{w} Just who?!\""
    ""

# game/scenario/sc_en.rpy:2716
translate russian s26_649c4fd5:

    # " Senpai's right straight tries to dig into Arcueid's face."
    ""

# game/scenario/sc_en.rpy:2719
translate russian s26_1c54ae85:

    # "\"Ho, hold| you're serious?\""
    ""

# game/scenario/sc_en.rpy:2720
translate russian s26_c8d39bb1:

    # " Arcueid retreats, surprised."
    ""

# game/scenario/sc_en.rpy:2721
translate russian s26_1bb8ddb7:

    # " Senpai's straight misses, but her murderous intent is still there."
    ""

# game/scenario/sc_en.rpy:2722
translate russian s26_76b2fe88_17:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2725
translate russian s26_2612a648:

    # "\"Uuuh?{w} You show your real face in a school|{w} that's not adult-like, Ciel.\""
    ""

# game/scenario/sc_en.rpy:2728
translate russian s26_61a408e5:

    # "\"I don't even have to listen.{w} Let's end this today, Arcueid Brunestud.\""
    ""

# game/scenario/sc_en.rpy:2729
translate russian s26_d304b723:

    # "\"Wooow|{w} Ciel| you're serious?{w} Getting mad after I said something truthful|{w} how immature of you.\""
    ""

# game/scenario/sc_en.rpy:2730
translate russian s26_0dd821b1:

    # "\"You|{w} you cat demon!\""
    ""

# game/scenario/sc_en.rpy:2731
translate russian s26_76b2fe88_18:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2740
translate russian s26_db570637:

    # " Senpai's left jab follows."
    ""

# game/scenario/sc_en.rpy:2741
translate russian s26_0af6a5fa_9:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2744
translate russian s26_4e04c907:

    # "\"Oooh-nya!\""
    ""

# game/scenario/sc_en.rpy:2745
translate russian s26_0af6a5fa_10:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2750
translate russian s26_06efb08e:

    # "\"Ciel, you're too soft.\""
    ""

# game/scenario/sc_en.rpy:2751
translate russian s26_0af6a5fa_11:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2756
translate russian s26_816a7be1:

    # "\"Ciel, you missed.\""
    ""

# game/scenario/sc_en.rpy:2757
translate russian s26_0af6a5fa_12:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2762
translate russian s26_e80b8af5:

    # "\"Ciel, you're too inexperienced.\""
    ""

# game/scenario/sc_en.rpy:2763
translate russian s26_0af6a5fa_13:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2764
translate russian s26_08544cb2:

    # " Arcueid plays with Senpai."
    ""

# game/scenario/sc_en.rpy:2765
translate russian s26_76b2fe88_19:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2773
translate russian s26_eb4075b5:

    # "\"Huuuk!{w} There!\""
    ""

# game/scenario/sc_en.rpy:2774
translate russian s26_6a379197:

    # " But even that doesn't last long."
    ""

# game/scenario/sc_en.rpy:2776
translate russian s26_5f6333dd:

    # " BAM!{w} A light sound."
    ""

# game/scenario/sc_en.rpy:2777
translate russian s26_449169a2:

    # " Senpai's jab really hit Arcueid's arm this time."
    ""

# game/scenario/sc_en.rpy:2780
translate russian s26_ee1e815f:

    # "\"Uuuh| a little impressive, Ciel.\""
    ""

# game/scenario/sc_en.rpy:2789
translate russian s26_e035fa22:

    # "\"I won't be played by you forever.{w} That cat face|{w} I'll turn it into a squished dumpling this time!\""
    ""

# game/scenario/sc_en.rpy:2790
translate russian s26_e33d020d:

    # " Oooh| Hitman style!{w} Is Senpai really thinking of killing?"
    ""

# game/scenario/sc_en.rpy:2791
translate russian s26_99bac7ae:

    # nvl clear
    # "\"Uuuh| that's unfair| you get your own sprite| How cheap of you-nya!\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:2801
translate russian s26_076db5b9:

    # "\"Hmm.{w} Such childish thoughts.{w} Now I can read your movements.{w} Your next move will be your last.\""
    ""

# game/scenario/sc_en.rpy:2802
translate russian s26_b56626d9:

    # " Ciel-senpai makes whooshing noises with her hands."
    ""

# game/scenario/sc_en.rpy:2803
translate russian s26_8a973a18:

    # " And from there the left hand whips out to Arcueid.{w} Ah| the role of a villain suits Ciel-senpai.{w} Maybe she should go into acting."
    ""

# game/scenario/sc_en.rpy:2804
translate russian s26_52805981:

    # nvl clear
    # "\"Hmph.{w} I guess I can't help it.{w} Then I'll use my trump card to counter yours-nya.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:2813
translate russian s26_23b482e4:

    # " Arcueid charges Ciel-senpai."
    ""

# game/scenario/sc_en.rpy:2816
translate russian s26_90e2dcfd:

    # "\"I'll kill you!\""
    ""

# game/scenario/sc_en.rpy:2817
translate russian s26_a61fb403:

    # " Senpai victoriously throws her punch."
    ""

# game/scenario/sc_en.rpy:2818
translate russian s26_91663558:

    # " The whip-like left fist ruthlessly drills into Arcueid's face---!"
    ""

# game/scenario/sc_en.rpy:2819
translate russian s26_76b2fe88_20:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2828
translate russian s26_d635cb05:

    # "\"Nya.\""
    ""

# game/scenario/sc_en.rpy:2829
translate russian s26_56955648:

    # " But Arcueid avoids as if it were nothing."
    ""

# game/scenario/sc_en.rpy:2837
translate russian s26_639f78e5:

    # "\"!?\""
    ""

# game/scenario/sc_en.rpy:2838
translate russian s26_74a64d49:

    # " Surprised, Senpai quickly throws her second."
    ""

# game/scenario/sc_en.rpy:2839
translate russian s26_95a62b56:

    # " This time it really hits Arcueid, who lost balance avoiding the last one!"
    ""

# game/scenario/sc_en.rpy:2840
translate russian s26_76b2fe88_21:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2845
translate russian s26_4607b15c:

    # "\"Nya nya.\""
    ""

# game/scenario/sc_en.rpy:2846
translate russian s26_5f238cad:

    # "\"Uh!\""
    ""

# game/scenario/sc_en.rpy:2847
translate russian s26_cbf8bccb:

    # " And then the third!{w} It flies like an arrow."
    ""

# game/scenario/sc_en.rpy:2852
translate russian s26_bd02f6a1:

    # "\"Nya nya nya.\""
    ""

# game/scenario/sc_en.rpy:2853
translate russian s26_90263df9:

    # " Avoided once more."
    ""

# game/scenario/sc_en.rpy:2854
translate russian s26_4519b5ab:

    # "\"Ah, you!!!!!\""
    ""

# game/scenario/sc_en.rpy:2855
translate russian s26_d9a12668:

    # " Swoosh, woosh, woosh!"
    ""

# game/scenario/sc_en.rpy:2856
translate russian s26_c2e54693:

    # " The punches fly like a machine gun."
    ""

# game/scenario/sc_en.rpy:2859
translate russian s26_a1f84743:

    # "\"Nya nya nya nya nya!\""
    ""

# game/scenario/sc_en.rpy:2860
translate russian s26_7c110292:

    # " Arcueid avoids with an oddly familiar circular motion."
    ""

# game/scenario/sc_en.rpy:2861
translate russian s26_76b2fe88_22:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2869
translate russian s26_267cd078:

    # "\"Kuuuuuuh!\""
    ""

# game/scenario/sc_en.rpy:2872
translate russian s26_67d90bd4:

    # " Sha, Senpai quickly widens the distance between them."
    ""

# game/scenario/sc_en.rpy:2873
translate russian s26_2648ec39:

    # " Arcueid doesn't follow and still does her swaying or waving or whatever defensive movement she did to avoid her punches."
    ""

# game/scenario/sc_en.rpy:2874
translate russian s26_b0de56c9_2:

    # "\"||||||\""
    ""

# game/scenario/sc_en.rpy:2882
translate russian s26_d6c5cd3d:

    # " Senpai glares at Arcueid angrily."
    ""

# game/scenario/sc_en.rpy:2883
translate russian s26_9a701f9d:

    # " Arcueid slowly approaches Senpai."
    ""

# game/scenario/sc_en.rpy:2884
translate russian s26_76b2fe88_23:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2887
translate russian s26_c94bb930:

    # "\"Huhuhu|{w} I can easily see that plucker skill.{w} I'll end this and go out to play with Shiki-nya.\""
    ""

# game/scenario/sc_en.rpy:2888
translate russian s26_b0de56c9_3:

    # "\"||||||\""
    ""

# game/scenario/sc_en.rpy:2889
translate russian s26_00951911:

    # " Senpai is silent."
    ""

# game/scenario/sc_en.rpy:2890
translate russian s26_bf78b7ff:

    # " Maybe she gave up after realizing that she can't read Arcueid's ever-changing movements."
    ""

# game/scenario/sc_en.rpy:2891
translate russian s26_1db251fc:

    # "\"It's the end-nya!{w} Die, Ciel!\""
    ""

# game/scenario/sc_en.rpy:2901
translate russian s26_925a0702:

    # " The rushing Arcueid."
    ""

# game/scenario/sc_en.rpy:2902
translate russian s26_4565fa6c:

    # "\"Shhh!!!\""
    ""

# game/scenario/sc_en.rpy:2903
translate russian s26_daf10037:

    # " The running jab of Senpai."
    ""

# game/scenario/sc_en.rpy:2904
translate russian s26_88bd7f32:

    # " But Arcueid revolves and revolves and stops to avoid."
    ""

# game/scenario/sc_en.rpy:2905
translate russian s26_661e58d9:

    # nvl clear
    # " But."
    nvl clear
    ""

# game/scenario/sc_en.rpy:2907
translate russian s26_b25abfad:

    # " At that moment, Senpai puts on a silent smile."
    ""

# game/scenario/sc_en.rpy:2908
translate russian s26_0af6a5fa_14:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2911
translate russian s26_a6a77f41:

    # "\"I knew you were going to stop!\""
    ""

# game/scenario/sc_en.rpy:2912
translate russian s26_0af6a5fa_15:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2913
translate russian s26_fa21aec3:

    # " The half-raised right fist lands a hit on the defenseless Arcueid's face!"
    ""

# game/scenario/sc_en.rpy:2914
translate russian s26_76b2fe88_24:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2923
translate russian s26_48dc4a14:

    # " BAM!{w} The sound of a clean hit."
    ""

# game/scenario/sc_en.rpy:2924
translate russian s26_678813ad:

    # " And in stereo, not mono."
    ""

# game/scenario/sc_en.rpy:2925
translate russian s26_b5bcf055:

    # " At the exact same time, each person's fist digs into the other's face."
    ""

# game/scenario/sc_en.rpy:2926
translate russian s26_ec9ba31b:

    # nvl clear
    # "\"Ah| how grand.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:2928
translate russian s26_9209f4e8:

    # " My murmur can't be heard, probably."
    ""

# game/scenario/sc_en.rpy:2929
translate russian s26_0a84e27b:

    # " Fantastically outstretched, good enough to be put in a textbook.{w} What a cross counter.{w} Senpai's is especially wonderful.{w} Arcueid, trying to counter against a perfect punch, put a spin into hers to increase the damage."
    ""

# game/scenario/sc_en.rpy:2930
translate russian s26_0af6a5fa_16:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2931
translate russian s26_4140fa8c:

    # " Indeed| the strength of Arc, the skill of Ciel."
    ""

# game/scenario/sc_en.rpy:2932
translate russian s26_0af6a5fa_17:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2933
translate russian s26_1a6fbf75:

    # " What perfect timing.{w} Seeing as how they hit each other at the same time, I wonder if these two are secretly connected inside."
    ""

# game/scenario/sc_en.rpy:2934
translate russian s26_2f799a98:

    # nvl clear
    # "\"Kuuu| you can throw a punch, Arcueid|\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:2936
translate russian s26_6a0b58e8:

    # "\"You too-nya|\""
    ""

# game/scenario/sc_en.rpy:2937
translate russian s26_f2dfca3d:

    # " Each of them glares at the other."
    ""

# game/scenario/sc_en.rpy:2938
translate russian s26_ee0bf315:

    # " And then the two collapse on the boxing rin-, no, the tatami mat."
    ""

# game/scenario/sc_en.rpy:2939
translate russian s26_76b2fe88_25:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2942
translate russian s26_04b18d66:

    # "\"Ah| it's about time.\""
    ""

# game/scenario/sc_en.rpy:2943
translate russian s26_44e4160e:

    # " Lunch time is almost over, now that I snap out of it."
    ""

# game/scenario/sc_en.rpy:2944
translate russian s26_e2c050f2:

    # " The two are unconscious and lie on the floor, pinching each other's cheek."
    ""

# game/scenario/sc_en.rpy:2945
translate russian s26_eda34f24:

    # "\"All right, let's go back|{w} Let's just go back|\""
    ""

# game/scenario/sc_en.rpy:2946
translate russian s26_b57f4226:

    # " I don't want to clean up after them, so I just leave as if I've seen something I shouldn't have."
    ""

# game/scenario/sc_en.rpy:2947
translate russian s26_76b2fe88_26:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:2962
translate russian s27_ba8b1800:

    # "This scene is \"School - Lunch time: Cafeteria - The news.\"{nw}"
    ""

# game/scenario/sc_en.rpy:2963
translate russian s27_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:2964
translate russian s27_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:2984
translate russian s27_f3af67f8:

    # "------I'll show you that I'm rich!"
    ""

# game/scenario/sc_en.rpy:2985
translate russian s27_65a02ad1:

    # nvl clear
    # "\"I've decided.{w} I'll eat lavishly at the cafeteria.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:2989
translate russian s27_dc08d446:

    # "\"Eh?{w} Really?{w} Unbelievable!{w} Don't tell me| maybe even an egg on top of a Yubu udon?!\""
    ""

# game/scenario/sc_en.rpy:2990
translate russian s27_e25beca1:

    # " Yubu udon, 250 yen, egg, 50 yen|"
    ""

# game/scenario/sc_en.rpy:2991
translate russian s27_fcaabce6:

    # "\"Arihiko, you can't call that lavish!\""
    ""

# game/scenario/sc_en.rpy:2992
translate russian s27_16ea824b:

    # "\"The, then, two persons' worth?!{w} Stop, nothing good comes out of bluffing!\""
    ""

# game/scenario/sc_en.rpy:2993
translate russian s27_d9f96a83:

    # " Two persons'| {w}plus a hundred yen|"
    ""

# game/scenario/sc_en.rpy:2994
translate russian s27_32996fd0:

    # "\"Donkatsu rice, and vegetable mix with soup.\""
    ""

# game/scenario/sc_en.rpy:2995
translate russian s27_b7c654b9:

    # " Donkatsu rice set, 650 yen."
    ""

# game/scenario/sc_en.rpy:2996
translate russian s27_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3000
translate russian s27_f05d2e90:

    # "\"Ar, are you serious, Tohno-kun?!{w} Buy me something with that money!\""
    ""

# game/scenario/sc_en.rpy:3001
translate russian s27_9dceac38:

    # "\"Quiet!{w} Don't leech off me.\""
    ""

# game/scenario/sc_en.rpy:3004
translate russian s27_38c7a2cc:

    # " I put down my head, sigh and march to the cafeteria like a robot."
    ""

# game/scenario/sc_en.rpy:3005
translate russian s27_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3010
translate russian s27_d496f5c7:

    # "------In the next news item|"
    ""

# game/scenario/sc_en.rpy:3011
translate russian s27_303a2407:

    # " There's been a seventh landslide accident in the city of X-hyun Sakae at seven in the morning."
    ""

# game/scenario/sc_en.rpy:3012
translate russian s27_67d7e1df:

    # " Thankfully damage was minor. The surrounding area was cut off for about an hour, but the repairs started and---"
    ""

# game/scenario/sc_en.rpy:3013
translate russian s27_646043c3:

    # nvl clear
    # " The TV shows today morning's news."
    nvl clear
    ""

# game/scenario/sc_en.rpy:3015
translate russian s27_fd6e747e:

    # " The school cafeteria plays the morning's news recorded by the principal who insists that it teaches the students current events."
    ""

# game/scenario/sc_en.rpy:3016
translate russian s27_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3017
translate russian s27_4ebe6d28:

    # " Slurp, slurp."
    ""

# game/scenario/sc_en.rpy:3018
translate russian s27_0af6a5fa_2:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3019
translate russian s27_9ffb4a1a:

    # " The one student who watches the news as he eats curry udon and eggs."
    ""

# game/scenario/sc_en.rpy:3020
translate russian s27_443fdddf:

    # "\"Whew| what a spineless wimp|\""
    ""

# game/scenario/sc_en.rpy:3021
translate russian s27_918db893:

    # " It goes without saying that that lonely student is Tohno Shiki."
    ""

# game/scenario/sc_en.rpy:3022
translate russian s27_4f705c69:

    # nvl clear
    # "\"Damn it| what's with this feeling of ultimate defeat|\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3024
translate russian s27_89b563dc:

    # " Even when I was fighting vampires, I didn't think I was this pathetic."
    ""

# game/scenario/sc_en.rpy:3025
translate russian s27_a28863a0:

    # nvl clear
    # "{fast}{nw}"
    nvl clear
    ""

# game/scenario/sc_en.rpy:3027
translate russian s27_06791775:

    # " Slurp, slurp, slurp."
    ""

# game/scenario/sc_en.rpy:3028
translate russian s27_0af6a5fa_3:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3029
translate russian s27_c5da3cef:

    # "\"I feel so pathetic|\""
    ""

# game/scenario/sc_en.rpy:3030
translate russian s27_72e62234:

    # " Ah, well| whatever."
    ""

# game/scenario/sc_en.rpy:3031
translate russian s27_babdd0a6:

    # " I didn't get swayed away by whims and saved up money in case I needed it later on."
    ""

# game/scenario/sc_en.rpy:3032
translate russian s27_2a7eb365:

    # nvl clear
    # " Slurp, slurp, slurp."
    nvl clear
    ""

# game/scenario/sc_en.rpy:3034
translate russian s27_0af6a5fa_4:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3035
translate russian s27_8b9cfbf4:

    # " Ah, but even then, the curry udon here is spicy.{w} Senpai says it's just right, but that's probably because Senpai's tongue got paralyzed after eating so much curry and curry only."
    ""

# game/scenario/sc_en.rpy:3036
translate russian s27_a28863a0_1:

    # nvl clear
    # "{fast}{nw}"
    nvl clear
    ""

# game/scenario/sc_en.rpy:3038
translate russian s27_eea62da7:

    # "------In our next story|"
    ""

# game/scenario/sc_en.rpy:3039
translate russian s27_c0b8aae3:

    # " X days ago mysterious murders have started to happen, and today another victim was found."
    ""

# game/scenario/sc_en.rpy:3040
translate russian s27_8913691b:

    # " Near the construction and redevelopment site, what is believed to be human blood has been------"
    ""

# game/scenario/sc_en.rpy:3041
translate russian s27_0af6a5fa_5:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3042
translate russian s27_33024352:

    # "\"--------\""
    ""

# game/scenario/sc_en.rpy:3043
translate russian s27_0c7e71fb:

    # " The unpleasant news continue."
    ""

# game/scenario/sc_en.rpy:3044
translate russian s27_ed1e6108:

    # " Vampires are gone, but there are still mysterious deaths happening around town."
    ""

# game/scenario/sc_en.rpy:3045
translate russian s27_2a7eb365_1:

    # nvl clear
    # " Slurp, slurp, slurp."
    nvl clear
    ""

# game/scenario/sc_en.rpy:3047
translate russian s27_c5a5dbca:

    # " I stand up after finishing the curry udon."
    ""

# game/scenario/sc_en.rpy:3048
translate russian s27_1cf3a7f9:

    # " The unpleasant news made my stomach queasy, so I couldn't drink the soup."
    ""

# game/scenario/sc_en.rpy:3049
translate russian s27_9e137935:

    # " I wondered if Senpai would throw a punch at me as I left the cafeteria."
    ""

# game/scenario/sc_en.rpy:3050
translate russian s27_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3066
translate russian s29_f64a961f:

    # "This scene is \"School - Lunch time: To the courtyard.\"{nw}"
    ""

# game/scenario/sc_en.rpy:3067
translate russian s29_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:3068
translate russian s29_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3086
translate russian s29_6d45518b:

    # " I'll go to the inner courtyard for lunch!"
    ""

# game/scenario/sc_en.rpy:3087
translate russian s29_98edfe48:

    # " I need to have a strong will to fend off Arihiko's provocation."
    ""

# game/scenario/sc_en.rpy:3088
translate russian s29_b0de56c9:

    # "\"||||||\""
    ""

# game/scenario/sc_en.rpy:3089
translate russian s29_7d9f50a0:

    # " Aaah| but before that|{nw}"
    ""

# game/scenario/sc_en.rpy:3090
translate russian s29_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3103
translate russian s32_caf59afd:

    # "This scene is \"School - Lunch time: Courtyard, black cat's rumba.\"{nw}"
    ""

# game/scenario/sc_en.rpy:3104
translate russian s32_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:3105
translate russian s32_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3123
translate russian s32_ba217421:

    # " It'd be nice to eat lunch in the courtyard sometimes."
    ""

# game/scenario/sc_en.rpy:3124
translate russian s32_5850ce6f:

    # "\"I've decided. I'll enjoy the sun as I eat today.\""
    ""

# game/scenario/sc_en.rpy:3128
translate russian s32_a672aa1f:

    # "\"Oooh, you're quite healthy!{w} Then,let me join you!{w} Is what I want to say, but I don't like waning sunlight.{w} So, unfortunately it's goodbye for now, Tohno.\""
    ""

# game/scenario/sc_en.rpy:3129
translate russian s32_6163074e:

    # "\"That's what I wanted.{w} I wanted to eat my lunch quietly today.\""
    ""

# game/scenario/sc_en.rpy:3130
translate russian s32_28b84f64:

    # "\"Is that so?{w} Don't get your lunch stolen by the black cat that supposedly hangs out in the courtyard.\""
    ""

# game/scenario/sc_en.rpy:3138
translate russian s32_fce8f04f:

    # " Arihiko cleanly backs off as if he had something else to do."
    ""

# game/scenario/sc_en.rpy:3139
translate russian s32_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3148
translate russian s32_7652cd15:

    # " I go to the courtyard."
    ""

# game/scenario/sc_en.rpy:3149
translate russian s32_1d1c5011:

    # " The sunlight is adequately warm, and the wind is nice, too."
    ""

# game/scenario/sc_en.rpy:3150
translate russian s32_22ed0b86:

    # " It's the perfect weather for a picnic."
    ""

# game/scenario/sc_en.rpy:3151
translate russian s32_5688912c:

    # " With this kind of a weather, the courtyard must be full.{w} Eh?"
    ""

# game/scenario/sc_en.rpy:3152
translate russian s32_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3153
translate russian s32_3c24a541:

    # "\"But| why is there no one here?\""
    ""

# game/scenario/sc_en.rpy:3154
translate russian s32_0af6a5fa_2:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3155
translate russian s32_ee2c93db:

    # "------For some reason I get angry."
    ""

# game/scenario/sc_en.rpy:3156
translate russian s32_9b3138f8:

    # " It's nice that I can exploit this weather for myself, but it'd still be a waste to spend the day in class."
    ""

# game/scenario/sc_en.rpy:3157
translate russian s32_a70fd409:

    # nvl clear
    # "\"Well, whatever.{w} I'll just lie on the grass.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3159
translate russian s32_d2b1b0c0:

    # " I say to myself as I climb over the fence."
    ""

# game/scenario/sc_en.rpy:3160
translate russian s32_4e3dbebd:

    # " The smell of sunlight rises from the grass.{w} It'd be nice to take a nap just like that."
    ""

# game/scenario/sc_en.rpy:3161
translate russian s32_2d3898d4:

    # nvl clear
    # "\"Mmm?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3163
translate russian s32_ded29abe:

    # " The shadow of a cat in the grass."
    ""

# game/scenario/sc_en.rpy:3173
translate russian s32_8607bc9c:

    # "\"Ah, it's really here.\""
    ""

# game/scenario/sc_en.rpy:3174
translate russian s32_71a475b2:

    # " Arihiko and the rumor were right."
    ""

# game/scenario/sc_en.rpy:3175
translate russian s32_009a4e31:

    # " The black cat sniffed the grass and approached with an attitude that seemed to say 'I'm not at all interested in you.'"
    ""

# game/scenario/sc_en.rpy:3176
translate russian s32_2b71ccf0:

    # nvl clear
    # "\"Eh?{w} I met you last time, too, didn't I?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3178
translate russian s32_feba5969:

    # " My disease of not being able to remember yesterday is affecting me at a time like this."
    ""

# game/scenario/sc_en.rpy:3179
translate russian s32_230643cb:

    # " But, I'm sure.{w} I feel as if I've spent time with a black cat on a nice day like this."
    ""

# game/scenario/sc_en.rpy:3180
translate russian s32_f15e24ff:

    # nvl clear
    # "\"Eh, whatever it may be, do you want some bread?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3182
translate russian s32_a1bc8210:

    # " I tear my sandwich and throw it at the black cat."
    ""

# game/scenario/sc_en.rpy:3185
translate russian s32_2b073a86:

    # " | Woow| it didn't even look!"
    ""

# game/scenario/sc_en.rpy:3192
translate russian s32_2ea17550:

    # "\"Uuh.{w} You seem like you come from a high-class family.\""
    ""

# game/scenario/sc_en.rpy:3193
translate russian s32_784fdd69:

    # " Then I'll try some ham next time."
    ""

# game/scenario/sc_en.rpy:3198
translate russian s32_44b67a96:

    # " Oh, a small reaction this time.{w} But it still doesn't approach.{w} Its pride must be huge."
    ""

# game/scenario/sc_en.rpy:3199
translate russian s32_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3206
translate russian s32_db35f8d2:

    # "\"All right!{w} Next one will be a sure one!{w} Salmon-studded ham!\""
    ""

# game/scenario/sc_en.rpy:3207
translate russian s32_d92ae0b2:

    # " Whoosh, whoosh, whoosh!"
    ""

# game/scenario/sc_en.rpy:3208
translate russian s32_4d084fe3:

    # " One by one salmon-studded ham surrounds the black cat."
    ""

# game/scenario/sc_en.rpy:3220
translate russian s32_1bb51c21:

    # " The black cat turns its back on the ham."
    ""

# game/scenario/sc_en.rpy:3221
translate russian s32_397d55b9:

    # " But, still, more ham!"
    ""

# game/scenario/sc_en.rpy:3233
translate russian s32_bac6c33a:

    # " Then, I'll change my trajectory| and| HAM!"
    ""

# game/scenario/sc_en.rpy:3245
translate russian s32_bf3093ee:

    # " Ham, ham, ham, ham| HAMHAMHAMHAM!!!"
    ""

# game/scenario/sc_en.rpy:3246
translate russian s32_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3251
translate russian s32_25cf9888:

    # "\"Ah| it| fainted|\""
    ""

# game/scenario/sc_en.rpy:3252
translate russian s32_1850cbab:

    # " After turning round and round, the black cat fainted on the grass."
    ""

# game/scenario/sc_en.rpy:3253
translate russian s32_4dad3ee3:

    # "\"Maybe| you really don't like it|\""
    ""

# game/scenario/sc_en.rpy:3254
translate russian s32_4ba7d451:

    # " I try to talk to it softly."
    ""

# game/scenario/sc_en.rpy:3255
translate russian s32_88206d78:

    # " The black cat flinched, and right when it moved its ear|{nw}"
    ""

# game/scenario/sc_en.rpy:3260
translate russian s32_fe6bac91:

    # " With a swoosh it went into the grass field."
    ""

# game/scenario/sc_en.rpy:3261
translate russian s32_43b979a5:

    # nvl clear
    # "\"Ah| did I cause it to hate me?\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3263
translate russian s32_d7639a2d:

    # " Complaining, I bite my sandwich."
    ""

# game/scenario/sc_en.rpy:3264
translate russian s32_c32d6bab:

    # "\"Uegh| it turned into just bread|\""
    ""

# game/scenario/sc_en.rpy:3265
translate russian s32_933225f9:

    # " I'm an idiot."
    ""

# game/scenario/sc_en.rpy:3266
translate russian s32_94a99dee:

    # " I threw all the ingredients away.{w} I deserve this result."
    ""

# game/scenario/sc_en.rpy:3267
translate russian s32_14df5b9b:

    # "\"I won the match, but I guess I lost the competition|\""
    ""

# game/scenario/sc_en.rpy:3268
translate russian s32_0d62b00c:

    # " Mmm| I'll bring some canned cat food next time."
    ""

# game/scenario/sc_en.rpy:3269
translate russian s32_7502f684:

    # " Well|{nw}"
    ""

# game/scenario/sc_en.rpy:3270
translate russian s32_077efa41:

    # " That's if I can remember tomorrow what I did today."
    ""

# game/scenario/sc_en.rpy:3271
translate russian s32_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3286
translate russian s34_f40e5945:

    # "This scene is \"School - Lunch time: Courtyard - No one.\"{nw}"
    ""

# game/scenario/sc_en.rpy:3287
translate russian s34_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:3288
translate russian s34_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3306
translate russian s34_fe39209c:

    # " No, there's something I must check up on."
    ""

# game/scenario/sc_en.rpy:3307
translate russian s34_1581f411:

    # "\"Sorry.{w} I need to do something in the courtyard.{w} I'll have to eat alone.\""
    ""

# game/scenario/sc_en.rpy:3311
translate russian s34_a001bbf5:

    # "\"Ah, I don't care, but what's the matter?{w} You don't look well.\""
    ""

# game/scenario/sc_en.rpy:3312
translate russian s34_3353dd64:

    # "\"Really? Sorry, Arihiko, I just feel that way.\""
    ""

# game/scenario/sc_en.rpy:3313
translate russian s34_c8a243ea:

    # "\"Hmmm| well, if you say so.\""
    ""

# game/scenario/sc_en.rpy:3320
translate russian s34_62e230d4:

    # " He trudges outside."
    ""

# game/scenario/sc_en.rpy:3321
translate russian s34_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3328
translate russian s34_7652cd15:

    # " I go to the courtyard."
    ""

# game/scenario/sc_en.rpy:3329
translate russian s34_1d1c5011:

    # " The sunlight is adequately warm, and the wind is nice, too."
    ""

# game/scenario/sc_en.rpy:3330
translate russian s34_3ea056f5:

    # " Perfect weather for a picnic."
    ""

# game/scenario/sc_en.rpy:3331
translate russian s34_eeff0b0b:

    # " If I lie down here, then maybe that child will come from the grass."
    ""

# game/scenario/sc_en.rpy:3332
translate russian s34_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3333
translate russian s34_ab92e8aa:

    # "\"Or not.\""
    ""

# game/scenario/sc_en.rpy:3334
translate russian s34_0af6a5fa_2:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3335
translate russian s34_7502f684:

    # " Well|{nw}"
    ""

# game/scenario/sc_en.rpy:3336
translate russian s34_2214d131:

    # " She went to end it with him."
    ""

# game/scenario/sc_en.rpy:3337
translate russian s34_c80759d3:

    # " It's a losing bet.{w} She went to her death|{w} how foolish."
    ""

# game/scenario/sc_en.rpy:3338
translate russian s34_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3341
translate russian s34_4842eca8:

    # "\"!!!\""
    ""

# game/scenario/sc_en.rpy:3342
translate russian s34_1ebcb5c6:

    # " I get mad at myself who knows this, but is just lying here."
    ""

# game/scenario/sc_en.rpy:3343
translate russian s34_0765b94f:

    # " Soon the dream will end."
    ""

# game/scenario/sc_en.rpy:3344
translate russian s34_9154d21e:

    # " Before that I'll at least have to go try to help her|"
    ""

# game/scenario/sc_en.rpy:3345
translate russian s34_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3360
translate russian s36_c14849e6:

    # "This scene is \"School - Fifth period: A free moment.\"{nw}"
    ""

# game/scenario/sc_en.rpy:3361
translate russian s36_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:3362
translate russian s36_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3382
translate russian s36_2da0cdc5:

    # " I walk down the hallway full of students."
    ""

# game/scenario/sc_en.rpy:3383
translate russian s36_f0b71181:

    # " Lunch is just about to end, so the hallway is really loud.{w} Perhaps this is the time when the hallway is at its loudest."
    ""

# game/scenario/sc_en.rpy:3384
translate russian s36_5f7f6cfa:

    # " There's a lot of laughter in the noise, so there's no feeling of depression."
    ""

# game/scenario/sc_en.rpy:3385
translate russian s36_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3386
translate russian s36_573e2c66:

    # " The happenings of yesterday, today, tomorrow|"
    ""

# game/scenario/sc_en.rpy:3387
translate russian s36_0af6a5fa_2:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3388
translate russian s36_d68dd699:

    # " All the conversations asked for a smile."
    ""

# game/scenario/sc_en.rpy:3389
translate russian s36_49e4a7c4:

    # nvl clear
    # "\"I guess it has something to do with the upcoming cultural festival, too.\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3391
translate russian s36_f1ea300d:

    # " I realize that even I am a little loosened up as I look outside the window."
    ""

# game/scenario/sc_en.rpy:3392
translate russian s36_5ae094d4:

    # " The face in the reflection seems peaceful."
    ""

# game/scenario/sc_en.rpy:3393
translate russian s36_b24014cd:

    # " A day without any complaints."
    ""

# game/scenario/sc_en.rpy:3394
translate russian s36_6a63fc46:

    # " I didn't have any headaches or anemia attacks recently, and on top of that I didn't even have to take off my glasses for anything."
    ""

# game/scenario/sc_en.rpy:3395
translate russian s36_6bf8c6f4:

    # " The reflected I looks peaceful just observing the hallway."
    ""

# game/scenario/sc_en.rpy:3396
translate russian s36_cf07e420:

    # " A transparent self-portrait."
    ""

# game/scenario/sc_en.rpy:3397
translate russian s36_73952609:

    # " The Tohno Shiki reflected on the glass, like the real Tohno Shiki, can't do anything to affect the world on the other side."
    ""

# game/scenario/sc_en.rpy:3398
translate russian s36_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3402
translate russian s36_0af6a5fa_3:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3403
translate russian s36_0af6a5fa_4:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3404
translate russian s36_0af6a5fa_5:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3405
translate russian s36_0af6a5fa_6:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3406
translate russian s36_0af6a5fa_7:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3407
translate russian s36_0af6a5fa_8:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3408
translate russian s36_08e9b233:

    # "------Suddenly,{nw}"
    ""

# game/scenario/sc_en.rpy:3409
translate russian s36_6f84eb5c:

    # " \ \ \ \ \ I was alone."
    ""

# game/scenario/sc_en.rpy:3410
translate russian s36_b27b8232:

    # nvl clear
    # " The noises were gone."
    nvl clear
    ""

# game/scenario/sc_en.rpy:3412
translate russian s36_ba54694c:

    # " There was no one in the hallway."
    ""

# game/scenario/sc_en.rpy:3413
translate russian s36_cf6e082c:

    # " The rowdy hallway turned into a deserted silence."
    ""

# game/scenario/sc_en.rpy:3414
translate russian s36_e6670582:

    # " It felt as if I were the only one who could not catch the train."
    ""

# game/scenario/sc_en.rpy:3415
translate russian s36_b371a773:

    # " A feeling that something important has just left me alone."
    ""

# game/scenario/sc_en.rpy:3416
translate russian s36_238d4bc5:

    # " Such a sad situation|{w} the only thing that the peaceful reflection did was watch."
    ""

# game/scenario/sc_en.rpy:3417
translate russian s36_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3430
translate russian s37_164fd66d:

    # "This scene is \"School - Fifth period: A free moment 2.\"{nw}"
    ""

# game/scenario/sc_en.rpy:3431
translate russian s37_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:3432
translate russian s37_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3452
translate russian s37_f8e00111:

    # " I walk the rowdy hallway."
    ""

# game/scenario/sc_en.rpy:3453
translate russian s37_336cacf9:

    # " Lunch is almost over."
    ""

# game/scenario/sc_en.rpy:3454
translate russian s37_9e4e3826:

    # " The students in the hallway are chatting dangerously close to the start of the fifth period."
    ""

# game/scenario/sc_en.rpy:3455
translate russian s37_55d59804:

    # " Walking down the hallway amidst the sound of laughter makes me loosen up a bit, too."
    ""

# game/scenario/sc_en.rpy:3456
translate russian s37_d22ca6f2:

    # " Bright sunlight and a peaceful scenery."
    ""

# game/scenario/sc_en.rpy:3457
translate russian s37_b2242900:

    # " An atmosphere where I can easily greet anyone I know, friendly and simply."
    ""

# game/scenario/sc_en.rpy:3458
translate russian s37_2a764554:

    # nvl clear
    # " The day goes on without any major changes."
    nvl clear
    ""

# game/scenario/sc_en.rpy:3460
translate russian s37_01d7f2db:

    # " All the things are half happy, a little sad, and afterwards is boredom."
    ""

# game/scenario/sc_en.rpy:3461
translate russian s37_d4123b3b:

    # " The cycle continues, without knowing an end."
    ""

# game/scenario/sc_en.rpy:3464
translate russian s37_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3465
translate russian s37_9597fcc6:

    # " But I did once think that it was weird."
    ""

# game/scenario/sc_en.rpy:3466
translate russian s37_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3473
translate russian s37_72f53ef5:

    # " That is|"
    ""

# game/scenario/sc_en.rpy:3476
translate russian s37_0af6a5fa_2:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3477
translate russian s37_880e8cc8:

    # " I see it every night and then forget."
    ""

# game/scenario/sc_en.rpy:3480
translate russian s37_0af6a5fa_3:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3481
translate russian s37_8de39916:

    # " It's a dream too brutal to forget."
    ""

# game/scenario/sc_en.rpy:3482
translate russian s37_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3484
translate russian s37_493b8c9b:

    # "\"Ha| what trivial thing am I thinking about.\""
    ""

# game/scenario/sc_en.rpy:3485
translate russian s37_5eec8e73:

    # " I hyped up a dream trivial enough for me to forget into some terrible nightmare."
    ""

# game/scenario/sc_en.rpy:3486
translate russian s37_4bb172f5:

    # " Today, just like yesterday, is a happy day.{w} I don't have to try my best to make it into a sad day."
    ""

# game/scenario/sc_en.rpy:3487
translate russian s37_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3489
translate russian s37_0af6a5fa_4:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3490
translate russian s37_0af6a5fa_5:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3491
translate russian s37_0af6a5fa_6:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3492
translate russian s37_0af6a5fa_7:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3493
translate russian s37_0af6a5fa_8:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3494
translate russian s37_0af6a5fa_9:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3495
translate russian s37_3e4cd39b:

    # "------Well, that is| {w=1.0}{nw}"
    ""

# game/scenario/sc_en.rpy:3496
translate russian s37_5b5395b2:

    # " \ \ \ \ \ \ \ \ \ \ \ I can't even remember| {w=1.0}{nw}"
    ""

# game/scenario/sc_en.rpy:3497
translate russian s37_57c6f98f:

    # " \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ Yesterday|"
    ""

# game/scenario/sc_en.rpy:3498
translate russian s37_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3501
translate russian s37_538454ec:

    # "\"Uuh!\""
    ""

# game/scenario/sc_en.rpy:3502
translate russian s37_2bc90db3:

    # " I feel nauseous and lean on the wall."
    ""

# game/scenario/sc_en.rpy:3503
translate russian s37_0aaa8812:

    # " My face meets the window, so I can watch the outside."
    ""

# game/scenario/sc_en.rpy:3504
translate russian s37_88fe6cb7:

    # " The outside is as usual."
    ""

# game/scenario/sc_en.rpy:3505
translate russian s37_b2f4b8cd:

    # " The reflection is as usual."
    ""

# game/scenario/sc_en.rpy:3506
translate russian s37_30d6554b:

    # " Without any changes, they watch the surroundings, and I cannot immerse myself in them."
    ""

# game/scenario/sc_en.rpy:3507
translate russian s37_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3514
translate russian s37_fee0b082:

    # " Ding! Dong!"
    ""

# game/scenario/sc_en.rpy:3515
translate russian s37_67fcf858:

    # " Is it tinnitus?"
    ""

# game/scenario/sc_en.rpy:3516
translate russian s37_48e76442:

    # " A bell rings far away."
    ""

# game/scenario/sc_en.rpy:3517
translate russian s37_0af6a5fa_10:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3518
translate russian s37_3a2dbf78:

    # "--------Ah, I know, this is the sound of a funeral bell."
    ""

# game/scenario/sc_en.rpy:3519
translate russian s37_07699a82:

    # nvl clear
    # " Ding! Dong!"
    nvl clear
    ""

# game/scenario/sc_en.rpy:3521
translate russian s37_9ebcb1a3:

    # " The crushing bell sound."
    ""

# game/scenario/sc_en.rpy:3522
translate russian s37_2ce91982:

    # " With each ring, a part of the world collapses."
    ""

# game/scenario/sc_en.rpy:3523
translate russian s37_0af6a5fa_11:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3525
translate russian s37_fee0b082_1:

    # " Ding! Dong!"
    ""

# game/scenario/sc_en.rpy:3526
translate russian s37_47e59bb5:

    # " Not here, but somewhere away from here is collapsing."
    ""

# game/scenario/sc_en.rpy:3527
translate russian s37_104a302b:

    # nvl clear
    # " Just like to everything else, there's an end to the world."
    nvl clear
    ""

# game/scenario/sc_en.rpy:3529
translate russian s37_dbd89aa7:

    # " There's no such thing as a perpetual today."
    ""

# game/scenario/sc_en.rpy:3530
translate russian s37_b36fbc31:

    # " With realization, it collapses instantly."
    ""

# game/scenario/sc_en.rpy:3531
translate russian s37_612822d9:

    # " After an irreversible train ride to the farthest station, one grieves."
    ""

# game/scenario/sc_en.rpy:3532
translate russian s37_01f27c81:

    # " Something like an unspeakable ridiculousness?"
    ""

# game/scenario/sc_en.rpy:3533
translate russian s37_ba3f2f46:

    # " The real last station is the one before, but once this happens, there's nothing that can be done."
    ""

# game/scenario/sc_en.rpy:3534
translate russian s37_0eaf4c0e:

    # " As one stands still, the train rides to the last station."
    ""

# game/scenario/sc_en.rpy:3535
translate russian s37_5d3c6d5f:

    # " One station a day."
    ""

# game/scenario/sc_en.rpy:3536
translate russian s37_76b2fe88_5:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3538
translate russian s37_0af6a5fa_12:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3539
translate russian s37_0af6a5fa_13:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3540
translate russian s37_0af6a5fa_14:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3541
translate russian s37_0af6a5fa_15:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3542
translate russian s37_0af6a5fa_16:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3543
translate russian s37_0af6a5fa_17:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3544
translate russian s37_3646b254:

    # " Look, look, everyone."
    ""

# game/scenario/sc_en.rpy:3545
translate russian s37_f4612f1c:

    # " Can I indeed get off at the station right before the last|?"
    ""

# game/scenario/sc_en.rpy:3546
translate russian s37_76b2fe88_6:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3561
translate russian s38_2e09e7cd:

    # "This scene is \"School - Fifth period: A free moment 3.\"{nw}"
    ""

# game/scenario/sc_en.rpy:3562
translate russian s38_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:3563
translate russian s38_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3583
translate russian s38_336cacf9:

    # " Lunch is almost over."
    ""

# game/scenario/sc_en.rpy:3584
translate russian s38_9e4e3826:

    # " The students in the hallway are chatting dangerously close to the start of the fifth period."
    ""

# game/scenario/sc_en.rpy:3585
translate russian s38_55d59804:

    # " Walking down the hallway amidst the sound of laughter makes me loosen up a bit, too."
    ""

# game/scenario/sc_en.rpy:3586
translate russian s38_d22ca6f2:

    # " Bright sunlight and a peaceful scenery."
    ""

# game/scenario/sc_en.rpy:3587
translate russian s38_b2242900:

    # " An atmosphere where I can easily greet anyone I know, friendly and simply."
    ""

# game/scenario/sc_en.rpy:3588
translate russian s38_2a764554:

    # nvl clear
    # " The day goes on without any major changes."
    nvl clear
    ""

# game/scenario/sc_en.rpy:3590
translate russian s38_01d7f2db:

    # " All the things are half happy, a little sad, and afterwards is boredom."
    ""

# game/scenario/sc_en.rpy:3591
translate russian s38_447f61a7:

    # " The cycle continues, not knowing an end."
    ""

# game/scenario/sc_en.rpy:3592
translate russian s38_d54bf62d:

    # nvl clear
    # "\"For some reason, recently|\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3594
translate russian s38_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3595
translate russian s38_98998d2c:

    # " One could say that this balance is weird."
    ""

# game/scenario/sc_en.rpy:3596
translate russian s38_3f121dc8:

    # " Sadness is relatively rare, and only happy things are noticeable."
    ""

# game/scenario/sc_en.rpy:3597
translate russian s38_38432582:

    # " It feels like the best day of a lifetime."
    ""

# game/scenario/sc_en.rpy:3598
translate russian s38_52e938fc:

    # "\"I must be torturing myself unnecessarily.{w} I'm fearing that it would return to normal, when I should just be enjoying the day.\""
    ""

# game/scenario/sc_en.rpy:3599
translate russian s38_be558904:

    # " That's right|"
    ""

# game/scenario/sc_en.rpy:3600
translate russian s38_042827dc:

    # " If I'm afraid of misfortune after happiness, I should at least try my best to enjoy the happiness."
    ""

# game/scenario/sc_en.rpy:3601
translate russian s38_76b2fe88:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3604
translate russian s38_0af6a5fa_2:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3605
translate russian s38_0af6a5fa_3:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3606
translate russian s38_0af6a5fa_4:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3607
translate russian s38_0af6a5fa_5:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3608
translate russian s38_0af6a5fa_6:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3609
translate russian s38_0af6a5fa_7:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3610
translate russian s38_39df1765:

    # " Even though it may just be someone's perfect dream."
    ""

# game/scenario/sc_en.rpy:3611
translate russian s38_76b2fe88_1:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3618
translate russian s38_0af6a5fa_8:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3619
translate russian s38_0af6a5fa_9:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3620
translate russian s38_0af6a5fa_10:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3621
translate russian s38_e6b92987:

    # "--------Suddenly|"
    ""

# game/scenario/sc_en.rpy:3622
translate russian s38_0af6a5fa_11:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3623
translate russian s38_7054010e:

    # "\"Dr| dream?\""
    ""

# game/scenario/sc_en.rpy:3624
translate russian s38_0af6a5fa_12:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3625
translate russian s38_176b3827:

    # "--------I feel like I'm remembering something extremely important."
    ""

# game/scenario/sc_en.rpy:3626
translate russian s38_76b2fe88_2:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3629
translate russian s38_c614403d:

    # " I don't remember yesterday."
    ""

# game/scenario/sc_en.rpy:3630
translate russian s38_809cffc0:

    # " I thought that at first, but was it yesterday that I realized it was a mistake?"
    ""

# game/scenario/sc_en.rpy:3631
translate russian s38_0af6a5fa_13:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3632
translate russian s38_a5a75976:

    # " But I don't remember yesterday."
    ""

# game/scenario/sc_en.rpy:3633
translate russian s38_0af6a5fa_14:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3634
translate russian s38_0ddd0433:

    # " But there's nothing that can't be brought back to memory."
    ""

# game/scenario/sc_en.rpy:3635
translate russian s38_0af6a5fa_15:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3636
translate russian s38_69f29cad:

    # " Realizing that paradox always happens at the end."
    ""

# game/scenario/sc_en.rpy:3637
translate russian s38_35d4a069:

    # " I've probably already realized it."
    ""

# game/scenario/sc_en.rpy:3638
translate russian s38_a10dfc27:

    # " After thinking hard, opening my eyes, then not remembering|{w} this must be|"
    ""

# game/scenario/sc_en.rpy:3639
translate russian s38_76b2fe88_3:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3644
translate russian s38_d26015c0:

    # "\"--------------\""
    ""

# game/scenario/sc_en.rpy:3645
translate russian s38_4b32c17e:

    # " I feel a little nauseous."
    ""

# game/scenario/sc_en.rpy:3646
translate russian s38_7a5e0b40:

    # " Nausea like that caused by staring at the television for many hours."
    ""

# game/scenario/sc_en.rpy:3647
translate russian s38_76b2fe88_4:

    # nvl clear
    nvl clear

# game/scenario/sc_en.rpy:3660
translate russian s39_ad5458e9:

    # "This scene is \"School - Before the fifth period.\"{nw}"
    ""

# game/scenario/sc_en.rpy:3661
translate russian s39_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:3662
translate russian s39_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3682
translate russian s39_185a719c:

    # "\"Tohno-kun, classes have begun.\""
    ""

# game/scenario/sc_en.rpy:3683
translate russian s39_e38159a7:

    # "--------With that|"
    ""

# game/scenario/sc_en.rpy:3684
translate russian s39_783ecd2e:

    # " Mr. Kunihuji starts a conversation from the doorway of his classroom."
    ""

# game/scenario/sc_en.rpy:3685
translate russian s39_d6410dda:

    # "\"Ah, I'm sorry. I was a little out of it.\""
    ""

# game/scenario/sc_en.rpy:3686
translate russian s39_3d1bd3f3:

    # "\"Is that so?{w} Please hurry to your class.\""
    ""

# game/scenario/sc_en.rpy:3687
translate russian s39_59badacd:

    # " Mr. Kunihuji closes the classroom door behind him."
    ""

# game/scenario/sc_en.rpy:3688
translate russian s39_80094ba7:

    # " What| it wasn't anything important."
    ""

# game/scenario/sc_en.rpy:3689
translate russian s39_537334e1:

    # " I just didn't hear the bell and didn't go back to class."
    ""

# game/scenario/sc_en.rpy:3690
translate russian s39_4eaaf9d2:

    # nvl clear
    # "\"--------\""
    nvl clear
    ""

# game/scenario/sc_en.rpy:3692
translate russian s39_f23afa60:

    # " Class has already started."
    ""

# game/scenario/sc_en.rpy:3693
translate russian s39_d764f815:

    # " I should go straight to my classroom, but for some reason|{nw}"
    ""

# game/scenario/sc_en.rpy:3694
translate russian s39_0af6a5fa_1:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3708
translate russian s40_e8068f6c:

    # "This scene is “School - Fifth Period: To the end of the world.”{nw}"
    ""

# game/scenario/sc_en.rpy:3709
translate russian s40_5a857b50:

    # "Would you like to skip?{nw}"
    ""

# game/scenario/sc_en.rpy:3710
translate russian s40_0af6a5fa:

    # "{fast}{nw}"
    ""

# game/scenario/sc_en.rpy:3728
translate russian s40_5473245a:

    # "--------I really don't feel like going to class."
    ""

# game/scenario/sc_en.rpy:3729
translate russian s40_0af6a5fa_1:

    # "{fast}{nw}"
    ""

